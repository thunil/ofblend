...
PYTHON() class Grid4dBase : public PbClass
{
  public:
	...
	inline int getStrideT() const { return mStrideT; }
	...
	inline IndexInt index(const Vec4i &pos) const
	{
		...
		return (IndexInt)pos.x + (IndexInt)mSize.x * pos.y +
		       (IndexInt)mStrideZ * pos.z + (IndexInt)mStrideT * pos.t;
	}
  ...
  protected:
	...
	Vec4i mSize;
	Real mDx;
	// precomputed Z,T shift: to ensure 2D compatibility, always use this
	// instead of sx*sy !
	IndexInt mStrideZ;
	IndexInt mStrideT;
};
...
PYTHON() template <class T> class Grid4d : public Grid4dBase
{
  public:
	...
	PYTHON() void printGrid(int zSlice = -1, int tSlice = -1,
	                        bool printIndex = false, int bnd = 0);
...
{
	return Vec4(Real(s1[0]) / s2[0], Real(s1[1]) / s2[1], Real(s1[2]) / s2[2],
	            Real(s1[3]) / s2[3]);
}
...
inline void Grid4dBase::checkIndex(int i, int j, int k, int t) const
{
	if (i < 0 || j < 0 || i >= mSize.x || j >= mSize.y || k < 0 ||
	    k >= mSize.z || t < 0 || t >= mSize.t) {
		std::ostringstream s;
		s << "Grid4d " << mName << " dim " << mSize << " : index " << i << ","
		  << j << "," << k << "," << t << " out of bound ";
		errMsg(s.str());
	}
}
...
inline void Grid4dBase::checkIndex(IndexInt idx) const
{
	if (idx < 0 || idx >= mSize.x * mSize.y * mSize.z * mSize.t) {
		std::ostringstream s;
		s << "Grid4d " << mName << " dim " << mSize << " : index " << idx
		  << " out of bound ";
		errMsg(s.str());
	}
}
...
{
	return (p.x >= 0 && p.y >= 0 && p.z >= 0 && p.t >= 0 && p.x < mSize.x &&
	        p.y < mSize.y && p.z < mSize.z && p.t < mSize.t);
}
...
KERNEL(idx) template <class T> void Grid4dSetConst(Grid4d<T> &me, T value)
{
	me[idx] = value;
}
...
template <class T>
template <class S>
Grid4d<T> &Grid4d<T>::operator+=(const Grid4d<S> &a)
{
...
template <class T>
template <class S>
Grid4d<T> &Grid4d<T>::operator+=(const S &a)
{
...
void KnInterpolateGrid4dTempl(Grid4d<S> &target, Grid4d<S> &source,
                              const Vec4 &sourceFactor, Vec4 offset)
{
...
//! helper for interpolations, same as in grid.h , but takes an additional
//optional "desired" size
void gridFactor4d(Vec4 s1, Vec4 s2, Vec4 optSize, Vec4 scale, Vec4 &srcFac,
                  Vec4 &retOff);
/*inline void gridFactor4d(Vec4 s1, Vec4 s2, Vec4 optSize, Vec4 scale, Vec4&
srcFac, Vec4& retOff ) {
