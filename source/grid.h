...
PYTHON() class GridBase : public PbClass
{
  public:
	...
	//! Check if indices are within bounds, otherwise error (should only be
	...
	inline IndexInt index(const Vec3i &pos) const
	{
		...
		return (IndexInt)pos.x + (IndexInt)mSize.x * pos.y +
		       (IndexInt)mStrideZ * pos.z;
	}
  ...
  protected:
	...
	Real mDx;
	bool m3D;
	// precomputed Z shift: to ensure 2D compatibility, always use this instead
	// of sx*sy !
	IndexInt mStrideZ;
};
...
PYTHON() template <class T> class Grid : public GridBase
{
  public:
	...
	inline void setInterpolated(const Vec3 &pos, const T &val,
	                            Grid<Real> &sumBuffer) const
	{
	...
	//! warning - do not use "=" for grids in python, this copies the reference!
	// not the grid content...
	...
	//! add scaled other grid to current one (note, only "Real" factor, "T" type
	// not supported here!)
...
{
  public:
	PYTHON() MACGrid(FluidSolver *parent, bool show = true)
	    : Grid<Vec3>(parent, show)
	{
	...
	inline void setInterpolated(const Vec3 &pos, const Vec3 &val, Vec3 *tmp)
	{
	...
	template <int comp>
	inline Real getInterpolatedComponentHi(const Vec3 &pos, int order) const
	{
		switch (order) {
		case 1:
			return interpolComponent<comp>(mData, mSize, mStrideZ, pos);
		case 2:
			return interpolCubicMAC(mData, mSize, mStrideZ,
			                        pos)[comp]; // warning - not yet optimized
		default:
			assertMsg(false, "Unknown interpolation order " << order);
		}
	}
...
{
  public:
	PYTHON() FlagGrid(FluidSolver *parent, int dim = 3, bool show = true)
...
//! helper to compute grid conversion factor between local coordinates of two
// grids
...
// the code below is meant only as an example for a grid with a more complex
// data type
...
template <class S> inline nbVector operator*(const nbVector &v, S s)
{
	assertMsg(false, "Never call!");
	return nbVector();
}
template <class S> inline nbVector operator*(S s, const nbVector &v)
{
	assertMsg(false, "Never call!");
	return nbVector();
}
...
template <>
inline nbVector safeDivide<nbVector>(const nbVector &a, const nbVector &b)
{
	assertMsg(false, "Never call!");
	return nbVector();
}
...
// (python keyword changed here, because the preprocessor does not yet parse
// #ifdefs correctly)
...
{
	// if (i<0 || j<0  || i>=mSize.x || j>=mSize.y || (is3D() && (k<0|| k>=
	// mSize.z))) {
	if (i < 0 || j < 0 || i >= mSize.x || j >= mSize.y || k < 0 ||
	    k >= mSize.z) {
		std::ostringstream s;
		s << "Grid " << mName << " dim " << mSize << " : index " << i << ","
		  << j << "," << k << " out of bound ";
		errMsg(s.str());
	}
}
...
inline void GridBase::checkIndex(IndexInt idx) const
{
	if (idx < 0 || idx >= mSize.x * mSize.y * mSize.z) {
		std::ostringstream s;
		s << "Grid " << mName << " dim " << mSize << " : index " << idx
		  << " out of bound ";
		errMsg(s.str());
	}
}
...
{
	return (p.x >= 0 && p.y >= 0 && p.z >= 0 && p.x < mSize.x &&
	        p.y < mSize.y && p.z < mSize.z);
}

bool GridBase::isInBounds(const Vec3i &p, int bnd) const
{
	bool ret = (p.x >= bnd && p.y >= bnd && p.x < mSize.x - bnd &&
	            p.y < mSize.y - bnd);
	if (this->is3D()) {
...
inline Vec3 MACGrid::getAtMACX(int i, int j, int k) const
{
	...
	Vec3 v = Vec3((mData[idx].x),
	              0.25 * (mData[idx].y + mData[idx - 1].y +
	                      mData[idx + mSize.x].y + mData[idx + mSize.x - 1].y),
	              0.);
	if (this->is3D()) {
		DEBUG_ONLY(checkIndex(idx + mStrideZ - 1));
		v[2] = 0.25 * (mData[idx].z + mData[idx - 1].z +
		               mData[idx + mStrideZ].z + mData[idx + mStrideZ - 1].z);
	}
	return v;
}
...
inline Vec3 MACGrid::getAtMACY(int i, int j, int k) const
{
	DEBUG_ONLY(checkIndex(i + 1, j - 1, k));
	const IndexInt idx = index(i, j, k);
	Vec3 v = Vec3(0.25 * (mData[idx].x + mData[idx - mSize.x].x +
	                      mData[idx + 1].x + mData[idx + 1 - mSize.x].x),
	              (mData[idx].y), 0.);
	if (this->is3D()) {
		DEBUG_ONLY(checkIndex(idx + mStrideZ - mSize.x));
		v[2] = 0.25 *
		       (mData[idx].z + mData[idx - mSize.x].z +
		        mData[idx + mStrideZ].z + mData[idx + mStrideZ - mSize.x].z);
	}
	return v;
}
...
inline Vec3 MACGrid::getAtMACZ(int i, int j, int k) const
{
	...
	Vec3 v = Vec3(0.25 * (mData[idx].x + mData[idx - mStrideZ].x +
	                      mData[idx + 1].x + mData[idx + 1 - mStrideZ].x),
	              0.25 * (mData[idx].y + mData[idx - mStrideZ].y +
	                      mData[idx + mSize.x].y +
...
KERNEL(idx) template <class T, class S>
...
KERNEL(idx) template <class T> void gridSetConst(Grid<T> &grid, T value)
{
	grid[idx] = value;
}
...
void knInterpolateGridTempl(Grid<S> &target, Grid<S> &source,
                            const Vec3 &sourceFactor, Vec3 offset,
                            int orderSpace = 1)
{
...
template <class GRID> inline void interpolGridTempl(GRID &target, GRID &source)
{
	errMsg("interpolGridTempl - Only valid for specific instantiations");
}

template <>
inline void interpolGridTempl<Grid<Real> >(Grid<Real> &target,
                                           Grid<Real> &source)
{
	Vec3 sourceFactor = calcGridSizeFactor(source.getSize(), target.getSize());
	knInterpolateGridTempl<Real>(target, source, sourceFactor,
	                             sourceFactor * 0.5);
}
template <>
inline void interpolGridTempl<Grid<Vec3> >(Grid<Vec3> &target,
                                           Grid<Vec3> &source)
{
	Vec3 sourceFactor = calcGridSizeFactor(source.getSize(), target.getSize());
	knInterpolateGridTempl<Vec3>(target, source, sourceFactor,
	                             sourceFactor * 0.5);
}
