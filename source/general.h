...
extern void updateQtGui(bool full, int frame, float time,
                        const std::string &curPlugin);
#else
// dummy function if GUI is not enabled
inline void updateQtGui(bool full, int frame, float time,
                        const std::string &curPlugin)
{
}
#endif
...
#define throwError(msg)                                                        \
	{                                                                          \
		...
		__s << msg << std::endl << "Error raised in " << __FILE__ << ":"       \
		    << __LINE__;                                                       \
		throw Manta::Error(__s.str());                                         \
	}
#define errMsg(msg) throwError(msg);
#define assertMsg(cond, msg)                                                   \
	if (!(cond)) throwError(msg)
...
template <class T> inline T cubed(T a) { return a * a; }
