...
class WaveletNoiseField : public PbClass
{
  public:
	...
	//! compute wavelet decomposition of an input grid (stores residual
	//coefficients)
	static void computeCoefficients(Grid<Real> &input, Grid<Real> &tempIn1,
	                                Grid<Real> &tempIn2);
  ...
  protected:
	...
	static void downsampleNeumann(const Real *from, Real *to, int n,
	                              int stride);
		...
		for (int y = -1; y <= 1; y++)
			for (int x = -1; x <= 1; x++) {
				...
				neighbors[x + 1][y + 1][z + 1] =
				    data[zC * NOISE_TILE_SIZE * NOISE_TILE_SIZE +
				         yC * NOISE_TILE_SIZE + xC];
			}
...
{
	// gradients of w0-w2
	Vec3 d0 = evaluateVec(pos, 0), d1 = evaluateVec(pos, 1),
