...
typedef struct
{
	...
	int dimX, dimY,
	    dimZ; // underlying solver resolution (all data in local coordinates!)
...
void readBobjFile(const string &name, Mesh *mesh, bool append)
{
	...
	if (!gzf) errMsg("readBobj: unable to open file");
	...
	gzclose(gzf);
	debMsg("read mesh , triangles " << mesh->numTris() << ", vertices "
	                                << mesh->numNodes() << " ",
	       1);
#else
...
void writeBobjFile(const string &name, Mesh *mesh)
{
	...
	if (!gzf) errMsg("writeBobj: unable to open file");
	...
	if (mesh->getType() == Mesh::TypeVortexSheet) {
		...
		for (int tri = 0; tri < numTris; tri++) {
			...
			if (area > 0) triDensity[tri] = vmesh->sheet(tri).smokeAmount;
		}
...
void readObjFile(const std::string &name, Mesh *mesh, bool append)
{
	...
	if (!ifs.good()) errMsg("can't open file '" + name + "'");

	if (!append) mesh->clear();
		...
		if (id == "vt") {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (face.find('/') != string::npos)
					face =
					    face.substr(0, face.find('/')); // ignore other indices
				int idx = atoi(face.c_str()) - 1;
				if (idx < 0) errMsg("invalid face encountered");
				idx += nodebase;
				t.c[i] = idx;
			}
			mesh->addTri(t);
...
template <>
void gridConvertWrite(gzFile &gzf, Grid<double> &grid, void *ptr,
                      UniHeader &head)
{
	head.bytesPerElement = sizeof(float);
	gzwrite(gzf, &head, sizeof(UniHeader));
	float *ptrf = (float *)ptr;
	for (int i = 0; i < grid.getSizeX() * grid.getSizeY() * grid.getSizeZ();
	     ++i, ++ptrf) {
		*ptrf = (float)grid[i];
	}
...
template <>
void gridConvertWrite(gzFile &gzf, Grid<Vector3D<double> > &grid, void *ptr,
                      UniHeader &head)
{
	...
	gzwrite(gzf, ptr,
	        sizeof(Vector3D<float>) * head.dimX * head.dimY * head.dimZ);
}

template <class T>
void pdataConvertWrite(gzFile &gzf, ParticleDataImpl<T> &pdata, void *ptr,
                       UniPartHeader &head)
{
	errMsg("unknown type, not yet supported");
}
...
template <>
void pdataConvertWrite(gzFile &gzf, ParticleDataImpl<int> &pdata, void *ptr,
                       UniPartHeader &head)
{
...
template <>
void pdataConvertWrite(gzFile &gzf, ParticleDataImpl<double> &pdata, void *ptr,
                       UniPartHeader &head)
{
	head.bytesPerElement = sizeof(float);
	gzwrite(gzf, &head, sizeof(UniPartHeader));
...
template <>
void pdataConvertWrite(gzFile &gzf, ParticleDataImpl<Vec3> &pdata, void *ptr,
                       UniPartHeader &head)
{
	head.bytesPerElement = sizeof(Vector3D<float>);
	gzwrite(gzf, &head, sizeof(UniPartHeader));
...
template <>
void gridReadConvert<int>(gzFile &gzf, Grid<int> &grid, void *ptr,
                          int bytesPerElement)
{
	gzread(gzf, ptr,
	       sizeof(int) * grid.getSizeX() * grid.getSizeY() * grid.getSizeZ());
	assertMsg(bytesPerElement == sizeof(int), "grid element size doesn't match "
	                                              << bytesPerElement << " vs "
	                                              << sizeof(int));
...
template <>
void gridReadConvert<double>(gzFile &gzf, Grid<double> &grid, void *ptr,
                             int bytesPerElement)
{
	gzread(gzf, ptr,
	       sizeof(float) * grid.getSizeX() * grid.getSizeY() * grid.getSizeZ());
...
template <>
void gridReadConvert<Vec3>(gzFile &gzf, Grid<Vec3> &grid, void *ptr,
                           int bytesPerElement)
{
	gzread(gzf, ptr, sizeof(Vector3D<float>) * grid.getSizeX() *
	                     grid.getSizeY() * grid.getSizeZ());
...
template <class T>
void pdataReadConvert(gzFile &gzf, ParticleDataImpl<T> &grid, void *ptr,
                      int bytesPerElement)
{
...
template <>
void pdataReadConvert<int>(gzFile &gzf, ParticleDataImpl<int> &pdata, void *ptr,
                           int bytesPerElement)
{
...
template <>
void pdataReadConvert<double>(gzFile &gzf, ParticleDataImpl<double> &pdata,
                              void *ptr, int bytesPerElement)
{
...
template <>
void pdataReadConvert<Vec3>(gzFile &gzf, ParticleDataImpl<Vec3> &pdata,
                            void *ptr, int bytesPerElement)
{
...
template <class T> void writeGridTxt(const string &name, Grid<T> *grid)
{
	...
	if (!ofs.good()) errMsg("can't open file " << name);
	FOR_IJK(*grid)
	{
...
template <class T> void readGridRaw(const string &name, Grid<T> *grid)
{
	...
	assertMsg(bytes == readBytes,
	          "can't read raw file, stream length does not match, "
	              << bytes << " vs " << readBytes);
	gzclose(gzf);
#else
	debMsg("file format not supported without zlib", 1);
...
PYTHON() void printUniFileInfoString(const string &name)
{
	...
	if (gzf) {
		...
		if (!strcmp(ID, "MNT2")) {
			UniHeader head;
			assertMsg(gzread(gzf, &head, sizeof(UniHeader)) ==
			              sizeof(UniHeader),
...
template <class T> void writeGridUni(const string &name, Grid<T> *grid)
{
	...
	// "misuse" temp grid as storage for floating point values (we have double,
...
template <class T> void readGridUni(const string &name, Grid<T> *grid)
{
	...
	if (!strcmp(ID, "DDF2")) {
		...
		assertMsg(gzread(gzf, &head, sizeof(UniLegacyHeader)) ==
		              sizeof(UniLegacyHeader),
		          "can't read file, no header present");
		assertMsg(head.dimX == grid->getSizeX() &&
		              head.dimY == grid->getSizeY() &&
		              head.dimZ == grid->getSizeZ(),
		          "grid dim doesn't match");
		assertMsg(head.bytesPerElement * head.elements == sizeof(T),
		          "grid type doesn't match");
		// skip flags
		...
		assertMsg(gzread(gzf, &head, sizeof(UniLegacyHeader2)) ==
		              sizeof(UniLegacyHeader2),
		          "can't read file, no header present");
		assertMsg(
		    head.dimX == grid->getSizeX() && head.dimY == grid->getSizeY() &&
		        head.dimZ == grid->getSizeZ(),
		    "grid dim doesn't match, " << Vec3(head.dimX, head.dimY, head.dimZ)
		                               << " vs " << grid->getSize());
		assertMsg(head.gridType == grid->getType(),
		          "grid type doesn't match " << head.gridType << " vs "
		                                     << grid->getType());
		assertMsg(head.bytesPerElement == sizeof(T),
		          "grid element size doesn't match " << head.bytesPerElement
		                                             << " vs " << sizeof(T));
		gzread(gzf, &((*grid)[0]),
		       sizeof(T) * head.dimX * head.dimY * head.dimZ);
		...
		assertMsg(
		    ...
		    "grid dim doesn't match, " << Vec3(head.dimX, head.dimY, head.dimZ)
		                               << " vs " << grid->getSize());
		assertMsg(unifyGridType(head.gridType) ==
		              unifyGridType(grid->getType()),
		          "grid type doesn't match " << head.gridType << " vs "
		                                     << grid->getType());
#if FLOATINGPOINT_PRECISION != 1
...
{
	debMsg("writing real grid " << grid->getName() << " to vol file " << name,
	       1);
	...
	fwrite(&(*grid)[0], sizeof(float),
	       grid->getSizeX() * grid->getSizeY() * grid->getSizeZ(), fp);
#else
	// explicitly convert each entry to float - we might have double precision
	// in mantaflow
	FOR_IDX(*grid)
	{
...
{
	debMsg("reading real grid " << grid->getName() << " from vol file " << name,
	       1);
	...
	if (header.dimX != grid->getSizeX() || header.dimY != grid->getSizeY() ||
	    header.dimZ != grid->getSizeZ())
		errMsg("grid dim doesn't match, "
		       << Vec3(header.dimX, header.dimY, header.dimZ) << " vs "
		       << grid->getSize());
#if FLOATINGPOINT_PRECISION != 1
	errMsg("Not yet supported");
#else
	fread(&((*grid)[0]), 1,
	      sizeof(float) * header.dimX * header.dimY * header.dimZ, fp);
#endif
...
template <class T>
void readGrid4dUni(const string &name, Grid4d<T> *grid, int readTslice,
                   Grid4d<T> *slice, void **fileHandle)
{
	if (grid)
		debMsg("reading grid " << grid->getName() << " from uni file " << name,
		       1);
	if (slice)
		debMsg("reading slice " << slice->getName() << ",t=" << readTslice
		                        << " from uni file " << name,
		       1);
#if FLOATINGPOINT_PRECISION != 1
	errMsg("NYI");
	...
	// optionally - reuse file handle, if valid one is passed in fileHandle
	// pointer...
	if ((!fileHandle) || (fileHandle && (*fileHandle == NULL))) {
		...
		void *ptr = &((*slice)[0]);
		gzread(gzf, ptr,
		       sizeof(T) * slice->getStrideT() * 1); // quick and dirty...
		return;
	}
	...
	if (!strcmp(ID, "M4T2")) {
		...
		if (readTslice < 0) {
			assertMsg(head.dimX == grid->getSizeX() &&
			              ...
			              head.dimZ == grid->getSizeZ(),
			          "grid dim doesn't match, "
			              << Vec3(head.dimX, head.dimY, head.dimZ) << " vs "
			              << grid->getSize());
			assertMsg(unifyGridType(head.gridType) ==
			              unifyGridType(grid->getType()),
			          "grid type doesn't match " << head.gridType << " vs "
			...
			// read full 4d grid
			assertMsg(fourthDim == grid->getSizeT(), "grid dim4 doesn't match, "
			                                             << fourthDim << " vs "
			                                             << grid->getSize());
			...
			for (int t = 0; t < fourthDim; ++t) {
				void *ptr = &((*grid)[head.dimX * head.dimY * head.dimZ * t]);
				gzread(gzf, ptr,
				       sizeof(T) * head.dimX * head.dimY * head.dimZ * 1);
			}
		} else {
			assertMsg(head.dimX == slice->getSizeX() &&
			              head.dimY == slice->getSizeY() &&
			              head.dimZ == slice->getSizeZ(),
			          "grid dim doesn't match, "
			              << Vec3(head.dimX, head.dimY, head.dimZ) << " vs "
			              << slice->getSize());
			assertMsg(unifyGridType(head.gridType) ==
			              unifyGridType(slice->getType()),
			          "grid type doesn't match " << head.gridType << " vs "
			                                     << slice->getType());

			assertMsg(slice, "No 3d slice grid data given");
			assertMsg(readTslice < fourthDim, "grid dim4 slice too large "
			                                      << readTslice << " vs "
			                                      << fourthDim);
			void *ptr = &((*slice)[0]);
			gzseek(gzf,
			       sizeof(T) * head.dimX * head.dimY * head.dimZ * readTslice +
			           sizeof(UniHeader) + sizeof(int) + 4,
			       SEEK_SET);
...
{
	debMsg("writing particles " << parts->getName() << " to uni file " << name,
	       1);
...
#else
	assertMsg(sizeof(BasicParticleData) == PartSysSize,
	          "particle data size doesn't match");
	gzwrite(gzf, &head, sizeof(UniPartHeader));
...
{
	debMsg("reading particles " << parts->getName() << " from uni file "
	                            << name,
	       1);
	...
	if (!strcmp(ID, "PB01")) {
		...
		// current file format
		UniPartHeader head;
		assertMsg(gzread(gzf, &head, sizeof(UniPartHeader)) ==
		              sizeof(UniPartHeader),
		          "can't read file, no header present");
		assertMsg(
		    ((head.bytesPerElement == PartSysSize) && (head.elementType == 0)),
		...
		assertMsg(bytes == readBytes,
		          "can't read uni file, stream length does not match, "
		              << bytes << " vs " << readBytes);
#endif
...
{
	debMsg("writing particle data " << pdata->getName() << " to uni file "
	                                << name,
	       1);
...
#if FLOATINGPOINT_PRECISION != 1
	// always write float values, even if compiled with double precision (as for
	// grids)
...
{
	debMsg("reading particle data " << pdata->getName() << " from uni file "
	                                << name,
	       1);
...
#else
		assertMsg(
		    ((head.bytesPerElement == sizeof(T)) && (head.elementType == 1)),
...
template void writePdataUni<int>(const std::string &name,
                                 ParticleDataImpl<int> *pdata);
template void writePdataUni<Real>(const std::string &name,
                                  ParticleDataImpl<Real> *pdata);
template void writePdataUni<Vec3>(const std::string &name,
                                  ParticleDataImpl<Vec3> *pdata);
template void readPdataUni<int>(const std::string &name,
                                ParticleDataImpl<int> *pdata);
template void readPdataUni<Real>(const std::string &name,
                                 ParticleDataImpl<Real> *pdata);
template void readPdataUni<Vec3>(const std::string &name,
                                 ParticleDataImpl<Vec3> *pdata);

template void readGrid4dUni<int>(const string &name, Grid4d<int> *grid,
                                 int readTslice, Grid4d<int> *slice,
                                 void **fileHandle);
template void readGrid4dUni<Real>(const string &name, Grid4d<Real> *grid,
                                  int readTslice, Grid4d<Real> *slice,
                                  void **fileHandle);
template void readGrid4dUni<Vec3>(const string &name, Grid4d<Vec3> *grid,
                                  int readTslice, Grid4d<Vec3> *slice,
                                  void **fileHandle);
template void readGrid4dUni<Vec4>(const string &name, Grid4d<Vec4> *grid,
                                  int readTslice, Grid4d<Vec4> *slice,
                                  void **fileHandle);
...
template <>
void writeGridRaw<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
template <>
void writeGridUni<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
template <>
void writeGridVol<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
template <>
void writeGridTxt<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
template <> void readGridRaw<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
template <> void readGridUni<nbVector>(const string &name, Grid<nbVector> *grid)
{
	assertMsg(false, "Not supported right now.");
};
