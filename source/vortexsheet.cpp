...
VortexSheetMesh::VortexSheetMesh(FluidSolver *parent)
    : Mesh(parent), mTexOffset(0.0f)
{
...
void VortexSheetMesh::calcVorticity()
{
	for (size_t tri = 0; tri < mTris.size(); tri++) {
		...
		if (area < 1e-10) {
			...
			v.smokeAmount = 0;
			v.vorticity = (v.circulation[0] * e0 + v.circulation[1] * e1 +
			               v.circulation[2] * e2) /
			              area;
		}
	}
...
void VortexSheetMesh::calcCirculation()
{
	for (size_t tri = 0; tri < mTris.size(); tri++) {
		...
		float cx, cy, cz;
		SolveOverconstraint34(e0.x, e0.y, e0.z, e1.x, e1.y, e1.z, e2.x, e2.y,
		                      e2.z, v.vorticity.x, v.vorticity.y, v.vorticity.z,
		                      cx, cy, cz);
