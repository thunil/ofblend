...
{
	Vec3 p = m.nodes(m.corners(ca.prev).node).pos +
	         m.nodes(m.corners(ca.next).node).pos;
...
Vec3 OneSidedButterflySubdivision(Mesh &m, const int valence, const Corner &c)
{
	...
	if (valence == 3) {
		...
		for (int j = 0; j < valence; j++) {
			double s =
			    (0.25 + cos(2 * M_PI * j * rv) + 0.5 * cos(4 * M_PI * j * rv)) *
			    rv;
			Vec3 p = m.nodes(m.corners(m.corners(current).prev).node).pos;

			out += s * p;
			current =
			    m.corners(m.corners(m.corners(current).next).opposite).next;
		}
...
// Interpolating Subdivision for Meshes with Arbitrary Topology
// Denis Zorin, Peter Schroder, and Wim Sweldens
Vec3 ModifiedButterflySubdivision(Mesh &m, const Corner &ca, const Corner &cb,
                                  const Vec3 &fallback)
{
...
void CollapseEdge(Mesh &m, const int trinum, const int which,
                  const Vec3 &edgevect, const Vec3 &endpoint,
                  vector<int> &deletedNodes, std::map<int, bool> &taintedTris,
                  int &numCollapses, bool doTubeCutting)
{
	if (gAbort) return;
	...
	//      Delete references to Corners of deleted triangles in both P0 and
	//      P1's Corner list
	...
	if (nonmanifold && ca_old[1].opposite >= 0 && cb_old[1].opposite >= 0 &&
	    ca_old[2].opposite >= 0 &&
	    cb_old[2].opposite >=
	        0) // collapsing this edge would create a non-manifold edge
	{
		...
		if (m.corners(ca_old[1].opposite).node ==
		    m.corners(ca_old[2].opposite).node)
			botTet = true;

		if (m.corners(cb_old[1].opposite).node ==
		    m.corners(cb_old[2].opposite).node)
			topTet = true;
		...
		if (topTet ^ botTet) {
			...
			// update tri props of all adjacent triangles of P0,P1 (do before CT
			// updates!)
			for (int i = 0; i < m.numTriChannels(); i++) {
			}; // TODO: handleTriPropertyEdgeCollapse(trinum, P2,P3,  cc_old[0],
			   // cd_old[0]);
			...
			if (cl > 2) { // nonmanifold
				// this can happen if collapsing the first tet leads to another
				// similar collapse that requires the collapse of a tet.
				// for now, just move on and pick this up later.

				// if the original component was very small, this first collapse
				// could have led to a tiny piece of nonmanifold geometry.
		...
		} else if (topTet && botTet && ca_old[1].opposite >= 0 &&
		           ca_old[2].opposite >= 0 && cb_old[1].opposite >= 0 &&
		           cb_old[2].opposite >= 0) {
			if (!(m.corners(ca_old[1].opposite).node ==
			          m.corners(ca_old[2].opposite).node &&
			      m.corners(cb_old[1].opposite).node ==
			          m.corners(cb_old[2].opposite).node &&
				...
				// collapse top
				{
					...
					// update tri props of all adjacent triangles of P0,P1 (do
					// before CT updates!)
					// TODO: handleTriPropertyEdgeCollapse(trinum, P2,P3,
					// cc_old[0], cd_old[0]);
					...
					// Preserve connectivity in both triangles
					if (cc_old[1].opposite >= 0)
						m.corners(cc_old[1].opposite).opposite =
						    cc_old[2].opposite;
					if (cc_old[2].opposite >= 0)
						m.corners(cc_old[2].opposite).opposite =
						    cc_old[1].opposite;
					if (cd_old[1].opposite >= 0)
						m.corners(cd_old[1].opposite).opposite =
						    cd_old[2].opposite;
					if (cd_old[2].opposite >= 0)
						m.corners(cd_old[2].opposite).opposite =
						    cd_old[1].opposite;
				...
				// Though we've collapsed a lot of stuff, we still haven't
				// collapsed the original edge.
				// At this point we still haven't guaranteed that this original
				// collapse weill be safe.
				// quit for now, and we'll catch the remaining short edges the
			...
			int P2 = commonVert;
			int P1P2 = -1, P2P1, P2P0 = -1,
			    P0P2 = -1; // corners across from the cutting seam
			...
			// offset the verts in the normal directions to avoid self
			// intersections
			Vec3 offsetVec = cross(m.nodes(P1).pos - m.nodes(P0).pos,
			                       m.nodes(P2).pos - m.nodes(P0).pos);
			normalize(offsetVec);
			offsetVec *= 0.01; // HACK:
			...
			// create a list of all triangles which touch P0, P1, and P2 from
			// the top,
			...
			for (map<int, bool>::iterator tti = topTris.begin();
			     tti != topTris.end(); tti++) {
				// cout << "H " << tti->first << " : " <<
				// m.tris(tti->first).c[0] << " " << m.tris(tti->first).c[1] <<
				// " " << m.tris(tti->first).c[2] << " " << endl;
				for (int i = 0; i < 3; i++) {
					int cn = m.tris(tti->first).c[i];
					set<int> &ring = m.get1Ring(cn).nodes;

					if (ring.find(P0) != ring.end() && cn != P0 && cn != P1 &&
					    cn != P2 && cn != P0b && cn != P1b && cn != P2b) {
						ring.erase(P0);
						ring.insert(P0b);
						m.get1Ring(P0).nodes.erase(cn);
						m.get1Ring(P0b).nodes.insert(cn);
					}
					if (ring.find(P1) != ring.end() && cn != P0 && cn != P1 &&
					    cn != P2 && cn != P0b && cn != P1b && cn != P2b) {
						ring.erase(P1);
						ring.insert(P1b);
					...
					if (ring.find(P2) != ring.end() && cn != P0 && cn != P1 &&
					    cn != P2 && cn != P0b && cn != P1b && cn != P2b) {
						ring.erase(P2);
						ring.insert(P2b);
	...
	if (ca_old[1].opposite >= 0 && ca_old[2].opposite >= 0 &&
	    cb_old[1].opposite >= 0 && cb_old[2].opposite >= 0 &&
	    ca_old[0].opposite >= 0 && cb_old[0].opposite >= 0 &&
	    ((m.corners(ca_old[1].opposite).node ==
	          m.corners(ca_old[2].opposite)
	              .node && // two-pyramid tubey case (6 tris, 5 verts)
	      m.corners(cb_old[1].opposite).node ==
	          m.corners(cb_old[2].opposite).node &&
	      (m.corners(ca_old[1].opposite).node ==
	           m.corners(cb_old[1].opposite).node ||
	       (m.corners(ca_old[1].opposite).node ==
	            cb_old[0].node && // single tetrahedron case
	        m.corners(cb_old[1].opposite).node == ca_old[0].node))) ||
	     (m.corners(ca_old[0].opposite).tri ==
	          m.corners(cb_old[0].opposite).tri &&
	      m.corners(ca_old[1].opposite).tri ==
	          m.corners(cb_old[0].opposite).tri &&
	      ...
	      m.corners(cb_old[0].opposite).tri ==
	          m.corners(ca_old[0].opposite).tri &&
	      m.corners(cb_old[1].opposite).tri ==
	          m.corners(ca_old[0].opposite).tri &&
	      m.corners(cb_old[2].opposite).tri ==
	          m.corners(ca_old[0].opposite).tri))) {
		...
		for (map<int, bool>::iterator mit = markedTris.begin();
		     mit != markedTris.end(); mit++) {
		...
		for (map<int, bool>::iterator mit = markedverts.begin();
		     mit != markedverts.end(); mit++)
	...
	// update tri props of all adjacent triangles of P0,P1 (do before CT
	// updates!)
	// TODO: handleTriPropertyEdgeCollapse(trinum, P0,P1,  ca_old[0],
	// cb_old[0]);
