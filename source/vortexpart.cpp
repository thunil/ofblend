...
inline Vec3 VortexKernel(const Vec3 &p, const vector<VortexParticleData> &vp,
                         Real scale)
{
	Vec3 u(0.0);
...
KERNEL(pts) returns(vector<Vec3> u(size)) vector<Vec3> KnVpAdvectMesh(
    vector<Node> &nodes, const vector<VortexParticleData> &vp, Real scale)
{
...
KERNEL(pts) returns(vector<Vec3> u(size)) vector<Vec3> KnVpAdvectSelf(
    vector<VortexParticleData> &vp, Real scale)
{
...
void VortexParticleSystem::applyToMesh(Mesh &mesh, Real scale,
                                       int integrationMode)
{
	KnVpAdvectMesh kernel(mesh.getNodeData(), mData,
