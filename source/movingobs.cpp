    ...
    : PbClass(parent), mEmptyType(emptyType)
{
	...
	sIDcnt++;
	if (sIDcnt > 15)
		errMsg("currently only 5 separate moving obstacles supported (are you "
		       "generating them in a loop?)");
}

void MovingObstacle::add(Shape *shape) { mShapes.push_back(shape); }
...
void MovingObstacle::moveLinear(Real t, Real t0, Real t1, Vec3 p0, Vec3 p1,
                                FlagGrid &flags, MACGrid &vel, bool smooth)
{
	...
	if (alpha >= 0 && alpha <= 1) {
		...
		FOR_IDX(flags)
		{
			if ((flags[idx] & mID) != 0) flags[idx] = mEmptyType;
		}
		// apply new flags
		for (size_t i = 0; i < mShapes.size(); i++) {
#if NOPYTHON != 1
