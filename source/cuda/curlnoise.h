...
struct CudaNoiseDev
{
	...
	__device__ inline float3 synthesizeK41(CompressedTex tex, const float3 &p,
	                                       int octaves, float L0);
};
...
__device__ inline float3 CudaNoiseDev::eval(CompressedTex noise,
                                            const float3 &p)
{
	float cache[12];
...
__device__ inline float3 CudaNoiseDev::synthesizeK41(CompressedTex tex,
                                                     const float3 &p,
                                                     int octaves, float L0)
{
