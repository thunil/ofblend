...
{
	if (er != cudaSuccess) {
		throw Error("CUDA assert error: " +
		            std::string(cudaGetErrorString(er)));
	}
}
// *****************************************************************
...
{
	float *x, *y, *z;
	__device__ inline float3 get(int i) const
	{
		return make_float3(x[i], y[i], z[i]);
	};
	__device__ inline void set(int i, const float3 &v)
	{
		x[i] = v.x;
		y[i] = v.y;
		z[i] = v.z;
...
struct CVec3Array
{
	...
	CVec3Ptr data()
	{
		CVec3Ptr a = {thrust::raw_pointer_cast(dx.data()),
...
{
	CArray(const std::vector<T> &v) : dev(v.begin(), v.end()) {}
	CArray(int sz)
	{
		dev.resize(sz);
		host.resize(sz);
	}
	T *data() { return thrust::raw_pointer_cast(dev.data()); }
	void upload()
	{
		if (host.size() == dev.size()) dev = host;
	}
	void download() { host = dev; }
