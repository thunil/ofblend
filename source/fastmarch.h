...
//  This class exists in two versions: for scalar, and for vector values - the
//  only difference are
...
class FmHeapEntryOut
{
  public:
	...
	Real time;
	static inline bool compare(const Real x, const Real y) { return x > y; }
...
template <class T, int TDIR> class FastMarch
{
  ...
  public:
	...
	FastMarch(FlagGrid &flags, Grid<int> &fmFlags, Grid<Real> &levelset,
	          Real maxTime, MACGrid *velTransport = NULL);
	~FastMarch() {}
  ...
  protected:
	...
	template <int C>
	inline Real calcWeights(int &okCnt, int &invcnt, Real *v, const Vec3i &idx);
