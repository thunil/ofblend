...
{
	if (flags.isFluid(idx) && grid[idx] > 0) grid[idx] = 1.0 / grid[idx];
}

//! Kernel: Squared sum over grid
KERNEL(idx, reduce = +)
    returns(double sum = 0) double GridSumSqr(Grid<Real> &grid)
{
...
{
	Vec3 v = Vec3(0., 0., 0.5 * ((grid(i + 1, j, k).y - grid(i - 1, j, k).y) -
...
{
	Vec3 del =
	    Vec3(grid(i + 1, j, k).x, grid(i, j + 1, k).y, 0.) - grid(i, j, k);
	if (grid.is3D())
...
{
	Vec3 grad =
	    (Vec3(grid(i, j, k)) - Vec3(grid(i - 1, j, k), grid(i, j - 1, k), 0.));
	if (grid.is3D())
		grad[2] -= grid(i, j, k - 1);
	else
		grad[2] = 0.;
...
{
	laplace(i, j, k) =
	    -(6.0 * grid(i, j, k) - grid(i + 1, j, k) - grid(i - 1, j, k) -
	      grid(i, j + 1, k) - grid(i, j - 1, k) - grid(i, j, k + 1) -
	      grid(i, j, k - 1));
}

//! Kernel: get component at MAC positions
KERNEL(bnd = 1) void GetShiftedComponent(const Grid<Vec3> &grid,
                                         Grid<Real> &comp, int dim)
{
	Vec3i ishift(i, j, k);
	ishift[dim]--;
...
{
	Vec3 v =
	    0.5 * (vel(i, j, k) + Vec3(vel(i + 1, j, k).x, vel(i, j + 1, k).y, 0.));
	if (vel.is3D())
...
{
	Vec3 v = 0.5 * (center(i, j, k) +
	                Vec3(center(i - 1, j, k).x, center(i, j - 1, k).y, 0.));
	if (vel.is3D())
...
//! Fill in the domain boundary cells (i,j,k=0/size-1) from the neighboring
//cells
