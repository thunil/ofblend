...
template <class T> Grid<T>::Grid(const Grid<T> &a) : GridBase(a.getParent())
{
	mSize = a.mSize;
	mType = a.mType;
	mStrideZ = a.mStrideZ;
...
template <class T> Grid<T>::~Grid() { mParent->freeGridPointer<T>(mData); }
...
returns(Real minVal = std::numeric_limits<Real>::max()) Real
...
KERNEL(idx, reduce = max)
returns(Real maxVal = -std::numeric_limits<Real>::max()) Real
...
KERNEL(idx, reduce = min) returns(
    int minVal = std::numeric_limits<int>::max()) int CompMinInt(Grid<int> &val)
{
	if (val[idx] < minVal) minVal = val[idx];
}
...
KERNEL(idx, reduce = max)
returns(int maxVal = -std::numeric_limits<int>::max()) int CompMaxInt(
    Grid<int> &val)
{
	if (val[idx] > maxVal) maxVal = val[idx];
}
...
{
	assertMsg(
	    a.mSize.x == mSize.x && a.mSize.y == mSize.y && a.mSize.z == mSize.z,
...
KERNEL(idx) template <class T> void knGridSetConstReal(Grid<T> &me, T val)
{
	me[idx] = val;
}
...
KERNEL(idx) template <class T> void knGridClamp(Grid<T> &me, T min, T max)
{
...
template <> Real Grid<Real>::getMax() { return CompMaxReal(*this); }
template <> Real Grid<Real>::getMin() { return CompMinReal(*this); }
...
template <> Real Grid<Vec3>::getMax() { return sqrt(CompMaxVec(*this)); }
template <> Real Grid<Vec3>::getMin() { return sqrt(CompMinVec(*this)); }
template <> Real Grid<Vec3>::getMaxAbs() { return sqrt(CompMaxVec(*this)); }
template <> Real Grid<int>::getMax() { return (Real)CompMaxInt(*this); }
template <> Real Grid<int>::getMin() { return (Real)CompMinInt(*this); }
...
PYTHON() Real gridMaxDiffInt(Grid<int> &g1, Grid<int> &g2)
{
	...
	{
		maxVal =
		    std::max(maxVal, (double)fabs((double)g1(i, j, k) - g2(i, j, k)));
	}
	return maxVal;
}
...
// simple helper functions to copy (convert) mac to vec3 , and levelset to real
// grids
// (are assumed to be the same for running the test cases - in general they're
// not!)
PYTHON() void copyMacToVec3(MACGrid &source, Grid<Vec3> &target)
{
	FOR_IJK(target) { target(i, j, k) = source(i, j, k); }
}

PYTHON() void convertMacToVec3(MACGrid &source, Grid<Vec3> &target)
{
	debMsg(
	    "Deprecated - do not use convertMacToVec3... use copyMacToVec3 instead",
	    1);
...
PYTHON() void resampleVec3ToMac(Grid<Vec3> &source, MACGrid &target)
{
	...
	{
		target(i, j, k)[0] =
		    0.5 * (source(i - 1, j, k)[0] + source(i, j, k))[0];
		target(i, j, k)[1] =
		    0.5 * (source(i, j - 1, k)[1] + source(i, j, k))[1];
		if (target.is3D()) {
			target(i, j, k)[2] =
			    0.5 * (source(i, j, k - 1)[2] + source(i, j, k))[2];
		}
	}
}
...
{
	FOR_IJK_BND(target, 1) { target(i, j, k) = source.getCentered(i, j, k); }
}
...
{
	debMsg("Deprecated - do not use convertLevelsetToReal... use "
	       "copyLevelsetToReal instead",
	       1);
...
PYTHON() void swapComponents(Grid<Vec3> &vel, int c1 = 0, int c2 = 1,
                             int c3 = 2)
{
	FOR_IJK(vel)
	{
...
// helper functions for UV grid data (stored grid coordinates as Vec3 values,
// and uv weight in entry zero)

// make uv weight accesible in python
PYTHON() Real getUvWeight(Grid<Vec3> &uv) { return uv[0][0]; }

// note - right now the UV grids have 0 values at the border after advection...
// could be fixed with an extrapolation step...
...
KERNEL() void knResetUvGrid(Grid<Vec3> &target)
{
...
{
	knResetUvGrid reset(
	    target); // note, llvm complains about anonymous declaration here... ?
}
PYTHON() void updateUvWeight(Real resetTime, int index, int numUvs,
                             Grid<Vec3> &uv)
{
	const Real t = uv.getParent()->getTime();
	Real timeOff = resetTime / (Real)numUvs;

	Real lastt = computeUvGridTime(
	    t + (Real)index * timeOff - uv.getParent()->getDt(), resetTime);
	...
	for (int i = 0; i < numUvs; ++i) {
		uvWTotal +=
		    computeUvRamp(computeUvGridTime(t + (Real)i * timeOff, resetTime));
	}
...
KERNEL(idx, reduce = +)
returns(double result = 0.0) double knGridTotalSum(const Grid<Real> &a,
                                                   FlagGrid *flags)
{
	if (flags) {
...
KERNEL(idx, reduce = +)
returns(int numEmpty = 0) int knCountFluidCells(FlagGrid &flags)
{
...
KERNEL(idx) void knGetComponent(const Grid<Vec3> &source, Grid<Real> &target,
                                int component)
{
	target[idx] = source[idx][component];
}
PYTHON() void getComponent(const Grid<Vec3> &source, Grid<Real> &target,
                           int component)
{
	knGetComponent(source, target, component);
}

KERNEL(idx) void knSetComponent(const Grid<Real> &source, Grid<Vec3> &target,
                                int component)
{
	target[idx][component] = source[idx];
}
PYTHON() void setComponent(const Grid<Real> &source, Grid<Vec3> &target,
                           int component)
{
...
void FlagGrid::InitMaxXWall(const int &boundaryWidth, Grid<Real> &phiWalls)
{
	...
	{
		phiWalls(i, j, k) =
		    std::min(mSize.x - i - 1.5 - w, (double)phiWalls(i, j, k));
	}
}
...
void FlagGrid::InitMaxYWall(const int &boundaryWidth, Grid<Real> &phiWalls)
{
	...
	{
		phiWalls(i, j, k) =
		    std::min(mSize.y - j - 1.5 - w, (double)phiWalls(i, j, k));
	}
}
...
void FlagGrid::InitMaxZWall(const int &boundaryWidth, Grid<Real> &phiWalls)
{
	...
	{
		phiWalls(i, j, k) =
		    std::min(mSize.z - k - 1.5 - w, (double)phiWalls(i, j, k));
	}
}

void FlagGrid::initDomain(const int &boundaryWidth, const string &wall,
                          const string &open, const string &inflow,
                          const string &outflow, Grid<Real> *phiWalls)
{
...
void FlagGrid::updateFromLevelset(LevelsetGrid &levelset)
{
	FOR_IDX(*this)
	{
		if (!isObstacle(idx)) {
			...
			mData[idx] |=
			    (phi <= 0) ? TypeFluid : TypeEmpty; // set resepctive flag
		}
	}
}
...
void FlagGrid::fillGrid(int type)
{
	FOR_IDX(*this)
	{
		if ((mData[idx] & TypeObstacle) == 0 &&
		    (mData[idx] & TypeInflow) == 0 && (mData[idx] & TypeOutflow) == 0 &&
		    (mData[idx] & TypeOpen) == 0)
...
// todo fix, missing:  template<> const char* Namify<nbVector>::S =
// "TestDatatype";
...
KERNEL() void knNbvecTestKernel(Grid<nbVector> &target)
{
	target(i, j, k).push_back(i + j + k);
}
...
// instantiate test datatype , not really required for simulations, mostly here
