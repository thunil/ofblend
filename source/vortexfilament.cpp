 ...
 *
 * Vortex filament
 * (warning, the vortex methods are currently experimental, and not fully
 *supported!)
 *
 ******************************************************************************/
...
inline Vec3 FilamentKernel(const Vec3 &pos, const vector<VortexRing> &rings,
                           const vector<BasicParticleData> &fp, Real reg,
                           Real cutoff, Real scale)
{
	...
	for (size_t i = 0; i < rings.size(); i++) {
		...
		for (int j = 0; j < N; j++) {
			...
			if (r0_2 > cutoff2 || r1_2 > cutoff2 || r0_2 < mindist ||
			    r1_2 < mindist)
				continue;
			...
			const Real A = str * (dot(r1, e) * r1n - dot(r0, e) * r0n) /
			               (a2 + normSquare(cp));
			u += A * cp;
		}
	}
...
KERNEL(pts) returns(vector<Vec3> u(size)) vector<Vec3> KnFilamentAdvectParts(
    vector<BasicParticleData> &nodes, vector<BasicParticleData> &fp,
    const vector<VortexRing> &rings, Real reg, Real cutoff, Real scale)
{
...
KERNEL(pts) returns(vector<Vec3> u(size)) vector<Vec3> KnFilamentAdvectMesh(
    vector<Node> &nodes, const vector<VortexRing> &rings,
    const vector<BasicParticleData> &fp, Real reg, Real cutoff, Real scale)
{
...
void VortexFilamentSystem::advectSelf(Real scale, Real regularization,
                                      int integrationMode)
{
	KnFilamentAdvectParts kernel(mData, mData, mSegments, regularization, 1e10,
	                             scale * getParent()->getDt());
	integratePointSet(kernel, integrationMode);
}

void VortexFilamentSystem::advectMesh(Mesh &mesh, Real scale,
                                      Real regularization, int integrationMode)
{
	KnFilamentAdvectMesh kernel(mesh.getNodeData(), mSegments, mData,
	                            regularization, 1e10,
	                            scale * getParent()->getDt());
	integratePointSet(kernel, integrationMode);
}

void VortexFilamentSystem::advectParticles(BasicParticleSystem &sys, Real scale,
                                           Real regularization,
                                           int integrationMode)
{
	KnFilamentAdvectParts kernel(sys.getData(), mData, mSegments,
	                             regularization, 1e10,
	                             scale * getParent()->getDt());
	integratePointSet(kernel, integrationMode);
}
...
void VortexFilamentSystem::remesh(Real maxLen, Real minLen)
{
	...
	for (int i = 0; i < segSize(); i++) {
		...
		for (;;) {
			...
			for (int j = 0; j < oldLen; j++) {
				...
				if (l2 > maxLen2) {
					...
					const Vec3 mp =
					    hermiteSpline(p0, p1, crTangent(p_1, p0, p1),
					                  crTangent(p0, p1, p2), 0.5);
		...
		for (;;) {
			...
			int newLen = oldLen;
			for (int j = 0; j < N; j++) {
				if (mData[r.idx0(j)].flag & PDELETE ||
			...
			for (int j = 0, copyFrom = 0; j < newLen; j++, copyFrom++) {
				while (deleted[copyFrom])
					copyFrom++;
				if (j != copyFrom) r.indices[j] = r.indices[copyFrom];
			}
			r.indices.resize(newLen);
		}
	}
...
Real evaluateRefU(int N, Real L, Real circ, Real reg)
{
	...
	for (int i = 0; i < N; i++) {
		pos[i].pos = Vec3(r * cos(2.0 * M_PI * (Real)i / N),
		                  r * sin(2.0 * M_PI * (Real)i / N), 0);
		pos[i].flag = 0;
...
bool powerMethod(const vector<Vec3> &gamma, Real l, Real r, Vec3 &lT)
{
	...
	for (int i = 0; i < maxIter; i++) {
		...
		// if ((i%1) == 0) cout << "iteration " << i << " residual: " <<
		// norm(lT-lastLT) << endl;
		if (norm(lT - lastLT) < epsilon) return true;
	}
	return false;
}
...
bool darboux(const vector<Vec3> &from, vector<Vec3> &to, Real l, Real r)
{
	...
	Vec3 lT(0, 0, l);
	if (!powerMethod(from, l, r, lT)) return false;
...
void VortexFilamentSystem::doublyDiscreteUpdate(Real reg)
{
	...
	for (int rc = 0; rc < segSize(); rc++) {
		...
		vector<Vec3> gamma(N);
		for (int i = 0; i < N; i++)
			gamma[i] = mData[r.indices[i]].pos;

		// N=1000; L=2.0*M_PI; reg=0.1; r.circulation=1;

		// compute reference parameters
		const Real U =
		    0.5 * r.circulation / L * (log(4.0 * L / (M_PI * reg)) - 1.0);
...
void VortexFilamentSystem::addLine(const Vec3 &p0, const Vec3 &p1,
                                   Real circulation)
{
...
void VortexFilamentSystem::addRing(const Vec3 &position, Real circulation,
                                   Real radius, Vec3 normal, int number)
{
	normalize(normal);
	Vec3 worldup(0, 1, 0);
	if (norm(normal - worldup) < 1e-5) worldup = Vec3(1, 0, 0);

	Vec3 u = cross(normal, worldup);
	normalize(u);
