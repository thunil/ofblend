...
{
	if (mCorners.size() != 3 * mTris.size()) rebuildCorners();
	if (m1RingLookup.size() != mNodes.size()) rebuildLookup();
}
...
KERNEL(pts) returns(vector<Vec3> u(size)) vector<Vec3> KnAdvectMeshInGrid(
    vector<Node> &nodes, const FlagGrid &flags, const MACGrid &vel,
    const Real dt)
{
...
{
	// delete triangles by overwriting them with elements from the end of the
	// array.
	if (tri != (int)mTris.size() - 1) {
		...
		// and DEFINITELY don't transfer .opposite data to other, untainted
		// triangles.
		...
		// 1) update c.node, c.opposite (c.next and c.prev should be fine as
		// they are)
		for (int c = 0; c < 3; c++) {
...
void Mesh::removeNodes(const vector<int> &deletedNodes)
{
	...
	// the size of mNodes will be the current size - the size of the deleted
	// array.
	// We are going to move the elements at the end of the array
	// (everything with an index >= newsize)
	// to the deleted spots.
	// We have to map all references to the last few nodes to their new
	// locations.
	...
	for (di = 0; di < (int)deletedNodes.size(); di++) {
		if (deletedNodes[di] >= newsize)
			new_index[deletedNodes[di] - newsize] =
			    -1; // tag this node as invalid
	}
	for (di = 0, ni = 0; ni < (int)new_index.size(); ni++, di++) {
		...
		if (ni >= (int)new_index.size()) break;
	...
	for (int i = 0; i < (int)new_index.size(); i++) {
		if (new_index[i] != -1) mNodes[new_index[i]] = mNodes[newsize + i];
	}
	...
	for (int i = 0; i < (int)new_index.size(); i++) {
		if (new_index[i] != -1) {
			m1RingLookup[new_index[i]].nodes.swap(
			    m1RingLookup[newsize + i].nodes);
			m1RingLookup[new_index[i]].tris.swap(
			    m1RingLookup[newsize + i].tris);
		}
	}
...
void Mesh::addCorner(Corner a) { mCorners.push_back(a); }
...
{
	mNodes.push_back(a);
	if (m1RingLookup.size() < mNodes.size()) m1RingLookup.resize(mNodes.size());
...
void Mesh::sanityCheck(bool strict, vector<int> *deletedNodes,
                       map<int, bool> *taintedTris)
{
	...
	if ((int)m1RingLookup.size() != nodes) errMsg("1Ring size wrong");
	for (size_t t = 0; t < mTris.size(); t++) {
		...
		for (int c = 0; c < 3; c++) {
			...
			if (node < 0 || node >= nodes || next < 0 || next >= nodes ||
			    prev < 0 || prev >= nodes)
				errMsg("invalid node entry");
			if (mCorners[corner].node != node || mCorners[corner].tri != (int)t)
				errMsg("invalid basic corner entry");
			if (rnext < 0 || rnext >= corners || rprev < 0 ||
			    rprev >= corners || ro >= corners)
				errMsg("invalid corner links");
			if (mCorners[rnext].node != next || mCorners[rprev].node != prev)
				errMsg("invalid corner next/prev");
			if (strict && ro < 0) errMsg("opposite missing");
			if (mCorners[ro].opposite != corner) errMsg("invalid opposite ref");
			set<int> &rnodes = m1RingLookup[node].nodes;
			set<int> &rtris = m1RingLookup[node].tris;
			if (rnodes.find(next) == rnodes.end() ||
			    rnodes.find(prev) == rnodes.end()) {
				debMsg("Tri " << t << " " << node << " " << next << " " << prev,
				       1);
				for (set<int>::iterator it = rnodes.begin(); it != rnodes.end();
				     ++it)
					debMsg(*it, 1);
	...
	for (int n = 0; n < nodes; n++) {
		...
		if (docheck) {
			...
			for (set<int>::iterator it = st.begin(); it != st.end(); ++it) {
				...
				if (taintedTris &&
				    taintedTris->find(*it) != taintedTris->end()) {
					cout << *it << endl;
					errMsg("tainted tri still is use");
				}
			}
			if (sn.size() != sn2.size()) errMsg("invalid nodes in 1ring");
			for (set<int>::iterator it = sn.begin(), it2 = sn2.begin();
			     it != sn.end(); ++it, ++it2) {
				if (*it != *it2) {
					cout << "Node " << n << ": " << *it << " vs " << *it2
					     << endl;
...
// void SDFKernel(const int* partStart, const int* partLen, CVec3Ptr pos,
// CVec3Ptr normal, Real* sdf, Vec3i gridRes, int intRadius, Real safeRadius2,
// Real cutoff2, Real isigma2);
//! helper for rasterization
static void SDFKernel(Grid<int> &partStart, Grid<int> &partLen, CVec3Ptr pos,
                      CVec3Ptr normal, LevelsetGrid &sdf, Vec3i gridRes,
                      int intRadius, Real safeRadius2, Real cutoff2,
                      Real isigma2)
{
	for (int cnt_x(0); cnt_x < gridRes[0]; ++cnt_x) {
		for (int cnt_y(0); cnt_y < gridRes[1]; ++cnt_y) {
			for (int cnt_z(0); cnt_z < gridRes[2]; ++cnt_z) {
				...
				if (cell.x >= gridRes.x || cell.y >= gridRes.y ||
				    cell.z >= gridRes.z)
					return;
				...
				Vec3i minBlock = Vec3i(max(cell.x - intRadius, 0),
				                       max(cell.y - intRadius, 0),
				                       max(cell.z - intRadius, 0));
				...
				for (int i = minBlock.x; i <= maxBlock.x; i++)
					for (int j = minBlock.y; j <= maxBlock.y; j++)
						for (int k = minBlock.z; k <= maxBlock.z; k++) {
							...
							// find source cell, and divide it into thread
							// blocks
				...
				// writeback
				if (sum > 0.0f) {
					// sdf[cell.x + gridRes.x * (cell.y + gridRes.y * cell.z)] =
					// dist / sum;
...
void ApplyMeshToGrid(Grid<T> *grid, Grid<Real> sdf, T value,
...
void Mesh::applyMeshToGrid(GridBase *grid, FlagGrid *respectFlags, Real cutoff)
{
	...
	if (grid->getType() & GridBase::TypeInt)
		ApplyMeshToGrid<int>((Grid<int> *)grid, mesh_sdf,
		                     _args.get<int>("value"), respectFlags);
	else if (grid->getType() & GridBase::TypeReal)
		ApplyMeshToGrid<Real>((Grid<Real> *)grid, mesh_sdf,
		                      _args.get<Real>("value"), respectFlags);
	else if (grid->getType() & GridBase::TypeVec3)
		ApplyMeshToGrid<Vec3>((Grid<Vec3> *)grid, mesh_sdf,
		                      _args.get<Vec3>("value"), respectFlags);
	else
...
void meshSDF(Mesh &mesh, LevelsetGrid &levelset, Real sigma, Real cutoff)
{
	...
	for (int i = 0; i < mesh.numTris(); i++) {
		...
		if (bigEdges > 0) {
			...
			center.insert(center.end(), samplePoints.begin(),
			              samplePoints.end());
		}
	}
	...
	SDFKernel(srcCellStart, srcPerCell, reorderPos.data(), reorderNormal.data(),
	...
	FOR_IJK(levelset)
	{
		if (levelset(i, j, k) >= cutoff - 1.0f) outside.push(Vec3i(i, j, k));
	}
	while (!outside.empty()) {
		Vec3i c = outside.top();
		outside.pop();
