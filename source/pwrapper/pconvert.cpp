...
template <> PyObject *toPy<int>(const int &v) { return PyLong_FromLong(v); }
...
template <> PyObject *toPy<bool>(const bool &v) { return PyBool_FromLong(v); }
template <> PyObject *toPy<Vec3i>(const Vec3i &v)
{
	float x = (float)v.x, y = (float)v.y, z = (float)v.z;
	return PyObject_CallFunction((PyObject *)&PbVec3Type, (char *)"fff", x, y,
	                             z);
}
...
{
	float x = (float)v.x, y = (float)v.y, z = (float)v.z;
	return PyObject_CallFunction((PyObject *)&PbVec4Type, (char *)"ffff", x, y,
	                             z);
}
...
template <> PyObject *fromPy<PyObject *>(PyObject *obj) { return obj; }
template <> int fromPy<int>(PyObject *obj)
{
	...
	if (PyFloat_Check(obj)) {
		...
		if (fabs(a - floor(a + 0.5)) > 1e-5) errMsg("argument is not an int");
		return (int)(a + 0.5);
	}
...
template <> bool isPy<PyObject *>(PyObject *obj) { return true; }
...
template <> bool isPy<bool>(PyObject *obj) { return PyBool_Check(obj); }
...
template <> bool isPy<PbType>(PyObject *obj) { return PyType_Check(obj); }
...
void PbArgs::check()
{
	...
	for (map<string, DataElement>::iterator it = mData.begin();
	     it != mData.end(); it++) {
		if (!it->second.visited) errMsg("Argument '" + it->first + "' unknown");
	}
...
FluidSolver *PbArgs::obtainParent()
{
	...
	for (map<string, DataElement>::iterator it = mData.begin();
		...
		if (obj) {
			if (solver == NULL) solver = obj->getParent();
		}
	}
	for (vector<DataElement>::iterator it = mLinData.begin();
	     it != mLinData.end(); it++) {
...
void PbArgs::visit(int number, const string &key)
{
	...
	if (lu != mData.end()) lu->second.visited = true;
}
...
PyObject *PbArgs::getItem(const std::string &key, bool strict, ArgLocker *lk)
{
	map<string, DataElement>::iterator lu = mData.find(key);
	if (lu == mData.end()) {
		if (strict) errMsg("Argument '" + key + "' is not defined.");
		return NULL;
	}
