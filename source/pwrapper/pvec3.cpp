...
static int PbVec3Init(PbVec3 *self, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTuple(args, "|fff", &x1, &x2, &x3)) return -1;
...
static PyMemberDef PbVec3Members[] = {
	...
	{(char *)"y", T_FLOAT, offsetof(PbVec3, data) + sizeof(float), 0,
	 (char *)"Y" },
	{(char *)"z", T_FLOAT, offsetof(PbVec3, data) + sizeof(float) * 2, 0,
	 (char *)"Z" },
	{ NULL } // Sentinel
};

static PyMethodDef PbVec3Methods[] = {
	//{"name", (PyCFunction)Noddy_name, METH_NOARGS, "Return the name, combining
...
#define OPHEADER                                                               \
	if (!PyObject_TypeCheck(a, &PbVec3Type) &&                                 \
	    !PyObject_TypeCheck(b, &PbVec3Type)) {                                 \
		Py_INCREF(Py_NotImplemented);                                          \
		return Py_NotImplemented;                                              \
	}                                                                          \
	CONVERTVEC(a)                                                              \
...
static PyMemberDef PbVec4Members[] = {
	...
	{(char *)"y", T_FLOAT, offsetof(PbVec4, data) + sizeof(float) * 1, 0,
	 (char *)"Y" },
	{(char *)"z", T_FLOAT, offsetof(PbVec4, data) + sizeof(float) * 2, 0,
	 (char *)"Z" },
	{(char *)"t", T_FLOAT, offsetof(PbVec4, data) + sizeof(float) * 3, 0,
	 (char *)"T" },
	{ NULL } // Sentinel
};
...
static int PbVec4Init(PbVec4 *self, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTuple(args, "|ffff", &x1, &x2, &x3, &x4)) return -1;
