...
struct Method
{
	Method(const string &n, const string &d, GenericFunction f)
	    : name(n), doc(d), func(f)
	{
	}
	string name, doc;
	...
	PyMethodDef def()
	{
		PyMethodDef def = { &name[0], (PyCFunction)func,
			                METH_VARARGS | METH_KEYWORDS, &doc[0] };
		return def;
	}
};
struct GetSet
...
class WrapperRegistry
{
  public:
	...
	void addOperator(const std::string &classname,
	                 const std::string &methodname, OperatorFunction method);
	void addConstructor(const std::string &classname, Constructor method);
	void addGetSet(const std::string &classname, const std::string &property,
	               Getter getfunc, Setter setfunc);
	...
	PyObject *createPyObject(const std::string &classname,
	                         const std::string &name, Manta::PbArgs &args,
...
ClassData *WrapperRegistry::getOrConstructClass(const string &classname)
{
	...
	if (tplIdx != string::npos) {
		...
		data->cTemplate =
		    classname.substr(tplIdx + 1, classname.find('>') - tplIdx - 1);
	}
	data->baseclass = NULL;
...
void WrapperRegistry::addClass(const string &pyName, const string &internalName,
                               const string &baseclass)
{
	...
	if (data->pyName.empty()) data->pyName = pythonName;
	mClasses[pythonName] = data;
	if (!baseclass.empty()) data->baseclassName = baseclass;
}
...
void WrapperRegistry::addGetSet(const string &classname, const string &property,
                                Getter getfunc, Setter setfunc)
{
...
void WrapperRegistry::addMethod(const string &classname,
                                const string &methodname, GenericFunction func)
{
	string aclass = classname;
	if (aclass.empty()) aclass = "__modclass__";
...
void WrapperRegistry::addParentMethods(ClassData *cur, ClassData *base)
{
	...
	for (vector<Method>::iterator it = base->methods.begin();
	     it != base->methods.end(); ++it)
		addMethod(cur->cName, it->name, it->func);

	for (map<string, GetSet>::iterator it = base->getset.begin();
	     it != base->getset.end(); ++it)
	...
	for (int i = 0; i < (int)mClassList.size(); i++) {
		...
		if (!bname.empty()) {
			...
			if (!mClassList[i]->baseclass)
				errMsg("Registering class '" + mClassList[i]->cName +
				       "' : Base class '" + bname + "' not found");
		}
	}
...
{
	for (int i = 0; i < (int)mClassList.size(); i++) {
		mClassList[i]->getset["_class"] =
		    GetSet("_class", "C class name", (Getter)cbGetClass, 0);
		mClassList[i]->getset["_cname"] =
		    GetSet("_cname", "Full C name", (Getter)cbGetCName, 0);
		mClassList[i]->getset["_T"] =
		    GetSet("_T", "C template argument", (Getter)cbGetTemplate, 0);
	}
}
...
void WrapperRegistry::registerOperators(ClassData *cls)
{
	...
	for (map<string, OperatorFunction>::iterator it = cls->ops.begin();
	     it != cls->ops.end(); it++) {
...
{
	vector<string> add;
	for (vector<ClassData *>::iterator it = mClassList.begin();
	     it != mClassList.end(); ++it) {
...
{
	for (map<string, ClassData *>::iterator it = mClasses.begin();
	     it != mClasses.end(); ++it) {
		if (it->first == name || it->second->cName == name) return it->second;
	}
	return NULL;
}
...
bool WrapperRegistry::canConvert(ClassData *from, ClassData *to)
{
	...
	if (from->baseclass) return canConvert(from->baseclass, to);
	return false;
}
                                          ...
                                          Manta::PbClass *parent)
{
	...
	if (!classdef) errMsg("Class " + classname + " doesn't exist.");
	...
	if (self->classdef->constructor(obj, args.linArgs(), nkw) < 0)
		errMsg(
		    "error raised in constructor"); // assume condition is already set
...
PyObject *WrapperRegistry::initModule()
{
	...
	for (int i = 0; i < (int)mClassList.size(); i++) {
		...
		cls->genGetSet.clear();
		for (vector<Method>::iterator i2 = cls->methods.begin();
		     i2 != cls->methods.end(); ++i2)
			cls->genMethods.push_back(i2->def());
		for (map<string, GetSet>::iterator i2 = cls->getset.begin();
		     i2 != cls->getset.end(); ++i2)
...
// prepare module info
	...
	static PyModuleDef MainModule = {
		PyModuleDef_HEAD_INIT,             gDefaultModuleName.c_str(),
		"Bridge module to the C++ solver", -1,
		NULL,                              NULL,
		NULL,                              NULL,
		NULL
	};
	...
	for (vector<ClassData *>::iterator it = mClassList.begin();
		...
		PyTypeObject t = {
			PyVarObject_HEAD_INIT(NULL, 0)(char *)
			data.pyName.c_str(),                      // tp_name
		...
		if (PyType_Ready(&data.typeInfo) < 0) continue;

		for (map<string, ClassData *>::iterator i2 = mClasses.begin();
		     i2 != mClasses.end(); ++i2) {
			if (*it != i2->second) continue;
			// register all aliases
			Py_INCREF(castPy(&data.typeInfo));
			PyModule_AddObject(module, (char *)i2->first.c_str(),
			                   (PyObject *)&data.typeInfo);
		}
	}

	// externals
	for (vector<InitFunc>::iterator it = mExtInitializers.begin();
	     it != mExtInitializers.end(); ++it) {
		(*it)(module);
	}
...
bool canConvert(PyObject *obj, const string &classname)
{
	...
	if (!dest) errMsg("Classname '" + classname + "' is not registered.");
...
PyObject *copyObject(Manta::PbClass *cls, const string &classname)
{
	...
	// allocate new object
	PbObject *obj =
	    (PbObject *)classdef->typeInfo.tp_alloc(&(classdef->typeInfo), 0);
...
Manta::PbClass *createPy(const std::string &classname, const std::string &name,
                         Manta::PbArgs &args, Manta::PbClass *parent)
{
	PyObject *obj = WrapperRegistry::instance().createPyObject(classname, name,
	                                                           args, parent);
	return ((PbObject *)obj)->instance;
}
...
Register::Register(const string &className, const string &funcName,
                   GenericFunction func)
{
...
Register::Register(const string &className, const string &property,
                   Getter getter, Setter setter)
{
	WrapperRegistry::instance().addGetSet(className, property, getter, setter);
}
Register::Register(const string &className, const string &pyName,
                   const string &baseClass)
{
