...
void pbSetError(const string &fn, const string &ex)
{
	...
	if (!ex.empty()) PyErr_SetString(PyExc_RuntimeError, ex.c_str());
}
...
PbClass::PbClass(const PbClass &a)
    : mMutex(), mParent(a.mParent), mPyObject(0), mName("_unnamed"),
      mHidden(false)
{
}
...
PbClass::~PbClass()
{
	for (vector<PbClass *>::iterator it = mInstances.begin();
...
void PbClass::lock() { mMutex.lock(); }
void PbClass::unlock() { mMutex.unlock(); }
bool PbClass::tryLock() { return mMutex.tryLock(); }
...
int PbClass::getNumInstances() { return mInstances.size(); }
...
PbClass *PbClass::createPyObject(const string &classname, const string &name,
                                 PbArgs &args, PbClass *parent)
{
...
{
	if (getParent() == NULL) {
		errMsg("New class " + mName +
		       ": no parent given -- specify using parent=xxx !");
	}
}
...
void PbClass::renameObjects()
{
	...
	PyObject *loc_mod =
	    PyMapping_GetItemString(sys_mod_dict, (char *)"__main__");
	if (!loc_mod) return;
		...
		if (obj->getName().empty()) {
			...
			while (PyDict_Next(locdict, &lpos, &lkey, &lvalue)) {
				if (lvalue == obj->mPyObject) {
					...
					obj->setName(varName);
					// cout << "assigning variable name '" << varName << "' to
					// unnamed instance" << endl;
					break;
				}
			}
		}
