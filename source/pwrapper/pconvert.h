...
{
	if (PbClass::isNullRef(obj)) return false;
	PbClass *pbo = Pb::objFromPy(obj);
	const std::string &type = Namify<typename remove_pointers<T>::type>::S;
...
{
	throw Error("Unknown type conversion. Did you pass a PbClass by value? "
	            "(Don't: always pass grids/particlesystems etc. by reference "
	            "or using a pointer.)");
}
...
class PbArgs
{
  public:
	...
	{
		if (mData.find(key) != mData.end()) mData.erase(mData.find(key));
	}
	...
	template <class T>
	inline T get(const std::string &key, int number = -1, ArgLocker * lk = NULL)
	{
		visit(number, key);
	...
	template <class T>
	inline T getOpt(const std::string &key, int number, T defarg,
	                ArgLocker *lk = NULL)
	{
		visit(number, key);
		PyObject *o = getItem(key, false, lk);
	...
	template <class T>
	inline T *getPtrOpt(const std::string &key, int number, T *defarg,
	                    ArgLocker *lk = NULL)
	{
		visit(number, key);
		PyObject *o = getItem(key, false, lk);
	...
	template <class T>
	inline T *getPtr(const std::string &key, int number = -1,
	                 ArgLocker * lk = NULL)
	{
		visit(number, key);
		PyObject *o = getItem(key, false, lk);
  ...
  protected:
	PyObject *getItem(const std::string &key, bool strict,
	                  ArgLocker *lk = NULL);
