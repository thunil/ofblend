...
template <class T> T *FluidSolver::GridStorage<T>::get(Vec3i size)
{
	if ((int)grids.size() <= used) {
		debMsg("FluidSolver::GridStorage::get Allocating new "
...
template <class T> void FluidSolver::GridStorage<T>::release(T *ptr)
{
	...
	used--;
	if (used < 0) errMsg("temp grid inconsistency");
	grids[used] = ptr;
}
...
{
	return mGrids4dInt.get(
	    Vec3i(mGridSize[0] * mGridSize[1], mGridSize[2], mFourthDim));
}
template <> Real *FluidSolver::getGrid4dPointer<Real>()
{
	return mGrids4dReal.get(
	    Vec3i(mGridSize[0] * mGridSize[1], mGridSize[2], mFourthDim));
}
template <> Vec3 *FluidSolver::getGrid4dPointer<Vec3>()
{
	return mGrids4dVec.get(
	    Vec3i(mGridSize[0] * mGridSize[1], mGridSize[2], mFourthDim));
}
template <> Vec4 *FluidSolver::getGrid4dPointer<Vec4>()
{
	return mGrids4dVec4.get(
	    Vec3i(mGridSize[0] * mGridSize[1], mGridSize[2], mFourthDim));
}
...
FluidSolver::FluidSolver(Vec3i gridsize, int dim, int fourthDim)
    : PbClass(this), mDt(1.0), mTimeTotal(0.), mFrame(0), mCflCond(1000),
      mDtMin(1.), mDtMax(1.), mFrameLength(1.), mGridSize(gridsize), mDim(dim),
      mTimePerFrame(0.), mLockDt(false), mFourthDim(fourthDim)
{
	assertMsg(dim == 2 || dim == 3, "Can only create 2D and 3D solvers");
	assertMsg(dim != 2 || gridsize.z == 1,
	          "Trying to create 2D solver with size.z != 1");
}
...
PbClass *FluidSolver::create(PbType t, PbTypeVec T, const string &name)
{
	...
	if (t.str() == "")
		errMsg("Need to specify object type. Use e.g. Solver.create(FlagGrid, "
		       "...) or Solver.create(type=FlagGrid, ...)");

	PbClass *ret =
	    PbClass::createPyObject(t.str() + T.str(), name, _args, this);
#else
	PbClass *ret = NULL;
#endif
	return ret;
...
//! helper to unify printing from python scripts and printing internal messages
//(optionally pass debug level to control amount of output)
...
void FluidSolver::printMemInfo()
{
	...
	msg << "Allocated grids: int " << mGridsInt.used << "/"
	    << mGridsInt.grids.size() << ", ";
	msg << "                 real " << mGridsReal.used << "/"
	    << mGridsReal.grids.size() << ", ";
	msg << "                 vec3 " << mGridsVec.used << "/"
	    << mGridsVec.grids.size() << ". ";
	msg << "                 vec4 " << mGridsVec4.used << "/"
	    << mGridsVec4.grids.size() << ". ";
	if (has4D()) {
		msg << "Allocated 4d grids: int " << mGrids4dInt.used << "/"
		    << mGrids4dInt.grids.size() << ", ";
		msg << "                    real " << mGrids4dReal.used << "/"
		    << mGrids4dReal.grids.size() << ", ";
		msg << "                    vec3 " << mGrids4dVec.used << "/"
		    << mGrids4dVec.grids.size() << ". ";
		msg << "                    vec4 " << mGrids4dVec4.used << "/"
		    << mGrids4dVec4.grids.size() << ". ";
	}
	printf("%s\n", msg.str().c_str());
}
...
PYTHON() void setDebugLevel(int level = 1) { gDebugLevel = level; }

void FluidSolver::adaptTimestep(Real maxVel)
{
	if (!mLockDt) {
		// calculate current timestep from maxvel, clamp range
		mDt = std::max(std::min((Real)(mCflCond / (maxVel + 1e-05)), mDtMax),
		               mDtMin);
		if ((mTimePerFrame + mDt * 1.05) > mFrameLength) {
			...
			// avoid tiny timesteps and strongly varying ones, do 2 medium size
			// ones if necessary...
			mDt = (mFrameLength - mTimePerFrame + 1e-04) * 0.5;
			mLockDt = true;
		}
	}
	debMsg("Frame " << mFrame << " current max vel: " << maxVel
	                << " , dt: " << mDt << ", " << mTimePerFrame << "/"
	                << mFrameLength << " lock:" << mLockDt,
	       2);

	// sanity check
	assertMsg((mDt > (mDtMin / 2.)),
	          "Invalid dt encountered! Shouldnt happen...");
}
