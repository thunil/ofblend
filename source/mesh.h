...
template <class T> struct SimpleTriChannel : public TriChannel
{
	...
	void remove(int tri)
	{
		if (tri != (int)data.size() - 1) data[tri] = *data.rbegin();
		data.pop_back();
	}
	virtual int size() { return data.size(); }
...
PYTHON() class Mesh : public PbClass
{
  public:
	...
	PYTHON() void advectInGrid(FlagGrid &flaggrid, MACGrid &vel,
	                           int integrationMode);
	PYTHON() void scale(Vec3 s);
	PYTHON() void offset(Vec3 o);

	PYTHON() void computeLevelset(LevelsetGrid &levelset, Real sigma,
	                              Real cutoff = -1.);
	//! map mesh to grid with sdf
	PYTHON() void applyMeshToGrid(GridBase *grid, FlagGrid *respectFlags = 0,
	                              Real cutoff = -1.);
	...
	{
		return (mNodes[mTris[t].c[0]].flags & NfFixed) ||
		       (mNodes[mTris[t].c[1]].flags & NfFixed) ||
	...
	inline Real getFaceArea(int t)
	{
		Vec3 c0 = mNodes[mTris[t].c[0]].pos;
		return 0.5 * norm(cross(mNodes[mTris[t].c[1]].pos - c0,
		                        mNodes[mTris[t].c[2]].pos - c0));
	}
	inline Vec3 getFaceNormal(int t)
	{
		Vec3 c0 = mNodes[mTris[t].c[0]].pos;
		return getNormalized(cross(mNodes[mTris[t].c[1]].pos - c0,
		                           mNodes[mTris[t].c[2]].pos - c0));
	}
	inline Vec3 getFaceCenter(int t)
	{
		return (mNodes[mTris[t].c[0]].pos + mNodes[mTris[t].c[1]].pos +
		        mNodes[mTris[t].c[2]].pos) /
		       3.0;
	}
	...
	void sanityCheck(bool strict = true, std::vector<int> *deletedNodes = 0,
...
template <class T>
void SimpleNodeChannel<T>::renumber(const std::vector<int> &newIndex,
                                    int newsize)
{
	for (size_t i = 0; i < newIndex.size(); i++) {
		if (newIndex[i] != -1) data[newIndex[i]] = data[newsize + i];
	}
	data.resize(newsize);
}
