...
PYTHON() class ParticleBase : public PbClass
{
  public:
	...
	enum ParticleStatus {
		...
		PDELETE =
		    (1
		     << 10), // mark as deleted, will be deleted in next compress() step
	...
	//! copy all the particle data thats registered with the other particle
	//system to this one
	...
	// slow virtual function to query size, do not use in kernels! use size()
	// instead
	...
	//! add a position as potential candidate for new particle (todo, make
	//usable from parallel threads)
	inline void addBuffered(const Vec3 &pos);

	// particle data functions

	//! create a particle data object
	PYTHON() PbClass *create(PbType type, PbTypeVec T = PbTypeVec(),
  ...
  protected:
	...
	//! allow automatic compression / resize? disallowed for, eg, flip particle
	//systems
	bool mAllowCompress;

	//! store particle data , each pointer has its own storage vector of a
	//certain type (int, real, vec3)
	std::vector<ParticleDataBase *> mPartData;
	//! lists of different types, for fast operations w/o virtual function calls
	//(all calls necessary per particle)
	...
	//! indicate that pdata of this particle system is copied, and needs to be
	//freed
	bool mFreePdata;
};
...
PYTHON() template <class S> class ParticleSystem : public ParticleBase
{
  public:
	...
	// note , python binding disabled for now! cannot yet deal with long-long
	// types
	...
	// query status
	inline int getStatus(IndexInt idx)
	{
		DEBUG_ONLY(checkPartIndex(idx));
		return mData[idx].flag;
	}
	inline bool isActive(IndexInt idx)
	{
		DEBUG_ONLY(checkPartIndex(idx));
	...
	PYTHON() void setPos(IndexInt idx, const Vec3 &pos)
	{
		DEBUG_ONLY(checkPartIndex(idx));
		mData[idx].pos = pos;
	}
	PYTHON() Vec3 getPos(IndexInt idx)
	{
		DEBUG_ONLY(checkPartIndex(idx));
	...
	//! transform coordinate system from one grid size to another (usually upon
	//load)
	...
	//! insert buffered positions as new particles, update additional particle
	//data
	...
	PYTHON() void advectInGrid(FlagGrid &flags, MACGrid &vel,
	                           int integrationMode,
	                           bool deleteInObstacle = true,
...
PYTHON() class BasicParticleSystem : public ParticleSystem<BasicParticleData>
{
  public:
	...
	// dangerous, get low level access - avoid usage, only used in vortex
	// filament advection for now
	std::vector<BasicParticleData> &getData() { return mData; }

	PYTHON() void printParts(IndexInt start = -1, IndexInt stop = -1,
...
//  used for grid based neighborhood searches on generic particle systems
//  (stores
...
struct ParticleIndexData
{
  public:
	...
	IndexInt
	    sourceIndex; // index of this particle in the original particle system
	// note - the following two are needed for template instantiation, but not
	// used
...
{
  public:
	PYTHON() ParticleIndexSystem(FluidSolver *parent)
	    : ParticleSystem<ParticleIndexData>(parent){};
...
{
  public:
	PYTHON() ConnectedParticleSystem(FluidSolver *parent)
	    : ParticleSystem<DATA>(parent)
	{
	}
...
PYTHON() class ParticleDataBase : public PbClass
{
  public:
	...
	// interface functions, using assert instead of pure virtual for python
	// compatibility
	virtual IndexInt getSizeSlow() const
	{
		assertMsg(false, "Dont use, override...");
...
PYTHON() template <class T> class ParticleDataImpl : public ParticleDataBase
{
  public:
	...
	// set all values to 0, note - different from particleSystem::clear! doesnt
	// modify size of array (has to stay in sync with parent system)
	PYTHON() void clear();
	...
	PYTHON() void printPdata(IndexInt start = -1, IndexInt stop = -1,
	                         bool printIndex = false);
  ...
  protected:
	...
	//! optionally , we might have an associated grid from which to grab new
	//data
	Grid<T> *mpGridSource;
...
void ParticleBase::addBuffered(const Vec3 &pos) { mNewBuffer.push_back(pos); }
...
std::vector<Vec3> GridAdvectKernel(std::vector<S> &p, const MACGrid &vel,
                                   ...
                                   bool deleteInObstacle, bool stopInObstacle)
{
	...
	if (deleteInObstacle || stopInObstacle) {
		if (!flags.isInBounds(p[idx].pos, 1) || flags.isObstacle(p[idx].pos)) {
			if (stopInObstacle) u[idx] = 0.;
			// for simple tracer particles, its convenient to delete particles
			// right away
			// for other sim types, eg flip, we can try to fix positions later
			// on
			if (deleteInObstacle) p[idx].flag |= ParticleBase::PDELETE;
			return;
		}
	}
...
static inline Vec3 bisectBacktracePos(const FlagGrid &flags, const Vec3 &oldp,
                                      const Vec3 &newp)
{
	Real s = 0.;
	for (int i = 1; i < 5; ++i) {
                      ...
                      bool stopInObstacle = true)
{
	...
	if (!flags.isInBounds(p[idx].pos, 0)) {
		p[idx].pos =
		    clamp(p[idx].pos, Vec3(0.), toVec3(flags.getSize()) - Vec3(1.));
	}
...
// advection plugin
template <class S>
void ParticleSystem<S>::advectInGrid(FlagGrid &flags, MACGrid &vel,
                                     int integrationMode, bool deleteInObstacle,
                                     bool stopInObstacle)
{
	...
	// update positions
	GridAdvectKernel<S> kernel(mData, vel, flags, getParent()->getDt(),
	                           deleteInObstacle, stopInObstacle);
...
void KnProjectParticles(ParticleSystem<S> &part, Grid<Vec3> &gradient)
{
	...
	if (part.isActive(idx)) {
		...
		part[idx].pos = clamp(p, Vec3(1, 1, 1) + jitter,
		                      toVec3(gradient.getSize() - 1) - jitter);
	}
}
...
template <class S> void ParticleSystem<S>::compress()
{
	...
	if (nextRead < (IndexInt)mData.size())
		debMsg("Deleted " << ((IndexInt)mData.size() - nextRead)
		                  << " particles",
		       1); // debug info
...
template <class S> void ParticleSystem<S>::insertBufferedParticles()
{
	...
	if (mNewBuffer.size() > 0)
		debMsg("Added & initialized " << (IndexInt)mNewBuffer.size()
		                              << " particles",
		       1); // debug info
	mNewBuffer.clear();
}
...
void ConnectedParticleSystem<DATA, CON>::compress()
{
	...
	ParticleSystem<DATA>::mDeleteChunk =
	    ParticleSystem<DATA>::size() / DELETE_PART;
...
{
	ConnectedParticleSystem<DATA, CON> *nm =
	    new ConnectedParticleSystem<DATA, CON>(this->getParent());
	if (this->mAllowCompress) compress();
...
template <class S> std::string ParticleSystem<S>::infoString() const
{
	...
	s << "]";
	// for(IndexInt i=0; i<(IndexInt)mPartData.size(); ++i) { sstr << i<<":" <<
	// mPartData[i]->size() <<" "; }
	return s.str();
}
...
inline void ParticleDataBase::checkPartIndex(IndexInt idx) const
{
	...
	if (mpParticleSys && mpParticleSys->getSizeSlow() != mySize) {
		errMsg("ParticleData "
		       << " size " << mySize << " does not match parent! ("
		       << mpParticleSys->getSizeSlow() << ") ");
	}
}
