...
{
	// clone additional data , and make sure the copied particle system deletes
	// it
	nm->mFreePdata = true;
	for (IndexInt i = 0; i < (IndexInt)mPartData.size(); ++i) {
...
PbClass *ParticleBase::create(PbType t, PbTypeVec T, const string &name)
{
	...
	// debMsg( "Pdata creating '"<< t.str <<" with size "<< this->getSizeSlow(),
	// 5 );

	PbClass *pyObj = PbClass::createPyObject(t.str() + T.str(), name, _args,
	                                         this->getParent());

	ParticleDataBase *pdata = dynamic_cast<ParticleDataBase *>(pyObj);
	if (!pdata) {
		errMsg("Unable to get particle data pointer from newly created object. "
		       "Only create ParticleData type with a ParticleSys.creat() call, "
		       "eg, PdataReal, PdataVec3 etc.");
		delete pyObj;
		return NULL;
	} else {
...
void ParticleBase::registerPdata(ParticleDataBase *pdata)
{
	...
	if (pdata->getType() == ParticleDataBase::TypeReal) {
		ParticleDataImpl<Real> *pd =
		    dynamic_cast<ParticleDataImpl<Real> *>(pdata);
		...
		ParticleDataImpl<int> *pd =
		    dynamic_cast<ParticleDataImpl<int> *>(pdata);
		...
		ParticleDataImpl<Vec3> *pd =
		    dynamic_cast<ParticleDataImpl<Vec3> *>(pdata);
...
void BasicParticleSystem::writeParticlesText(string name)
{
	...
	ofs << this->size() << ", pdata: " << mPartData.size() << " ("
	    << mPdataInt.size() << "," << mPdataReal.size() << ","
	    << mPdataVec3.size() << ") \n";
...
void BasicParticleSystem::printParts(IndexInt start, IndexInt stop,
                                     bool printIndex)
{
...
ParticleDataBase::~ParticleDataBase()
{
	...
	if (mpParticleSys) mpParticleSys->deregister(this);
}
...
void ParticleDataImpl<T>::setSource(Grid<T> *grid, bool isMAC)
{
	...
	if (isMAC)
		assertMsg(dynamic_cast<MACGrid *>(grid) != NULL,
		          "Given grid is not a valid MAC grid");
}
...
template <typename T> void ParticleDataImpl<T>::load(string name)
{
	...
	if (ext == ".uni")
		...
		errMsg("particle data '" + name +
		       "' filetype not supported for loading");
}
...
template <typename T> void ParticleDataImpl<T>::save(string name)
{
	...
	if (ext == ".uni")
		...
		errMsg("particle data '" + name +
		       "' filetype not supported for saving");
}
...
KERNEL(pts) template <class T, class S>
...
KERNEL(pts) template <class T, class S>
void knPdataScaledAdd(ParticleDataImpl<T> &me, const ParticleDataImpl<T> &other,
                      const S &factor)
{
...
{
	assertMsg(a.mData.size() == mData.size(), "different pdata size "
	                                              << a.mData.size() << " vs "
	                                              << this->mData.size());
...
template <typename T> void ParticleDataImpl<T>::setConst(T s)
{
...
template <typename T> void ParticleDataImpl<T>::addConst(T s)
{
	knPdataAddScalar<T, T> op(*this, s);
}

template <typename T>
void ParticleDataImpl<T>::addScaled(const ParticleDataImpl<T> &a,
                                    const T &factor)
{
...
template <typename T> void ParticleDataImpl<T>::multConst(T s)
{
	knPdataMultScalar<T, T> op(*this, s);
}

template <typename T> void ParticleDataImpl<T>::clamp(Real min, Real max)
{
	knPdataClamp<T> op(*this, min, max);
}

template <typename T>
KERNEL(pts, reduce = min)
...
template <typename T> Real ParticleDataImpl<T>::getMinValue()
{
	return sqrt(CompPdata_Min<T>(*this));
}

template <typename T> Real ParticleDataImpl<T>::getMaxAbsValue()
{
...
template <typename T> Real ParticleDataImpl<T>::getMaxValue()
{
	return sqrt(CompPdata_Max<T>(*this));
}

template <typename T>
void ParticleDataImpl<T>::printPdata(IndexInt start, IndexInt stop,
                                     bool printIndex)
{
	std::ostringstream sstr;
	IndexInt s = (start > 0 ? start : 0);
...
KERNEL(pts, reduce = max)
    returns(Real maxVal = -std::numeric_limits<Real>::min()) Real
...
template <> Real ParticleDataImpl<Vec3>::getMinValue()
{
	return sqrt(CompPdata_MinVec3(*this));
}

template <> Real ParticleDataImpl<Vec3>::getMaxAbsValue()
{
...
template <> Real ParticleDataImpl<Vec3>::getMaxValue()
{
