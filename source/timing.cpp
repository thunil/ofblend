...
void TimingData::stop(FluidSolver *parent, const string &name)
{
	if (mLastPlugin == name && name != "FluidSolver::step") {
		...
		for (vector<TimingSet>::iterator it = cur.begin(); it != cur.end();
		     it++) {
...
void TimingData::step()
{
	...
	for (it = mData.begin(); it != mData.end(); it++) {
		for (vector<TimingSet>::iterator it2 = it->second.begin();
...
void TimingData::saveMean(const string &filename)
{
	...
	for (it = mData.begin(); it != mData.end(); it++)
		for (vector<TimingSet>::iterator it2 = it->second.begin();
			...
			total += it2->cur;
			string name = it->first;
			if (it->second.size() > 1) name += "[" + it2->solver + "]";

			ofs << name << " " << (it2->total / it2->num) << endl;
		}

	ofs << endl << "Total : " << total << " (mean " << total / num << ")"
	    << endl;
	ofs.close();
}
}
