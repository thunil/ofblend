...
static const Vec3i neighbors[6] = { Vec3i(-1, 0, 0), Vec3i(1, 0, 0),
	                                Vec3i(0, -1, 0), Vec3i(0, 1, 0),
	                                Vec3i(0, 0, -1), Vec3i(0, 0, 1) };

KERNEL(bnd = 1)
void InitFmIn(FlagGrid &flags, Grid<int> &fmFlags, Grid<Real> &phi,
              bool ignoreWalls, int obstacleType)
{
...
KERNEL(bnd = 1)
void InitFmOut(FlagGrid &flags, Grid<int> &fmFlags, Grid<Real> &phi,
               bool ignoreWalls, int obstacleType)
{
	const IndexInt idx = flags.index(i, j, k);
	const Real v = phi[idx];
...
KERNEL(bnd = 1)
void SetUninitialized(Grid<int> &flags, Grid<int> &fmFlags, Grid<Real> &phi,
                      const Real val, int ignoreWalls, int obstacleType)
{
	if (ignoreWalls) {
		if ((fmFlags(i, j, k) != FlagInited) &&
		    ((flags(i, j, k) & obstacleType) == 0)) {
			phi(i, j, k) = val;
		}
	} else {
...
inline bool isAtInterface(Grid<int> &fmFlags, Grid<Real> &phi, const Vec3i &p)
{
	...
	for (int nb = 0; nb < max; nb++) {
		...
		if ((inward && phi(pn) >= 0.) || (!inward && phi(pn) < 0.)) return true;
	}
	return false;
}
...
static void doReinitMarch(Grid<Real> &phi, FlagGrid &flags, Real maxTime,
                          MACGrid *velTransport, bool ignoreWalls,
                          bool correctOuterLayer, int obstacleType)
{
	...
	FOR_IJK_BND(flags, 1)
	{
		...
		if (isAtInterface<true>(fmFlags, phi, p)) {
			...
			for (int nb = 0; nb < 2 * dim; nb++) {
				const Vec3i pn(
				    p + neighbors[nb]); // index always valid due to bnd=1
	...
	SetUninitialized(flags, fmFlags, phi, -maxTime - 1., ignoreWalls,
	                 obstacleType);

	InitFmOut(flags, fmFlags, phi, ignoreWalls, obstacleType);

	FastMarch<FmHeapEntryOut, +1> marchOut(flags, fmFlags, phi, maxTime,
	                                       velTransport);

	// by default, correctOuterLayer is on
	if (correctOuterLayer) {
		// normal version, inwards march is done, now add all outside values
		// (0..2] to list
		// note, this might move the interface a bit! but keeps a nice signed
		// distance field...
		FOR_IJK_BND(flags, 1)
		{
			if (ignoreWalls && ((flags(i, j, k) & obstacleType) != 0)) continue;
			...
			for (int nb = 0; nb < 2 * dim; nb++) {
				...
				// only add nodes near interface, not e.g. outer boundary vs.
				// invalid region
				if (nbPhi < 0 && nbPhi >= -2) marchOut.addToList(p, pn);
			}
		}
	} else {
		...
		FOR_IJK_BND(flags, 1)
		{
			...
			if (isAtInterface<false>(fmFlags, phi, p)) {
				...
				for (int nb = 0; nb < 2 * dim; nb++) {
					...
					// check neighbors of neighbor
					if (phi(pn) > 0. &&
					    !isAtInterface<false>(fmFlags, phi, pn)) {
...
void LevelsetGrid::reinitMarching(FlagGrid &flags, Real maxTime,
                                  MACGrid *velTransport, bool ignoreWalls,
                                  bool correctOuterLayer, int obstacleType)
{
	doReinitMarch(*this, flags, maxTime, velTransport, ignoreWalls,
	              correctOuterLayer, obstacleType);
}
	...
	for (int i = 0; i < mSize.x - 1; i++)
		for (int j = 0; j < mSize.y - 1; j++)
			for (int k = 0; k < mSize.z - 1; k++) {
				Real value[8] = {
					get(i, j, k),             get(i + 1, j, k),
					get(i + 1, j + 1, k),     get(i, j + 1, k),
					get(i, j, k + 1),         get(i + 1, j, k + 1),
					get(i + 1, j + 1, k + 1), get(i, j + 1, k + 1)
				};
				...
				for (int l = 0; l < 8; l++) {
					value[l] *= -1;
					if (-value[l] <= invalidTime) skip = true;
					if (value[l] < isoValue) cubeIdx |= 1 << l;
				}
				...
				int triIndices[12];
				int *eVert[12] = {
					&edgeVX(i, j, k),         &edgeVY(i + 1, j, k),
					&edgeVX(i, j + 1, k),     &edgeVY(i, j, k),
					&edgeVX(i, j, k + 1),     &edgeVY(i + 1, j, k + 1),
					&edgeVX(i, j + 1, k + 1), &edgeVY(i, j, k + 1),
					&edgeVZ(i, j, k),         &edgeVZ(i + 1, j, k),
					&edgeVZ(i + 1, j + 1, k), &edgeVZ(i, j + 1, k)
				};

				const Vec3 pos[9] = {
					Vec3(i, j, k),             Vec3(i + 1, j, k),
					Vec3(i + 1, j + 1, k),     Vec3(i, j + 1, k),
					Vec3(i, j, k + 1),         Vec3(i + 1, j, k + 1),
					Vec3(i + 1, j + 1, k + 1), Vec3(i, j + 1, k + 1)
				};
				...
				for (int e = 0; e < 12; e++) {
					if (mcEdgeTable[cubeIdx] & (1 << e)) {
						...
						if (*eVert[e] == 0) {
							...
							const float mu =
							    (isoValue - valp1) / (valp2 - valp1);

							// init isolevel vertex
							Node vertex;
							vertex.pos = p1 + (p2 - p1) * mu + Vec3(Real(0.5));
							vertex.normal = getNormalized(
							    getGradient(*this, i + cubieOffsetX[e1],
							                j + cubieOffsetY[e1],
							                k + cubieOffsetZ[e1]) *
							        (1.0 - mu) +
							    getGradient(*this, i + cubieOffsetX[e2],
							                j + cubieOffsetY[e2],
							                k + cubieOffsetZ[e2]) *
							        (mu));
				...
				for (int e = 0; mcTriTable[cubeIdx][e] != -1; e += 3) {
					mesh.addTri(
					    Triangle(triIndices[mcTriTable[cubeIdx][e + 0]] - 1,
