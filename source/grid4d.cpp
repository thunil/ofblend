...
Grid4d<T>::Grid4d(FluidSolver *parent, bool show)
    : Grid4dBase(parent)
{
	assertMsg(parent->is3D() && parent->has4D(),
	          "Solver does not support 4d grids , or is two-dimensional!");
	...
	for (int c = 1; c < 3; ++c)
		if (mSize[c] > sizemax) sizemax = mSize[c];
...
template <class T>
Grid4d<T>::Grid4d(const Grid4d<T> &a)
    : Grid4dBase(a.getParent())
{
	mSize = a.mSize;
	mType = a.mType;
	mStrideZ = a.mStrideZ;
	...
	memcpy(mData, a.mData,
	       sizeof(T) * a.mSize.x * a.mSize.y * a.mSize.z * a.mSize.t);
}
...
KERNEL(idx, reduce = min)
    returns(int minVal = std::numeric_limits<int>::max()) int kn4dMinInt(
        Grid4d<int> &val)
{
	if (val[idx] < minVal) minVal = val[idx];
}
...
KERNEL(idx, reduce = max)
    returns(int maxVal = std::numeric_limits<int>::min()) int kn4dMaxInt(
        Grid4d<int> &val)
{
	if (val[idx] > maxVal) maxVal = val[idx];
}
...
KERNEL(idx, reduce = min)
    returns(Real minVal = std::numeric_limits<Real>::max()) template <class VEC>
...
KERNEL(idx, reduce = max) returns(
    Real maxVal = -std::numeric_limits<Real>::max()) template <class VEC>
...
template <class T>
Grid4d<T> &Grid4d<T>::copyFrom(const Grid4d<T> &a, bool copyType)
{
	assertMsg(a.mSize.x == mSize.x && a.mSize.y == mSize.y &&
	              a.mSize.z == mSize.z && a.mSize.t == mSize.t,
	          "different Grid4d resolutions " << a.mSize << " vs "
	                                          << this->mSize);
...
KERNEL(idx) template <class T> void kn4dSetConstReal(Grid4d<T> &me, T val)
{
	me[idx] = val;
}
KERNEL(idx) template <class T> void kn4dAddConstReal(Grid4d<T> &me, T val)
{
	me[idx] += val;
}
KERNEL(idx) template <class T> void kn4dMultConst(Grid4d<T> &me, T val)
{
	me[idx] *= val;
}
KERNEL(idx) template <class T> void kn4dClamp(Grid4d<T> &me, T min, T max)
{
...
template <class T>
void Grid4d<T>::addScaled(const Grid4d<T> &a, const T &factor)
{
...
template <> Real Grid4d<Real>::getMax() { return kn4dMaxReal(*this); }
template <> Real Grid4d<Real>::getMin() { return kn4dMinReal(*this); }
...
template <> Real Grid4d<int>::getMax() { return (Real)kn4dMaxInt(*this); }
template <> Real Grid4d<int>::getMin() { return (Real)kn4dMinInt(*this); }
...
template <class T>
void Grid4d<T>::printGrid(int zSlice, int tSlice, bool printIndex, int bnd)
{
	...
	out << endl;
	debMsg("Printing '" << this->getName() << "' " << out.str().c_str() << " ",
	       1);
}

// helper to set/get components of vec4 Grids
KERNEL(idx) void knGetComp4d(const Grid4d<Vec4> &src, Grid4d<Real> &dst, int c)
{
	dst[idx] = src[idx][c];
};
KERNEL(idx) void knSetComp4d(const Grid4d<Real> &src, Grid4d<Vec4> &dst, int c)
{
	dst[idx][c] = src[idx];
};
...
KERNEL(fourd) template <class T>
void knSetBnd4d(Grid4d<T> &grid, T value, int w)
{
	bool bnd = (i <= w || i >= grid.getSizeX() - 1 - w || j <= w ||
	            j >= grid.getSizeY() - 1 - w || k <= w ||
	            k >= grid.getSizeZ() - 1 - w || t <= w ||
	            t >= grid.getSizeT() - 1 - w);
	if (bnd) grid(i, j, k, t) = value;
}
...
KERNEL(fourd) template <class T> void knSetBnd4dNeumann(Grid4d<T> &grid, int w)
{
	...
	if (set) grid(i, j, k, t) = grid(si, sj, sk, st);
}
...
//! compute maximal diference of two cells in the grid, needed for testing
//system
PYTHON() Real grid4dMaxDiff(Grid4d<Real> &g1, Grid4d<Real> &g2)
{
	...
	{
		maxVal =
		    std::max(maxVal, (double)fabs(g1(i, j, k, t) - g2(i, j, k, t)));
	}
	return maxVal;
}
PYTHON() Real grid4dMaxDiffInt(Grid4d<int> &g1, Grid4d<int> &g2)
{
	...
	{
		maxVal = std::max(
		    maxVal, (double)fabs((double)g1(i, j, k, t) - g2(i, j, k, t)));
	}
	return maxVal;
}
...
void knSetRegion4d(Grid4d<S> &dst, Vec4 start, Vec4 end, S value)
{
	Vec4 p(i, j, k, t);
	for (int c = 0; c < 4; ++c)
		if (p[c] < start[c] || p[c] > end[c]) return;
	dst(i, j, k, t) = value;
}
...
PYTHON() void setRegion4dVec4(Grid4d<Vec4> &dst, Vec4 start, Vec4 end,
                              Vec4 value)
{
...
PYTHON() void getSliceFrom4dVec(Grid4d<Vec4> &src, int srct, Grid<Vec3> &dst,
                                Grid<Real> *dstt = NULL)
{
	const int bnd = 0;
	if (!src.isInBounds(Vec4i(bnd, bnd, bnd, srct))) return;
...
//! the other way, slow helper to place a 3d grid into a 4d one
PYTHON() void placeGrid3d(Grid<Real> &src, Grid4d<Real> &dst, int dstt)
{
	FOR_IJK(src)
	{
		if (!dst.isInBounds(Vec4i(i, j, k, dstt))) continue;
		dst(i, j, k, dstt) = src(i, j, k);
	}
}
...
KERNEL(fourd) template <class S>
void knInterpol4d(Grid4d<S> &target, Grid4d<S> &source, const Vec4 &srcFac,
                  const Vec4 &offset)
{
...
PYTHON() void interpolateGrid4d(Grid4d<Real> &target, Grid4d<Real> &source,
                                Vec4 offset = Vec4(0.), Vec4 scale = Vec4(1.),
                                Vec4 size = Vec4(-1.))
{
	Vec4 srcFac(1.), off2 = offset;
	gridFactor4d(toVec4(source.getSize()), toVec4(target.getSize()), size,
	             scale, srcFac, off2);
	knInterpol4d<Real>(target, source, srcFac, off2);
}
//! linearly interpolate vec4 data of a 4d grid
PYTHON() void interpolateGrid4dVec(Grid4d<Vec4> &target, Grid4d<Vec4> &source,
                                   Vec4 offset = Vec4(0.),
                                   Vec4 scale = Vec4(1.), Vec4 size = Vec4(-1.))
{
	Vec4 srcFac(1.), off2 = offset;
	gridFactor4d(toVec4(source.getSize()), toVec4(target.getSize()), size,
	             scale, srcFac, off2);
