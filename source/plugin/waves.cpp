...
void knCalcSecDeriv2d(const Grid<Real> &v, Grid<Real> &ret)
{
	...
	ret(i, j, k) = (-4. * v(i, j, k) + v(i - 1, j, k) + v(i + 1, j, k) +
	                v(i, j - 1, k) + v(i, j + 1, k));
};
...
KERNEL(bnd = 1, reduce = +)
    returns(double sum = 0) void knTotalSum(Grid<Real> &h)
{
	sum += h(i, j, k);
}
...
KERNEL(bnd = 1)
void MakeRhsWE(FlagGrid &flags, Grid<Real> &rhs, Grid<Real> &ut,
               Grid<Real> &utm1, Real s, bool crankNic = false)
{
	rhs(i, j, k) = (2. * ut(i, j, k) - utm1(i, j, k));
	if (crankNic) {
		rhs(i, j, k) += s * (-4. * ut(i, j, k) + 1. * ut(i - 1, j, k) +
		                     1. * ut(i + 1, j, k) + 1. * ut(i, j - 1, k) +
		                     1. * ut(i, j + 1, k));
	}
}

//! do a CG solve for the wave equation (note, out grid only there for
//debugging... could be removed)
PYTHON() void cgSolveWE(FlagGrid &flags, Grid<Real> &ut, Grid<Real> &utm1,
                        Grid<Real> &out, bool crankNic = false,
                        Real cSqr = 0.25, Real cgMaxIterFac = 1.5,
                        Real cgAccuracy = 1e-5)
{
	...
	GridCgInterface *gcg;
	if (flags.is3D())
		gcg = new GridCg<ApplyMatrix>(out, rhs, residual, search, flags, tmp,
		                              &A0, &Ai, &Aj, &Ak);
	else
		gcg = new GridCg<ApplyMatrix2D>(out, rhs, residual, search, flags, tmp,
		                                &A0, &Ai, &Aj, &Ak);
	...
	debMsg("FluidSolver::solvePressure iterations:"
	           << gcg->getIterations() << ", res:" << gcg->getSigma(),
	       1);
