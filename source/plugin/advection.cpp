...
void SemiLagrange(FlagGrid &flags, MACGrid &vel, Grid<T> &dst, Grid<T> &src,
                  Real dt, bool isLevelset, int orderSpace)
{
	// traceback position
	Vec3 pos =
	    Vec3(i + 0.5f, j + 0.5f, k + 0.5f) - vel.getCentered(i, j, k) * dt;
	dst(i, j, k) = src.getInterpolatedHi(pos, orderSpace);
}

//! Semi-Lagrange interpolation kernel for MAC grids
KERNEL(bnd = 1)
void SemiLagrangeMAC(FlagGrid &flags, MACGrid &vel, MACGrid &dst, MACGrid &src,
                     Real dt, int orderSpace)
{
	// get currect velocity at MAC position
	// no need to shift xpos etc. as lookup field is also shifted
	Vec3 xpos =
	    Vec3(i + 0.5f, j + 0.5f, k + 0.5f) - vel.getAtMACX(i, j, k) * dt;
	Real vx = src.getInterpolatedComponentHi<0>(xpos, orderSpace);
	Vec3 ypos =
	    Vec3(i + 0.5f, j + 0.5f, k + 0.5f) - vel.getAtMACY(i, j, k) * dt;
	Real vy = src.getInterpolatedComponentHi<1>(ypos, orderSpace);
	Vec3 zpos =
	    Vec3(i + 0.5f, j + 0.5f, k + 0.5f) - vel.getAtMACZ(i, j, k) * dt;
...
void MacCormackCorrect(FlagGrid &flags, Grid<T> &dst, Grid<T> &old,
                       Grid<T> &fwd, Grid<T> &bwd, Real strength,
                       bool isLevelSet, bool isMAC = false)
{
	dst[idx] = fwd[idx];

	if (flags.isFluid(idx)) {
		// only correct inside fluid region; note, strenth of correction can be
		// modified here
...
void MacCormackCorrectMAC(FlagGrid &flags, Grid<T> &dst, Grid<T> &old,
                          Grid<T> &fwd, Grid<T> &bwd, Real strength,
                          bool isLevelSet, bool isMAC = false)
{
	...
	for (int c = 0; c < 3; ++c) {
		if (skip[c]) {
			...
			dst(i, j, k)[c] =
			    fwd(i, j, k)[c] +
			    strength * 0.5 * (old(i, j, k)[c] - bwd(i, j, k)[c]);
		}
	}
}
...
template <class T>
inline T doClampComponent(const Vec3i &gridSize, T dst, Grid<T> &orig, T fwd,
                          const Vec3 &pos, const Vec3 &vel)
{
	T minv(std::numeric_limits<Real>::max()),
	    maxv(-std::numeric_limits<Real>::max());
	...
	for (int l = 0; l < 2; ++l) {
		...
		const int k0 =
		    clamp(currPos.z, 0, (orig.is3D() ? (gridSize.z - 1) : 1));
		const int i1 = i0 + 1, j1 = j0 + 1, k1 = (orig.is3D() ? (k0 + 1) : k0);
		if ((!orig.isInBounds(Vec3i(i0, j0, k0), 0)) ||
		    (!orig.isInBounds(Vec3i(i1, j1, k1), 0)))
			return fwd;
...
template <int c>
inline Real doClampComponentMAC(const Vec3i &gridSize, Real dst, MACGrid &orig,
                                Real fwd, const Vec3 &pos, const Vec3 &vel)
{
...
KERNEL(bnd = 1) template <class T>
void MacCormackClamp(FlagGrid &flags, MACGrid &vel, Grid<T> &dst, Grid<T> &orig,
                     Grid<T> &fwd, Real dt)
{
	T dval = dst(i, j, k);
	Vec3i gridUpper = flags.getSize() - 1;

	dval = doClampComponent<T>(gridUpper, dval, orig, fwd(i, j, k),
	                           Vec3(i, j, k), vel.getCentered(i, j, k) * dt);

	// lookup forward/backward , round to closest NB
	Vec3i posFwd = toVec3i(Vec3(i, j, k) + Vec3(0.5, 0.5, 0.5) -
	                       vel.getCentered(i, j, k) * dt);
	Vec3i posBwd = toVec3i(Vec3(i, j, k) + Vec3(0.5, 0.5, 0.5) +
	                       vel.getCentered(i, j, k) * dt);
	...
	if (posFwd.x < 0 || posFwd.y < 0 || posFwd.z < 0 || posBwd.x < 0 ||
	    ...
	    posFwd.y > gridUpper.y || ((posFwd.z > gridUpper.z) && flags.is3D()) ||
	    posBwd.x > gridUpper.x || posBwd.y > gridUpper.y ||
	    ((posBwd.z > gridUpper.z) && flags.is3D()) ||
...
KERNEL(bnd = 1)
void MacCormackClampMAC(FlagGrid &flags, MACGrid &vel, MACGrid &dst,
                        MACGrid &orig, MACGrid &fwd, Real dt)
{
	Vec3 pos(i, j, k);
	Vec3 dval = dst(i, j, k);
	Vec3 dfwd = fwd(i, j, k);
	...
	// note - the MAC version currently does not check whether source points
	// were inside an obstacle! (unlike centered version)
...
void fnAdvectSemiLagrange(FluidSolver *parent, FlagGrid &flags, MACGrid &vel,
                          ...
                          int orderSpace, bool openBounds, int bWidth)
{
	...
	for (int n = -nmax; n <= nmax; n++) {
		for (int m = -size; m <= size; m++) {
			for (int l = -size; l <= size; l++) {
				if (flags.isInBounds(Vec3i(i + l, j + m, k + n), 0) &&
				    (flags.isFluid(i + l, j + m, k + n) ||
				     flags.isOutflow(i + l, j + m, k + n))) {
					avg += vel(i + l, j + m, k + n);
					count++;
				}
			}
...
KERNEL() void extrapolateVelConvectiveBC(FlagGrid &flags, MACGrid &vel,
                                         MACGrid &velDst, MACGrid &velPrev,
                                         Real timeStep, int bWidth)
{
	if (flags.isOutflow(i, j, k)) {
		...
		for (int c = 0; c < dim; c++) {
			Real factor =
			    timeStep * max((Real)1.0, bulkVel[c]); // prevent the
			                                           // extrapolated velocity
			                                           // from exploding when
			                                           // bulk velocity below 1
			low[c] = flLow[c] = cur[c] - 1;
			up[c] = flUp[c] = cur[c] + 1;
			// iterate over bWidth to allow for extrapolation into more distant
			// outflow cells
			for (int d = 0; d < bWidth + 1; d++) {
				if (cur[c] > d && flags.isFluid(flLow)) {
					velDst(i, j, k)[c] =
					    ((vel(i, j, k)[c] - velPrev(i, j, k)[c]) / factor) +
					    vel(low)[c];
					done = true;
				}
				if (cur[c] < flags.getSize()[c] - d - 1 &&
				    flags.isFluid(flUp)) {
					// check for cells equally far away from two fluid cells ->
					// average value between both sides
					if (done)
						velDst(i, j, k)[c] =
						    0.5 * (velDst(i, j, k)[c] +
						           ((vel(i, j, k)[c] - velPrev(i, j, k)[c]) /
						            factor) +
						           vel(up)[c]);
					else
						velDst(i, j, k)[c] =
...
//! extrapolate normal velocity components into open boundary cells (marked as
//outflow cells)
void applyOutflowBC(FlagGrid &flags, MACGrid &vel, MACGrid &velPrev,
                    double timeStep, int bWidth = 1)
{
	MACGrid velDst(vel.getParent()); // do not overwrite vel while it is read
	extrapolateVelConvectiveBC(flags, vel, velDst, velPrev,
	                           max(1.0, timeStep * 4), bWidth);
...
void fnAdvectSemiLagrange<MACGrid>(FluidSolver *parent, FlagGrid &flags,
                                   MACGrid &vel, MACGrid &orig, int order,
                                   Real strength, int orderSpace,
                                   bool openBounds, int bWidth)
{
	Real dt = parent->getDt();
	...
	if (order == 1) {
		...
		// newGrid <- compute correction
		MacCormackCorrectMAC<Vec3>(flags, newGrid, orig, fwd, bwd, strength,
		                           false, true);
...
PYTHON() void advectSemiLagrange(FlagGrid *flags, MACGrid *vel, GridBase *grid,
                                 ...
                                 int orderSpace = 1, bool openBounds = false,
                                 int boundaryWidth = 1)
{
	assertMsg(order == 1 || order == 2, "AdvectSemiLagrange: Only order 1 "
	                                    "(regular SL) and 2 (MacCormack) "
	                                    "supported");

	// determine type of grid
	if (grid->getType() & GridBase::TypeReal) {
		fnAdvectSemiLagrange<Grid<Real> >(
		    flags->getParent(), *flags, *vel, *((Grid<Real> *)grid), order,
		    strength, orderSpace, openBounds, boundaryWidth);
	} else if (grid->getType() & GridBase::TypeMAC) {
		fnAdvectSemiLagrange<MACGrid>(flags->getParent(), *flags, *vel,
		                              *((MACGrid *)grid), order, strength,
		                              orderSpace, openBounds, boundaryWidth);
	} else if (grid->getType() & GridBase::TypeVec3) {
		fnAdvectSemiLagrange<Grid<Vec3> >(
		    flags->getParent(), *flags, *vel, *((Grid<Vec3> *)grid), order,
		    strength, orderSpace, openBounds, boundaryWidth);
	} else
		errMsg("AdvectSemiLagrange: Grid Type is not supported (only Real, "
		       "Vec3, MAC, Levelset)");
}
