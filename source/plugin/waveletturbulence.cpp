...
// first some fairly generic interpolation functions for grids with multiple
// sizes

//! same as in grid.h , but takes an additional optional "desired" size
inline void calcGridSizeFactorMod(Vec3i s1, Vec3i s2, Vec3i optSize, Vec3 scale,
                                  Vec3 &sourceFactor, Vec3 &retOff)
{
	for (int c = 0; c < 3; c++) {
		if (optSize[c] > 0) {
			s2[c] = optSize[c];
...
PYTHON() void interpolateGrid(Grid<Real> &target, Grid<Real> &source,
                              Vec3 scale = Vec3(1.), Vec3 offset = Vec3(0.),
                              Vec3i size = Vec3i(-1, -1, -1),
                              int orderSpace = 1)
{
	Vec3 sourceFactor(1.), off2 = offset;
	calcGridSizeFactorMod(source.getSize(), target.getSize(), size, scale,
	                      sourceFactor, off2);

	// a brief note on a mantaflow specialty: the target grid has to be the
	// first argument here!
	// the parent fluidsolver object is taken from the first grid, and it
	// determines the size of the
	// loop for the kernel call. as we're writing into target, it's important to
	// loop exactly over
	// all cells of the target grid... (note, when calling the plugin in python,
	// it doesnt matter anymore).

	// sourceFactor offset necessary to shift eval points by half a small cell
	// width
	knInterpolateGridTempl<Real>(target, source, sourceFactor, off2,
	                             orderSpace);
}

PYTHON() void interpolateGridVec3(Grid<Vec3> &target, Grid<Vec3> &source,
                                  Vec3 scale = Vec3(1.), Vec3 offset = Vec3(0.),
                                  Vec3i size = Vec3i(-1, -1, -1),
                                  int orderSpace = 1)
{
	...
	calcGridSizeFactorMod(source.getSize(), target.getSize(), size, scale,
	                      sourceFactor, off2);
	knInterpolateGridTempl<Vec3>(target, source, sourceFactor, off2,
	                             orderSpace);
}

//!interpolate a mac velocity grid from one size to another size
KERNEL()
void KnInterpolateMACGrid(MACGrid &target, MACGrid &source,
                          const Vec3 &sourceFactor, const Vec3 &off,
                          int orderSpace)
{
...
//! Apply vector noise to grid, this is a simplified version - no position
//scaling or UVs
KERNEL()
void knApplySimpleNoiseVec(FlagGrid &flags, Grid<Vec3> &target,
...
PYTHON() void applySimpleNoiseVec3(FlagGrid &flags, Grid<Vec3> &target,
                                   WaveletNoiseField &noise, Real scale = 1.0,
                                   Grid<Real> *weight = NULL)
{
	// note - passing a MAC grid here is slightly inaccurate, we should evaluate
...
KERNEL()
void knApplySimpleNoiseReal(FlagGrid &flags, Grid<Real> &target,
                            WaveletNoiseField &noise, Real scale,
                            Grid<Real> *weight)
{
	if (!flags.isFluid(i, j, k)) return;
...
PYTHON() void applySimpleNoiseReal(FlagGrid &flags, Grid<Real> &target,
                                   WaveletNoiseField &noise, Real scale = 1.0,
                                   Grid<Real> *weight = NULL)
{
...
//! This is the version with more functionality - supports uv grids, and
//on-the-fly interpolation
//! of input grids.
KERNEL()
void knApplyNoiseVec(FlagGrid &flags, Grid<Vec3> &target,
                     WaveletNoiseField &noise, Real scale, Real scaleSpatial,
                     Grid<Real> *weight, Grid<Vec3> *uv, bool uvInterpol,
                     const Vec3 &sourceFactor)
{
	...
	if (uv) {
		if (!uvInterpol) {
			...
			// uv coordinates are in local space - so we need to adjust the
...
PYTHON() void applyNoiseVec3(FlagGrid &flags, Grid<Vec3> &target,
                             WaveletNoiseField &noise, Real scale = 1.0,
                             Real scaleSpatial = 1.0, Grid<Real> *weight = NULL,
                             Grid<Vec3> *uv = NULL)
{
	...
	if (uv && weight)
		assertMsg(uv->getSize() == weight->getSize(),
		          "UV and weight grid have to match!");

	// note - passing a MAC grid here is slightly inaccurate, we should evaluate
	// each component separately
	knApplyNoiseVec(flags, target, noise, scale, scaleSpatial, weight, uv,
	                uvInterpol, sourceFactor);
}
...
PYTHON() void computeVorticity(MACGrid &vel, Grid<Vec3> &vorticity,
                               Grid<Real> *norm)
{
...
// note - very similar to KnComputeProductionStrain, but for use as wavelet turb
// weighting
KERNEL(bnd = 1)
void KnComputeStrainRateMag(const MACGrid &vel, const Grid<Vec3> &velCenter,
                            Grid<Real> &prod)
{
                               ...
                               int flagTo = FlagGrid::TypeObstacle)
{
	...
	const Vec3i nb[6] = { Vec3i(1, 0, 0),  Vec3i(-1, 0, 0), Vec3i(0, 1, 0),
	...
	FOR_IJK_BND(flags, 0)
	{
		if (flags(i, j, k) & flagFrom) tmp(Vec3i(i, j, k)) = 1;
		if (!foundTarget && (flags(i, j, k) & flagTo)) foundTarget = true;
	}
	// optimization, skip extrapolation if we dont have any cells to extrapolate
	// to
	if (!foundTarget) {
...
PYTHON() void extrapolateSimpleFlags(FlagGrid &flags, GridBase *val,
                                     int distance = 4,
                                     int flagFrom = FlagGrid::TypeFluid,
                                     int flagTo = FlagGrid::TypeObstacle)
{
	if (val->getType() & GridBase::TypeReal) {
		extrapolSimpleFlagsHelper<Real>(flags, *((Grid<Real> *)val), distance,
		                                flagFrom, flagTo);
	} else if (val->getType() & GridBase::TypeInt) {
		extrapolSimpleFlagsHelper<int>(flags, *((Grid<int> *)val), distance,
		                               flagFrom, flagTo);
	} else if (val->getType() & GridBase::TypeVec3) {
		extrapolSimpleFlagsHelper<Vec3>(flags, *((Grid<Vec3> *)val), distance,
		                                flagFrom, flagTo);
	} else
		errMsg("extrapolateSimpleFlags: Grid Type is not supported (only int, "
		       "Real, Vec3)");
}
