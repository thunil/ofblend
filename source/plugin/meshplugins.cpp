...
//! Mesh smoothing
/*! see Desbrun 99 "Implicit fairing of of irregular meshes using diffusion and
 * curvature flow"*/
PYTHON() void smoothMesh(Mesh &mesh, Real strength, int steps = 1,
                         Real minLength = 1e-5)
{
	...
	for (int s = 0; s < steps; s++) {
		...
		for (int c = 0; c < numCorners; c++) {
			...
			visited[node] = true;
			temp[node] = pos;
			if (totalLen != 0) temp[node] += dx * (str / totalLen);
		}

		// copy back
		for (int n = 0; n < numNodes; n++)
			if (!mesh.isNodeFixed(n)) mesh.nodes(n).pos = temp[n];
	}
...
PYTHON() void subdivideMesh(Mesh &mesh, Real minAngle, Real minLength,
                            Real maxLength, bool cutTubes = false)
{
	...
	for (int t = 0; t < mesh.numTris(); t++) {
		if (taintedTris.find(t) != taintedTris.end()) continue;
		...
		if (k[0] && k[1])
			CollapseEdge(mesh, t, 2, mesh.getEdge(t, 0), mesh.getNode(t, 0),
			             deletedNodes, taintedTris, edgeKill, cutTubes);
		else if (k[1] && k[2])
			CollapseEdge(mesh, t, 0, mesh.getEdge(t, 1), mesh.getNode(t, 1),
			             deletedNodes, taintedTris, edgeKill, cutTubes);
		else if (k[2] && k[0])
			CollapseEdge(mesh, t, 1, mesh.getEdge(t, 2), mesh.getNode(t, 2),
			             deletedNodes, taintedTris, edgeKill, cutTubes);
	}
	...
	if (minAngle > 0) {
		for (int t = 0; t < mesh.numTris(); t++) {
			...
			// we achieve this in a method very similar to the above subdivision
			// method.
			...
			Vec3 e0 = mesh.getEdge(t, 0), e1 = mesh.getEdge(t, 1),
			     e2 = mesh.getEdge(t, 2);
			Vec3 ne0 = e0;
			Vec3 ne1 = e1;
			Vec3 ne2 = e2;
			...
			if (worstAngle < minAngle) {
				...
				CollapseEdge(mesh, t, which, edgevect, endpoint, deletedNodes,
				             taintedTris, edgeCollsAngle, cutTubes);
			}
		}
	}
	...
	if (maxLength > 0) {
		...
		while (!pq.empty() && pq.top().first > maxLength2) {
			// we only want to run through the edge list ONCE
			// and we want to subdivide the original edges before we subdivide
			// any newer, shorter edges,
			// so whenever we subdivide, we add the 2 new triangles on the end
			// of the SurfaceTri vector
			...
			if (taintedTris.find(triA) != taintedTris.end()) continue;

			// first we find the maximum length edge in this triangle
			Vec3 e0 = mesh.getEdge(triA, 0), e1 = mesh.getEdge(triA, 1),
			     e2 = mesh.getEdge(triA, 2);
			Real d0 = normSquare(e0);
			Real d1 = normSquare(e1);
			Real d2 = normSquare(e2);
			...
			if (haveB)
				newNode.pos = ModifiedButterflySubdivision(
				    mesh, ca_old[0], cb_old[0], newNode.pos);
			...
			// write to array
			mesh.addTri(
			    Triangle(ca_old[0].node, ca_old[1].node, mesh.numNodes()));
			mesh.addTri(
			    Triangle(ca_old[0].node, mesh.numNodes(), ca_old[2].node));
			if (haveB) {
				mesh.addTri(
				    Triangle(cb_old[0].node, cb_old[1].node, mesh.numNodes()));
				mesh.addTri(
				    Triangle(cb_old[0].node, mesh.numNodes(), cb_old[2].node));
			}
			...
			Real areaA1 = mesh.getFaceArea(triA1),
			     areaA2 = mesh.getFaceArea(triA2);
			...
			for (int i = 0; i < mesh.numTriChannels(); i++) {
				...
				if (haveB) {
					mesh.triChannel(i)
					    ->addSplit(triB, areaB1 / (areaB1 + areaB2));
					mesh.triChannel(i)
					    ->addSplit(triB, areaB2 / (areaB1 + areaB2));
				}
			}
			...
			for (int i = mesh.numTris() - nt; i < mesh.numTris(); i++) {
				...
				Vec3 ne0 = mesh.getEdge(i, 0), ne1 = mesh.getEdge(i, 1),
				     ne2 = mesh.getEdge(i, 2);
	...
	if (minLength > 0) {
		...
		for (int t = 0; t < mesh.numTris(); t++) {
			...
			if (dist2 < minLength2) {
				CollapseEdge(mesh, t, which, edgevect, endpoint, deletedNodes,
				             taintedTris, edgeCollsLen, cutTubes);
			}
		}
	}
	...
	cout << "Surface subdivision finished with " << mesh.numNodes()
	     << " surface nodes and " << mesh.numTris();
	cout << " surface triangles, edgeSubdivs:" << edgeSubdivs
	     << ", edgeCollapses: " << edgeCollsLen;
...
PYTHON() void killSmallComponents(Mesh &mesh, int elements = 10)
{
	...
	if (!taintedTris.empty())
		cout << "Killed small components : " << deletedNodes.size()
		     << " nodes, " << taintedTris.size() << " tris deleted." << endl;
}
