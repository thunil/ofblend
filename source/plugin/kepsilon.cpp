...
KERNEL(idx)
void KnTurbulenceClamp(Grid<Real> &kgrid, Grid<Real> &egrid, Real minK,
                       Real maxK, Real minNu, Real maxNu)
{
	Real eps = egrid[idx];
	Real ke = clamp(kgrid[idx], minK, maxK);
	Real nu = keCmu * square(ke) / eps;
	if (nu > maxNu) eps = keCmu * square(ke) / maxNu;
	if (nu < minNu) eps = keCmu * square(ke) / minNu;

	kgrid[idx] = ke;
	egrid[idx] = eps;
}

//! Compute k-epsilon production term P = 2*nu_T*sum_ij(Sij^2) and the turbulent
//viscosity nu_T=C_mu*k^2/eps
KERNEL(bnd = 1)
void KnComputeProduction(const MACGrid &vel, const Grid<Vec3> &velCenter,
                         const Grid<Real> &ke, const Grid<Real> &eps,
                         Grid<Real> &prod, Grid<Real> &nuT, Grid<Real> *strain,
                         Real pscale = 1.0f)
{
	...
	if (curEps > 0) {
		...
		Vec3 diag =
		    Vec3(vel(i + 1, j, k).x, vel(i, j + 1, k).y, vel(i, j, k + 1).z) -
		    vel(i, j, k);
...
//! Compute k-epsilon production term P = 2*nu_T*sum_ij(Sij^2) and the turbulent
//viscosity nu_T=C_mu*k^2/eps
PYTHON() void KEpsilonComputeProduction(MACGrid &vel, Grid<Real> &k,
                                        Grid<Real> &eps, Grid<Real> &prod,
                                        Grid<Real> &nuT, Grid<Real> *strain = 0,
                                        Real pscale = 1.0f)
{
...
KERNEL(idx)
void KnAddTurbulenceSource(Grid<Real> &kgrid, Grid<Real> &egrid,
                           const Grid<Real> &pgrid, Real dt)
{
...
PYTHON() void KEpsilonBcs(FlagGrid &flags, Grid<Real> &k, Grid<Real> &eps,
                          Real intensity, Real nu, bool fillArea)
{
	// compute limits
...
//! Gradient diffusion smoothing. Not unconditionally stable -- should probably
//do substepping etc.
void ApplyGradDiff(const Grid<Real> &grid, Grid<Real> &res,
                   const Grid<Real> &nu, Real dt, Real sigma)
{
...
PYTHON() void KEpsilonGradientDiffusion(Grid<Real> &k, Grid<Real> &eps,
                                        Grid<Real> &nuT, Real sigmaU = 4.0,
                                        MACGrid *vel = 0)
{
	Real dt = k.getParent()->getDt();
