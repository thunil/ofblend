...
//! Apply noise to grid
KERNEL()
void KnApplyNoiseInfl(FlagGrid &flags, Grid<Real> &density,
                      WaveletNoiseField &noise, Grid<Real> &sdf, Real scale,
                      Real sigma)
{
	...
	if (density(i, j, k) < target) density(i, j, k) = target;
}

//! Init noise-modulated density inside shape
PYTHON() void densityInflow(FlagGrid &flags, Grid<Real> &density,
                            WaveletNoiseField &noise, Shape *shape,
                            Real scale = 1.0, Real sigma = 0)
{
...
KERNEL() void KnAddNoise(FlagGrid &flags, Grid<Real> &density,
                         WaveletNoiseField &noise, Grid<Real> *sdf, Real scale)
{
	if (!flags.isFluid(i, j, k) || (sdf && (*sdf)(i, j, k) > 0.)) return;
	density(i, j, k) += noise.evaluate(Vec3(i, j, k)) * scale;
}
PYTHON() void addNoise(FlagGrid &flags, Grid<Real> &density,
                       WaveletNoiseField &noise, Grid<Real> *sdf = NULL,
                       Real scale = 1.0)
{
	KnAddNoise(flags, density, noise, sdf, scale);
}

//! sample noise field and set pdata with its values (for convenience, scale the
//noise values)
KERNEL(pts) template <class T>
void knSetPdataNoise(BasicParticleSystem &parts, ParticleDataImpl<T> &pdata,
                     WaveletNoiseField &noise, Real scale)
{
	pdata[idx] = noise.evaluate(parts.getPos(idx)) * scale;
}
KERNEL(pts) template <class T>
void knSetPdataNoiseVec(BasicParticleSystem &parts, ParticleDataImpl<T> &pdata,
                        WaveletNoiseField &noise, Real scale)
{
	pdata[idx] = noise.evaluateVec(parts.getPos(idx)) * scale;
}
PYTHON() void setNoisePdata(BasicParticleSystem &parts,
                            ParticleDataImpl<Real> &pd,
                            WaveletNoiseField &noise, Real scale = 1.)
{
	knSetPdataNoise<Real>(parts, pd, noise, scale);
}
PYTHON() void setNoisePdataVec3(BasicParticleSystem &parts,
                                ParticleDataImpl<Vec3> &pd,
                                WaveletNoiseField &noise, Real scale = 1.)
{
	knSetPdataNoiseVec<Vec3>(parts, pd, noise, scale);
}
PYTHON() void setNoisePdataInt(BasicParticleSystem &parts,
...
PYTHON() Grid<Vec3> obstacleGradient(FlagGrid &flags)
{
	...
	FOR_IDX(levelset) { levelset[idx] = flags.isObstacle(idx) ? -0.5 : 0.5; }
	...
	FOR_IDX(levelset)
	{
		...
		if (s <= 0.1 || levelset[idx] >= 0) grad = Vec3(0.);
...
KERNEL()
void KnApplyEmission(FlagGrid &flags, Grid<Real> &density, Grid<Real> &emission,
                     bool isAbsolute)
{
...
PYTHON() void applyEmission(FlagGrid &flags, Grid<Real> &density,
                            Grid<Real> &emission, bool isAbsolute)
{
...
KERNEL()
void KnApplyDensity(FlagGrid &flags, Grid<Real> &density, Grid<Real> &sdf,
                    Real value, Real sigma)
{
	if (!flags.isFluid(i, j, k) || sdf(i, j, k) > sigma) return;
	density(i, j, k) = value;
}
//! Init noise-modulated density inside mesh
PYTHON() void densityInflowMeshNoise(FlagGrid &flags, Grid<Real> &density,
                                     WaveletNoiseField &noise, Mesh *mesh,
                                     Real scale = 1.0, Real sigma = 0)
{
...
PYTHON() void densityInflowMesh(FlagGrid &flags, Grid<Real> &density,
                                Mesh *mesh, Real value = 1., Real cutoff = 7,
                                Real sigma = 0)
{
	LevelsetGrid sdf(density.getParent(), false);
...
PYTHON() void checkSymmetry(Grid<Real> &a, Grid<Real> *err = NULL,
                            bool symmetrize = false, int axis = 0,
                            int bound = 0)
{
	const int c = axis;
	...
	FOR_IJK(a)
	{
		...
		mdx[c] = s - 1 - idx[c];
		if (bound > 0 &&
		    ((!a.isInBounds(idx, bound)) || (!a.isInBounds(mdx, bound))))
			continue;
...
PYTHON() void checkSymmetryVec3(Grid<Vec3> &a, Grid<Real> *err = NULL,
                                bool symmetrize = false, int axis = 0,
...
void projectImg(SimpleImage &img, Grid<Real> &val, int shadeMode = 0,
                Real scale = 1.);

//! output shaded (all 3 axes at once for 3D)
//! shading modes: 0 smoke, 1 surfaces
PYTHON() void projectPpmFull(Grid<Real> &val, string name, int shadeMode = 0,
                             Real scale = 1.)
{
	SimpleImage img;
...
// calculate the difference between two pdata fields (note - slow!, not
// parallelized)
PYTHON() Real pdataMaxDiff(ParticleDataBase *a, ParticleDataBase *b)
{
	double maxVal = 0.;
	// debMsg(" PD "<< a->getType()<<"  as"<<a->getSizeSlow()<<"
	// bs"<<b->getSizeSlow() , 1);
	assertMsg(a->getType() == b->getType(),
	          "pdataMaxDiff problem - different pdata types!");
	assertMsg(a->getSizeSlow() == b->getSizeSlow(),
	          "pdataMaxDiff problem - different pdata sizes!");
	...
	if (a->getType() & ParticleDataBase::TypeReal) {
		...
		errMsg(
		    "pdataMaxDiff: Grid Type is not supported (only Real, Vec3, int)");
	}
...
//*****************************************************************************
// helper functions for volume fractions (which are needed for second order
// obstacle boundaries)

KERNEL() void kninitVortexVelocity(Grid<Real> &phiObs, MACGrid &vel,
                                   const Vec3 &center, const Real &radius)
{
...
PYTHON() void initVortexVelocity(Grid<Real> &phiObs, MACGrid &vel,
                                 const Vec3 &center, const Real &radius)
{
...
KERNEL(bnd = 1)
void KnUpdateFractions(FlagGrid &flags, Grid<Real> &phiObs, MACGrid &fractions,
                       const int &boundaryWidth)
{
	...
	if (phiObs.is3D()) {
		fractions(i, j, k).z =
		    calcFraction(phiObs(i, j, k), phiObs(i, j, k - 1));
	}
	...
	if (i <= w + 1) { // min x
		if ((flags.isInflow(i - 1, j, k)) || (flags.isOutflow(i - 1, j, k)) ||
			...
			if (flags.is3D()) fractions(i, j, k).z = 1.;
		}
	}
	if (i >= flags.getSizeX() - w - 2) { // max x
		if ((flags.isInflow(i + 1, j, k)) || (flags.isOutflow(i + 1, j, k)) ||
			...
			if (flags.is3D()) fractions(i + 1, j, k).z = 1.;
		}
	}
	...
	if (j >= flags.getSizeY() - w - 2) { // max y
		if ((flags.isInflow(i, j + 1, k)) || (flags.isOutflow(i, j + 1, k)) ||
			...
			if (flags.is3D()) fractions(i, j + 1, k).z = 1.;
		}
	}
	...
	if (flags.is3D()) {
		if (k <= w + 1) { // min z
			if ((flags.isInflow(i, j, k - 1)) ||
			    (flags.isOutflow(i, j, k - 1)) || (flags.isOpen(i, j, k - 1))) {
				fractions(i, j, k).x = fractions(i, j, k).y = 1.;
				if (flags.is3D()) fractions(i, j, k).z = 1.;
			}
		}
		if (j >= flags.getSizeZ() - w - 2) { // max z
			if ((flags.isInflow(i, j, k + 1)) ||
				...
				if (flags.is3D()) fractions(i, j, k + 1).z = 1.;
			}
		}
	}
}

PYTHON() void updateFractions(FlagGrid &flags, Grid<Real> &phiObs,
                              MACGrid &fractions, const int &boundaryWidth = 0)
{
...
void KnUpdateFlags(FlagGrid &flags, MACGrid &fractions, Grid<Real> &phiObs)
{
	...
	if (test == 0. && phiObs(i, j, k) < 0.)
		flags(i, j, k) = FlagGrid::TypeObstacle;
	else
		flags(i, j, k) = FlagGrid::TypeEmpty;
}

PYTHON() void setObstacleFlags(FlagGrid &flags, MACGrid &fractions,
                               Grid<Real> &phiObs)
{
