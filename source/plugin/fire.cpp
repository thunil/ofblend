...
void KnProcessBurn(Grid<Real> &fuel, Grid<Real> &density, Grid<Real> &react,
                   ...
                   Real ignitionTemp, Real maxTemp, Real dt,
                   Vec3 flameSmokeColor)
{
	...
	if (fuel(i, j, k) < 0.0f) fuel(i, j, k) = 0.0f;
	...
	smokeEmit =
	    (smokeEmit + 0.5f) * (origFuel - fuel(i, j, k)) * 0.1f * flameSmoke;
	...
	if (smokeEmit > VECTOR_EPSILON) {
		...
		if (red)
			(*red)(i, j, k) =
			    ((*red)(i, j, k) + flameSmokeColor.x * smokeEmit) * smokeFactor;
		if (green)
			(*green)(i, j, k) =
			    ((*green)(i, j, k) + flameSmokeColor.y * smokeEmit) *
			    smokeFactor;
		if (blue)
			(*blue)(i, j, k) =
			    ((*blue)(i, j, k) + flameSmokeColor.z * smokeEmit) *
			    smokeFactor;
	}
}

PYTHON() void processBurn(Grid<Real> &fuel, Grid<Real> &density,
                          Grid<Real> &react, Grid<Real> *red = NULL,
                          Grid<Real> *green = NULL, Grid<Real> *blue = NULL,
