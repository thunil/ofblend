...
//! note - this is a simplified version , sampleLevelsetWithParticles has more
//functionality
PYTHON() void sampleFlagsWithParticles(FlagGrid &flags,
			...
			for (int dk = 0; dk < (is3D ? discretization : 1); dk++)
				for (int dj = 0; dj < discretization; dj++)
					for (int di = 0; di < discretization; di++) {
						Vec3 subpos =
						    pos + disp * Vec3(0.5 + di, 0.5 + dj, 0.5 + dk);
						subpos +=
						    jlen * (Vec3(1, 1, 1) - 2.0 * mRand.getVec3());
...
//! and skipEmpty for a continuous inflow (in the latter case, only empty cells
//will
//! be re-filled once they empty when calling sampleLevelsetWithParticles during
//! the main loop).
PYTHON() void sampleLevelsetWithParticles(LevelsetGrid &phi, FlagGrid &flags,
                                          BasicParticleSystem &parts,
                                          int discretization, Real randomness,
                                          bool reset = false,
...
KERNEL() void knClearFluidFLags(FlagGrid &flags, int dummy = 0)
{
	if (flags.isFluid(i, j, k)) {
		flags(i, j, k) =
		    (flags(i, j, k) | FlagGrid::TypeEmpty) & ~FlagGrid::TypeFluid;
	}
}
KERNEL(bnd = 1)
void knSetNbObstacle(FlagGrid &nflags, const FlagGrid &flags,
                     const Grid<Real> &phiObs)
{
	...
	if (flags.isEmpty(i, j, k)) {
		...
		if (set)
			nflags(i, j, k) =
			    (flags(i, j, k) | FlagGrid::TypeFluid) & ~FlagGrid::TypeEmpty;
	}
}
PYTHON() void markFluidCells(BasicParticleSystem &parts, FlagGrid &flags,
                             Grid<Real> *phiObs = NULL)
{
	...
	// special for second order obstacle BCs, check empty cells in boundary
	// region
	if (phiObs) {
		FlagGrid tmp(flags);
...
//! helper to calculate particle radius factor to cover the diagonal of a cell
//in 2d/3d
...
PYTHON() void adjustNumber(BasicParticleSystem &parts, MACGrid &vel,
                           FlagGrid &flags, int minParticles, int maxParticles,
                           LevelsetGrid &phi, Real radiusFactor = 1.,
                           Real narrowBand = -1., Grid<Real> *exclude = NULL)
{
	...
	for (IndexInt idx = 0; idx < (int)parts.size(); idx++) {
		if (parts.isActive(idx)) {
			...
			// dont delete particles in non fluid cells here, the particles are
			// "always right"
...
// simple and slow helper conversion to show contents of int grids like a real
// grid in the ui
// (use eg to quickly display contents of the particle-index grid)
PYTHON() void debugIntToReal(Grid<int> &source, Grid<Real> &dest,
                             Real factor = 1.)
{
...
// the particles in a cell i,j,k are particles[index(i,j,k)] to
// particles[index(i+1,j,k)-1]
// (ie,  particles[index(i+1,j,k)] alreadu belongs to cell i+1,j,k)
PYTHON() void gridParticleIndex(BasicParticleSystem &parts,
                                ParticleIndexSystem &indexSys, FlagGrid &flags,
                                Grid<int> &index, Grid<int> *counter = NULL)
{
...
KERNEL()
void ComputeUnionLevelsetPindex(Grid<int> &index, BasicParticleSystem &parts,
	...
	for (int zj = k - rZ; zj <= k + rZ; zj++)
		for (int yj = j - r; yj <= j + r; yj++)
			for (int xj = i - r; xj <= i + r; xj++) {
				...
				// note, for the particle indices in indexSys the access is
				// periodic (ie, dont skip for eg inBounds(sx,10,10)
...
PYTHON() void unionParticleLevelset(BasicParticleSystem &parts,
                                    ParticleIndexSystem &indexSys,
                                    FlagGrid &flags, Grid<int> &index,
                                    LevelsetGrid &phi, Real radiusFactor = 1.)
{
...
KERNEL()
void ComputeAveragedLevelsetWeight(BasicParticleSystem &parts, Grid<int> &index,
                                   ParticleIndexSystem &indexSys,
                                   LevelsetGrid &phi, Real radius = 1.)
{
	const Vec3 gridPos = Vec3(i, j, k) + Vec3(0.5); // shifted by half cell
...
{
	T val = me(i, j, k) + me(i + 1, j, k) + me(i - 1, j, k) + me(i, j + 1, k) +
	        me(i, j - 1, k);
	if (me.is3D()) {
...
PYTHON() void averagedParticleLevelset(BasicParticleSystem &parts,
                                       ParticleIndexSystem &indexSys,
                                       FlagGrid &flags, Grid<int> &index,
                                       LevelsetGrid &phi,
                                       Real radiusFactor = 1., int smoothen = 1,
                                       int smoothenNeg = 1)
{
	// use half a cell diagonal as base radius
	const Real radius = 0.5 * calculateRadiusFactor(phi, radiusFactor);
...
KERNEL(pts)
void knPushOutofObs(BasicParticleSystem &parts, FlagGrid &flags,
                    Grid<Real> &phiObs, Real shift = 0.05, Real thresh = 0.)
{
...
PYTHON() void pushOutofObs(BasicParticleSystem &parts, FlagGrid &flags,
                           Grid<Real> &phiObs, Real shift = 0.05,
                           Real thresh = 0.)
{
...
void knSafeDivReal(Grid<T> &me, const Grid<Real> &other,
                   Real cutoff = VECTOR_EPSILON)
{
	if (other[idx] < cutoff) {
		me[idx] = 0.;
	} else {
...
KERNEL(pts, single)
void knMapLinearVec3ToMACGrid(BasicParticleSystem &p, FlagGrid &flags,
                              MACGrid &vel, Grid<Vec3> &tmp,
...
void knMapLinear(BasicParticleSystem &p, FlagGrid &flags, Grid<T> &target,
                 Grid<Real> &gtmp, ParticleDataImpl<T> &psource)
{
	unusedParameter(flags);
	if (!p.isActive(idx)) return;
...
PYTHON() void mapPartsToGrid(FlagGrid &flags, Grid<Real> &target,
                             BasicParticleSystem &parts,
                             ParticleDataImpl<Real> &source)
{
	mapLinearRealHelper<Real>(flags, target, parts, source);
}
PYTHON() void mapPartsToGridVec3(FlagGrid &flags, Grid<Vec3> &target,
                                 BasicParticleSystem &parts,
                                 ParticleDataImpl<Vec3> &source)
{
	mapLinearRealHelper<Vec3>(flags, target, parts, source);
}
// integers need "max" mode, not yet implemented
// PYTHON() void mapPartsToGridInt ( FlagGrid& flags, Grid<int >& target ,
// BasicParticleSystem& parts , ParticleDataImpl<int >& source ) {
//	mapLinearRealHelper<int >(flags,target,parts,source);
//}

KERNEL(pts) template <class T>
void knMapFromGrid(BasicParticleSystem &p, Grid<T> &gsrc,
                   ParticleDataImpl<T> &target)
{
	if (!p.isActive(idx)) return;
	target[idx] = gsrc.getInterpolated(p[idx].pos);
}
PYTHON() void mapGridToParts(Grid<Real> &source, BasicParticleSystem &parts,
                             ParticleDataImpl<Real> &target)
{
	knMapFromGrid<Real>(parts, source, target);
}
PYTHON() void mapGridToPartsVec3(Grid<Vec3> &source, BasicParticleSystem &parts,
...
KERNEL(pts)
void knMapLinearMACGridToVec3_PIC(BasicParticleSystem &p, FlagGrid &flags,
                                  MACGrid &vel, ParticleDataImpl<Vec3> &pvel)
{
	if (!p.isActive(idx)) return;
	// pure PIC
...
KERNEL(pts)
void knMapLinearMACGridToVec3_FLIP(BasicParticleSystem &p, FlagGrid &flags,
                                   MACGrid &vel, MACGrid &oldVel,
                                   ParticleDataImpl<Vec3> &pvel, Real flipRatio)
{
	if (!p.isActive(idx)) return;
...
PYTHON() void flipVelocityUpdate(FlagGrid &flags, MACGrid &vel, MACGrid &velOld,
                                 ...
                                 Real flipRatio)
{
	knMapLinearMACGridToVec3_FLIP(parts, flags, vel, velOld, partVel,
	                              flipRatio);
}
...
void knCombineVels(MACGrid &vel, Grid<Vec3> &w, MACGrid &combineVel,
                   LevelsetGrid *phi, Real narrowBand, Real thresh)
{
...
PYTHON() void combineGridVel(MACGrid &vel, Grid<Vec3> &weight,
                             MACGrid &combineVel, LevelsetGrid *phi = NULL,
