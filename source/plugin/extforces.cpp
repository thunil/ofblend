...
KERNEL(bnd = 1) void KnAddForceField(FlagGrid &flags, MACGrid &vel,
                                     Grid<Vec3> &force)
{
	...
	if (vel.is3D() && (flags.isFluid(i, j, k - 1) ||
	                   (curFluid && flags.isEmpty(i, j, k - 1))))
...
KERNEL(bnd = 1) void KnAddBuoyancy(FlagGrid &flags, Grid<Real> &factor,
                                   MACGrid &vel, Vec3 strength)
{
	if (!flags.isFluid(i, j, k)) return;
	if (flags.isFluid(i - 1, j, k))
		vel(i, j, k).x +=
		    (0.5 * strength.x) * (factor(i, j, k) + factor(i - 1, j, k));
	if (flags.isFluid(i, j - 1, k))
		vel(i, j, k).y +=
		    (0.5 * strength.y) * (factor(i, j, k) + factor(i, j - 1, k));
	if (vel.is3D() && flags.isFluid(i, j, k - 1))
		vel(i, j, k).z +=
		    (0.5 * strength.z) * (factor(i, j, k) + factor(i, j, k - 1));
}

//! add Buoyancy force based on fctor (e.g. smoke density)
PYTHON() void addBuoyancy(FlagGrid &flags, Grid<Real> &density, MACGrid &vel,
                          Vec3 gravity, Real coefficient = 1.)
{
	Vec3 f = -gravity * flags.getParent()->getDt() /
	         flags.getParent()->getDx() * coefficient;
...
inline void convertDescToVec(const string &desc, Vector3D<bool> &lo,
                             Vector3D<bool> &up)
{
	for (size_t i = 0; i < desc.size(); i++) {
		if (desc[i] == 'x')
			...
			errMsg("invalid character in boundary description string. Only "
			       "[xyzXYZ] allowed.");
	}
}

//! add empty and outflow flag to cells of open boundaries
PYTHON() void setOpenBound(FlagGrid &flags, int bWidth, string openBound = "",
                           int type = FlagGrid::TypeOutflow |
                                      FlagGrid::TypeEmpty)
{
	if (openBound == "") return;
	Vector3D<bool> lo, up;
	...
	FOR_IJK(flags)
	{
		bool loX =
		    lo.x &&
		    i <= bWidth; // a cell which belongs to the lower x open bound
		...
		bool innerI =
		    i > bWidth && i < flags.getSizeX() - bWidth - 1; // a cell which
		                                                     // does not belong
		                                                     // to the lower or
		                                                     // upper x bound
		bool innerJ = j > bWidth && j < flags.getSizeY() - bWidth - 1;

		// when setting boundaries to open: don't set shared part of wall to
		// empty if neighboring wall is not open
		if ((!flags.is3D()) && (loX || upX || loY || upY)) {
			if ((loX || upX || innerI) && (loY || upY || innerJ) &&
			    flags.isObstacle(i, j, k))
				flags(i, j, k) = type;
		} else {
			bool loZ =
			    lo.z &&
			    k <= bWidth; // a cell which belongs to the lower z open bound
			bool upZ =
			    up.z &&
			    k >= flags.getSizeZ() - bWidth -
			             1; // a cell which belongs to the upper z open bound
			bool innerK = k > bWidth &&
			              k < flags.getSizeZ() - bWidth - 1; // a cell which
			                                                 // does not belong
			                                                 // to the lower or
			                                                 // upper z bound
			if (loX || upX || loY || upY || loZ || upZ) {
				if ((loX || upX || innerI) && (loY || upY || innerJ) &&
				    (loZ || upZ || innerK) && flags.isObstacle(i, j, k))
					flags(i, j, k) = type;
			}
		}
	}
...
//! delete fluid and ensure empty flag in outflow cells, delete particles and
//density and set phi to 0.5
PYTHON() void resetOutflow(FlagGrid &flags, Grid<Real> *phi = 0,
                           BasicParticleSystem *parts = 0, Grid<Real> *real = 0,
                           Grid<int> *index = 0,
                           ParticleIndexSystem *indexSys = 0)
{
	// check if phi and parts -> pindex and gpi already created -> access
	// particles from cell index, avoid extra looping over particles
	if (parts && (!index || !indexSys)) {
		if (phi)
			debMsg("resetOpenBound for phi and particles, but missing index "
			       "and indexSys for enhanced particle access!",
			       1);
	...
	FOR_IJK(flags)
	{
		if (flags.isOutflow(i, j, k)) {
			flags(i, j, k) = (flags(i, j, k) | FlagGrid::TypeEmpty) &
			                 ~FlagGrid::TypeFluid; // make sure there is not
			                                       // fluid flag set and to
			...
			if (parts && index && indexSys) {
				...
				for (int p = pStart; p < pEnd; ++p) {
					...
					if (parts->isActive(psrc) &&
					    flags.isInBounds(parts->getPos(psrc)))
						parts->kill(psrc);
				}
			}
...
PYTHON() void setInflowBcs(MACGrid &vel, string dir, Vec3 value)
{
	for (size_t i = 0; i < dir.size(); i++) {
		if (dir[i] >= 'x' && dir[i] <= 'z') {
			...
			errMsg("invalid character in direction string. Only [xyzXYZ] "
			       "allowed.");
	}
}
...
KERNEL() void KnSetWallBcsFrac(FlagGrid &flags, MACGrid &vel,
                               MACGrid &velTarget, Grid<Real> *phiObs,
                               const int &boundaryWidth = 0)
{
	...
	if (flags.isInBounds(Vec3i(i, j, k), 1)) {
		...
		if (curObs | flags.isObstacle(i - 1, j, k)) {
			Vec3 dphi(0., 0., 0.);
			const Real tmp1 =
			    (phiObs->get(i, j, k) + phiObs->get(i - 1, j, k)) * .5;
			Real tmp2 =
			    (phiObs->get(i, j + 1, k) + phiObs->get(i - 1, j + 1, k)) * .5;
			...
			if (phiObs->is3D()) {
				tmp2 =
				    (phiObs->get(i, j, k + 1) + phiObs->get(i - 1, j, k + 1)) *
				    .5;
				phi1 = (tmp1 + tmp2) * .5;
				tmp2 =
				    (phiObs->get(i, j, k - 1) + phiObs->get(i - 1, j, k - 1)) *
		...
		if (curObs | flags.isObstacle(i, j - 1, k)) {
			Vec3 dphi(0., 0., 0.);
			const Real tmp1 =
			    (phiObs->get(i, j, k) + phiObs->get(i, j - 1, k)) * .5;
			Real tmp2 =
			    (phiObs->get(i + 1, j, k) + phiObs->get(i + 1, j - 1, k)) * .5;
			Real phi1 = (tmp1 + tmp2) * .5;
			tmp2 =
			    (phiObs->get(i - 1, j, k) + phiObs->get(i - 1, j - 1, k)) * .5;
			Real phi2 = (tmp1 + tmp2) * .5;
			...
			if (phiObs->is3D()) {
				tmp2 =
				    (phiObs->get(i, j, k + 1) + phiObs->get(i, j - 1, k + 1)) *
				    .5;
				phi1 = (tmp1 + tmp2) * .5;
				tmp2 =
				    (phiObs->get(i, j, k - 1) + phiObs->get(i, j - 1, k - 1)) *
		...
		if (phiObs->is3D() && (curObs | flags.isObstacle(i, j, k - 1))) {
			Vec3 dphi(0., 0., 0.);
			const Real tmp1 =
			    (phiObs->get(i, j, k) + phiObs->get(i, j, k - 1)) * .5;

			Real tmp2;
			tmp2 =
			    (phiObs->get(i + 1, j, k) + phiObs->get(i + 1, j, k - 1)) * .5;
			Real phi1 = (tmp1 + tmp2) * .5;
			tmp2 =
			    (phiObs->get(i - 1, j, k) + phiObs->get(i - 1, j, k - 1)) * .5;
			Real phi2 = (tmp1 + tmp2) * .5;
			...
			tmp2 =
			    (phiObs->get(i, j + 1, k) + phiObs->get(i, j + 1, k - 1)) * .5;
			phi1 = (tmp1 + tmp2) * .5;
			tmp2 =
			    (phiObs->get(i, j - 1, k) + phiObs->get(i, j - 1, k - 1)) * .5;
			phi2 = (tmp1 + tmp2) * .5;
...
PYTHON() void setWallBcs(FlagGrid &flags, MACGrid &vel, MACGrid *fractions = 0,
...
KERNEL(bnd = 1) void KnConfForce(Grid<Vec3> &force, const Grid<Real> &grid,
                                 const Grid<Vec3> &curl, Real str)
{
