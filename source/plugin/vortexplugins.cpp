...
KERNEL(idx)
void KnAcceleration(MACGrid &a, const MACGrid &v1, const MACGrid &v0,
                    const Real idt)
{
                              ...
                              Real mult = 1.0)
{
	...
	cout << "vorticity: max " << maxV << " / mean " << meanV / mesh.numTris()
	     << endl;
}

PYTHON() void smoothVorticity(VortexSheetMesh &mesh, int iter = 1,
                              Real sigma = 0.2, Real alpha = 0.8)
{
...
PYTHON() void VPseedK41(VortexParticleSystem &system, Shape *shape,
                        Real strength = 0, Real sigma0 = 0.2, Real sigma1 = 1.0,
                        Real probability = 1.0, Real N = 3.0)
{
	...
	FOR_IJK(temp)
	{
		if (shape->isInsideGrid(i, j, k)) {
			if (rand.getReal() < probability * dt) {
				...
				Vec3 posUpd(i + rand.getReal(), j + rand.getReal(),
				            k + rand.getReal());
				normalize(randDir);
				Vec3 vorticity = randDir * strength *
				                 pow((Real)sigma, (Real)(-10. / 6. + N / 2.0));
...
PYTHON() void VICintegration(VortexSheetMesh &mesh, Real sigma, Grid<Vec3> &vel,
                             FlagGrid &flags, Grid<Vec3> *vorticity = NULL,
                             Real cgMaxIterFac = 1.5, Real cgAccuracy = 1e-3,
                             Real scale = 0.01, int precondition = 0)
{
		...
		for (int i = -sgi; i < sgi; i++) {
			...
			for (int j = -sgi; j < sgi; j++) {
				...
				for (int k = -sgi; k < sgi; k++) {
					...
					Vec3 d = pos - Vec3(i + 0.5 + floor(pos.x),
					                    j + 0.5 + floor(pos.y),
					                    k + 0.5 + floor(pos.z));
					Real dl = norm(d);
	...
	for (int c = 0; c < 3; c++) {
		...
		gcg->setPreconditioner((GridCgInterface::PreconditionType)precondition,
		                       &pca0, &pca1, &pca2, &pca3);
		...
		debMsg("VICintegration CG iterations:" << gcg->getIterations()
		                                       << ", res:" << gcg->getSigma(),
		       1);
		delete gcg;
...
//! Obtain density field from levelset with linear gradient of size sigma over
//the interface
PYTHON() void densityFromLevelset(LevelsetGrid &phi, Grid<Real> &density,
                                  Real value = 1.0, Real sigma = 1.0)
{
	FOR_IJK(phi)
	{
		// remove boundary
		if (i < 2 || j < 2 || k < 2 || i >= phi.getSizeX() - 2 ||
		    j >= phi.getSizeY() - 2 || k >= phi.getSizeZ() - 2)
			density(i, j, k) = 0;
			...
			density(i, j, k) =
			    clamp((Real)(0.5 * value / sigma * (1.0 - phi(i, j, k))),
			          (Real)0.0, value);
	}
}
