 ...
 *
 *  Here you can find the auto-generated documentation of the mantaflow
 *framework.
 *
 *  One of the most useful parts is probably the list of python functions,
 *classes and the C++ kernels.
 *  Those can be found found in the ''Modules'' section. For python functions
 *the parameters (e.g. Grids, Real or int values)
 *  are automatically transferred to and from python. Thus, this list is a good
 *reference how to call the functions used in the
 *  example scenes.
 *
 */

// Define plugin documentation group
// all kernels, plugin functions and classes will automatically be added to this
// group
...
void MuTime::get()
{
	...
	time =
	    (INT)(((double)liLastTime.QuadPart / liTimerFrequency.QuadPart) * 1000);
#else
	struct timeval tv;
	struct timezone tz;
	tz.tz_minuteswest = 0;
...
ostream &operator<<(ostream &os, const MuTime &t)
{
	...
	unsigned long ss =
	    (unsigned long)(((double)t.time / 1000.0) - ((double)ms * 60.0));
...
// print info about this mantaflow build, used eg by printBuildInfo in
// fluidsolver.cpp
