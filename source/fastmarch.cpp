...
FastMarch<COMP, TDIR>::FastMarch(FlagGrid &flags, Grid<int> &fmFlags,
                                 Grid<Real> &levelset, Real maxTime,
                                 MACGrid *velTransport)
    : mLevelset(levelset), mFlags(flags), mFmFlags(fmFlags)
{
	if (velTransport) mVelTransport.initMarching(velTransport, &flags);
...
template <class COMP, int TDIR>
template <int C>
Real FastMarch<COMP, TDIR>::calcWeights(int &okcnt, int &invcnt, Real *v,
                                        const Vec3i &idx)
{
	Real val = 0.;
...
void FastMarch<COMP, TDIR>::addToList(const Vec3i &p, const Vec3i &src)
{
	...
	// discard by source time now , TODO do instead before calling all
	// addtolists?
	...
	//     entries, but the earlier will be handled earlier, the second one will
	//     skip to the FlagInited check above
	if (!found) {
...
KERNEL(bnd = 1)
void knExtrapolateMACSimple(MACGrid &vel, int distance, Grid<int> &tmp,
                            const int d, const int c)
{
	static const Vec3i nb[6] = { Vec3i(1, 0, 0), Vec3i(-1, 0, 0),
...
KERNEL(bnd = 1)
void knUnprojectNormalComp(FlagGrid &flags, MACGrid &vel, Grid<Real> &phi,
                           Real maxDist)
{
...
// into obstacle is a special mode for second order obstable boundaries
// (extrapolating
// only fluid velocities, not those at obstacles)
PYTHON() void extrapolateMACSimple(FlagGrid &flags, MACGrid &vel,
                                   int distance = 4,
...
KERNEL(bnd = 1)
void knExtrapolateMACFromWeight(MACGrid &vel, Grid<Vec3> &weight, int distance,
                                const int d, const int c)
{
	static const Vec3i nb[6] = { Vec3i(1, 0, 0), Vec3i(-1, 0, 0),
		                         Vec3i(0, 1, 0), Vec3i(0, -1, 0),
...
// same as extrapolateMACSimple, but uses weight vec3 grid instead of flags to
// check
// for valid values (to be used in combination with mapPartsToMAC)
// note - the weight grid values are destroyed! the function is necessary due to
// discrepancies
...
PYTHON() void extrapolateMACFromWeight(MACGrid &vel, Grid<Vec3> &weight,
                                       int distance = 2)
{
	const int dim = (vel.is3D() ? 3 : 2);
	...
	for (int c = 0; c < dim; ++c) {
		...
		// reset weight values to 0 (uninitialized), and 1 (initialized inner
		// values)
		FOR_IJK_BND(vel, 1)
		{
			Vec3i p(i, j, k);
...
void knExtrapolateLsSimple(Grid<S> &val, int distance, Grid<int> &tmp,
...
PYTHON() void extrapolateLsSimple(Grid<Real> &phi, int distance = 4,
                                  bool inside = false)
{
...
PYTHON() void extrapolateVec3Simple(Grid<Vec3> &vel, Grid<Real> &phi,
                                    int distance = 4)
{
