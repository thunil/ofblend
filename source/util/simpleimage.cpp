...
bool SimpleImage::writePpm(std::string filename, int minx, int miny, int maxx,
                           int maxy, bool invertXY)
{
	int w = maxx - minx;
	int h = maxy - miny;
	...
	if (w <= 0 || h <= 0 || w > mSize[0] || h > mSize[1]) {
		errMsg("SimpleImage::WritePPM Invalid rect: w="
		       << w << ", h=" << h << ", size=" << mSize[0] << "," << mSize[1]
		       << " min/max: " << minx << "," << miny << " to " << maxx << ","
		       << maxy << ", resetting... ");
		minx = miny = 0;
		maxx = mSize[0] - 1;
		maxy = mSize[1] - 1;
	...
	if (fp == NULL) {
		errMsg("SimpleImage::WritePPM Unable to open '" << filename
		                                                << "' for writing");
		return false;
	}
	fprintf(fp, "P6\n%d %d\n255\n", w, h);
	...
	for (int j = maxy - 1; j >= miny; j--)
		for (int i = minx; i < maxx; i++) {
			...
			// col[1] = col[2] = col[0];
			// if (fwrite(col,1,3, fp) != 3) errMsg("SimpleImage::writePpm
			// fwrite failed");
			fwrite(col, 1, 3, fp);
			pixCnt++;
...
bool SimpleImage::initFromPpm(std::string filename)
{
	...
	if (!fp) {
		if (mAbortOnError)
			debMsg("SimpleImage Error - unable to open file '"
	...
	if (intsFound == 1) {
		...
		if (sscanf(line, "%d", &windH) != 1) {
			if (mAbortOnError)
				errMsg("initFromPpm Ppm dimensions not found!" << windW << ","
				                                               << windH);
			return 0;
		} else {
			// ok, found 2 lines
		...
		if (mAbortOnError)
			errMsg("initFromPpm Ppm dimensions not found at all!"
			       << windW << "," << windH);
		return 0;
	}
	...
	// Read in maximum value (ignore) , could be scanned with sscanf as well,
	// but this should be 255...
	// 3rd line
	if (fgets(line, MAXLINE, fp) == NULL) {
		if (mAbortOnError) errMsg("SimpleImage::initFromPpm fgets failed");
		return 0;
...
static void gridPrecompLight(Grid<Real> &density, Grid<Real> &L,
...
static inline void shadeCell(Vec3 &dst, int shadeMode, Real src, Real light,
                             int depthPos, Real depthInv)
{
	switch (shadeMode) {
                ...
                Real scale = 1.)
{
	...
	if (val.is3D()) {
		...
		{
			Vec3i idx(i, j, k);
			shadeCell(img(s[0] + s[2] + i, k), shadeMode, val(idx), L(idx), j,
			          si[1]);
		}
