...
template <class S> class Vector3D
{
  public:
	...
	//! Negation operator
	inline Vector3D<S> operator-() const { return Vector3D<S>(-x, -y, -z); }
	...
	inline bool empty() { return x == 0 && y == 0 && z == 0; }
...
{
	Vector3D<S> cp(((t.y * v.z) - (t.z * v.y)), ((t.z * v.x) - (t.x * v.z)),
	               ((t.x * v.y) - (t.y * v.x)));
	return cp;
}
...
template <class S>
inline const Vector3D<S> &projectNormalTo(const Vector3D<S> &v,
                                          const Vector3D<S> &n)
{
	S sprod = dot(v, n);
	return v - n * dot(v, n);
...
*/
template <class S>
inline Vector3D<S> refractVector(const Vector3D<S> &t,
                                 const Vector3D<S> &normal, S nt, S nair,
                                 int &refRefl)
{
...
{
	char buf[256];
	snprintf(buf, 256, "[%+4.6f,%+4.6f,%+4.6f]", (double)(*this)[0],
	         (double)(*this)[1], (double)(*this)[2]);
	// for debugging, optionally increase precision:
	// snprintf ( buf,256,"[%+4.16f,%+4.16f,%+4.16f]", ( double ) ( *this ) [0],
	// ( double ) ( *this ) [1], ( double ) ( *this ) [2] );
...
template <class T> inline Vec3 toVec3(T v) { return Vec3(v[0], v[1], v[2]); }
...
{
	return Vec3(clamp(a.x, b.x, c.x), clamp(a.y, b.y, c.y),
	            clamp(a.z, b.z, c.z));
}
template <> inline Vec3 safeDivide<Vec3>(const Vec3 &a, const Vec3 &b)
{
	return Vec3(safeDivide(a.x, b.x), safeDivide(a.y, b.y),
	            safeDivide(a.z, b.z));
}
