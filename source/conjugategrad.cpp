...
void InitPreconditionIncompCholesky(FlagGrid &flags, Grid<Real> &A0,
                                    Grid<Real> &Ai, Grid<Real> &Aj,
                                    Grid<Real> &Ak, Grid<Real> &orgA0,
                                    Grid<Real> &orgAi, Grid<Real> &orgAj,
                                    Grid<Real> &orgAk)
{
...
void InitPreconditionModifiedIncompCholesky2(FlagGrid &flags,
                                             Grid<Real> &Aprecond,
                                             Grid<Real> &A0, Grid<Real> &Ai,
                                             Grid<Real> &Aj, Grid<Real> &Ak)
{
	// compute IC according to Golub and Van Loan
	Aprecond.clear();
	...
	FOR_IJK(flags)
	{
		...
		e -= tau * (Ai(i - 1, j, k) * (Aj(i - 1, j, k) + Ak(i - 1, j, k)) *
		                square(Aprecond(i - 1, j, k)) +
		            Aj(i, j - 1, k) * (Ai(i, j - 1, k) + Ak(i, j - 1, k)) *
		                square(Aprecond(i, j - 1, k)) +
		            Ak(i, j, k - 1) * (Ai(i, j, k - 1) + Aj(i, j, k - 1)) *
		                square(Aprecond(i, j, k - 1)) +
		            0.);

		// stability cutoff
		if (e < sigma * A0(i, j, k)) e = A0(i, j, k);
...
//! Apply WT-style ICP
void ApplyPreconditionIncompCholesky(Grid<Real> &dst, Grid<Real> &Var1,
                                     FlagGrid &flags, Grid<Real> &A0,
                                     Grid<Real> &Ai, Grid<Real> &Aj,
                                     Grid<Real> &Ak, Grid<Real> &orgA0,
                                     Grid<Real> &orgAi, Grid<Real> &orgAj,
                                     Grid<Real> &orgAk)
{
	...
	FOR_IJK(dst)
	{
		if (!flags.isFluid(i, j, k)) continue;
		dst(i, j, k) =
		    A0(i, j, k) * (Var1(i, j, k) - dst(i - 1, j, k) * Ai(i - 1, j, k) -
	...
	FOR_IJK_REVERSE(dst)
	{
		...
		dst[idx] =
		    A0[idx] * (dst[idx] - dst(i + 1, j, k) * Ai[idx] -
		               dst(i, j + 1, k) * Aj[idx] - dst(i, j, k + 1) * Ak[idx]);
	}
}

//! Apply Bridson-style mICP
void ApplyPreconditionModifiedIncompCholesky2(Grid<Real> &dst, Grid<Real> &Var1,
                                              FlagGrid &flags,
                                              Grid<Real> &Aprecond,
                                              Grid<Real> &A0, Grid<Real> &Ai,
                                              Grid<Real> &Aj, Grid<Real> &Ak)
{
	...
	FOR_IJK_REVERSE(dst)
	{
		...
		dst[idx] = p * (dst[idx] - dst(i + 1, j, k) * Ai[idx] * p -
...
KERNEL(idx, reduce = +)
    returns(double result = 0.0) double GridDotProduct(const Grid<Real> &a,
                                                       const Grid<Real> &b)
{
	result += (a[idx] * b[idx]);
};
...
KERNEL(idx, reduce = +)
    returns(double sigma = 0) double InitSigma(FlagGrid &flags, Grid<Real> &dst,
                                               Grid<Real> &rhs,
                                               Grid<Real> &temp)
{
...
GridCg<APPLYMAT>::GridCg(Grid<Real> &dst, Grid<Real> &rhs, Grid<Real> &residual,
                         Grid<Real> &search, FlagGrid &flags, Grid<Real> &tmp,
                         Grid<Real> *pA0, Grid<Real> *pAi, Grid<Real> *pAj,
                         Grid<Real> *pAk)
    : GridCgInterface(), mInited(false), mIterations(0), mDst(dst), mRhs(rhs),
      mResidual(residual), mSearch(search), mFlags(flags), mTmp(tmp), mpA0(pA0),
      mpAi(pAi), mpAj(pAj), mpAk(pAk), mPcMethod(PC_None), mpPCA0(pA0),
      mpPCAi(pAi), mpPCAj(pAj), mpPCAk(pAk), mSigma(0.),
      mAccuracy(VECTOR_EPSILON), mResNorm(1e20)
{
	dst.clear();
	residual.clear();
	search.clear();
...
template <class APPLYMAT> void GridCg<APPLYMAT>::doInit()
{
	...
	if (mPcMethod == PC_ICP) {
		...
		InitPreconditionIncompCholesky(mFlags, *mpPCA0, *mpPCAi, *mpPCAj,
		                               *mpPCAk, *mpA0, *mpAi, *mpAj, *mpAk);
		ApplyPreconditionIncompCholesky(mTmp, mResidual, mFlags, *mpPCA0,
		                                *mpPCAi, *mpPCAj, *mpPCAk, *mpA0, *mpAi,
		                                *mpAj, *mpAk);
	} else if (mPcMethod == PC_mICP) {
		assertMsg(mDst.is3D(), "mICP only supports 3D grids so far");
		InitPreconditionModifiedIncompCholesky2(mFlags, *mpPCA0, *mpA0, *mpAi,
		                                        *mpAj, *mpAk);
...
template <class APPLYMAT> bool GridCg<APPLYMAT>::iterate()
{
	...
	gridScaledAdd<Real, Real>(mResidual, mTmp,
	                          -alpha); // residual += tmp * -alpha
	...
	// use the l2 norm of the residual for convergence check? (usually max norm
	...
	// if(mIterations % 10 == 9) debMsg("GridCg::Iteration i="<<mIterations<<",
	// resNorm="<<mResNorm<<" accuracy="<<mAccuracy, 1);
	...
	debMsg("PB-Cg::iter i=" << mIterations << " sigmaNew=" << sigmaNew
	                        << " sigmaLast=" << mSigma << " alpha=" << alpha
	                        << " beta=" << beta << " ",
	       CG_DEBUGLEVEL);
	mSigma = sigmaNew;
...
template <class APPLYMAT>
void GridCg<APPLYMAT>::setPreconditioner(PreconditionType method,
                                         Grid<Real> *A0, Grid<Real> *Ai,
                                         Grid<Real> *Aj, Grid<Real> *Ak)
{
	mPcMethod = method;
	if ((!A0->is3D()) && (mPcMethod != PC_None)) {
		if (gPrint2dWarning) {
			debMsg(
			    "Pre-conditioning only supported in 3D for now, disabling it.",
			    1);
