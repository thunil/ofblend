...
bool Shape::isInside(const Vec3 &pos) const { return false; }
...
KERNEL() template <class T>
void ApplyShapeToGridSmooth(Grid<T> *grid, Grid<Real> &phi, Real sigma,
                            Real shift, T value, FlagGrid *respectFlags)
{
	if (respectFlags && respectFlags->isObstacle(i, j, k)) return;
...
#if NOPYTHON != 1
	if (grid->getType() & GridBase::TypeInt)
		ApplyShapeToGridSmooth<int>((Grid<int> *)grid, phi, sigma, shift,
		                            _args.get<int>("value"), respectFlags);
	else if (grid->getType() & GridBase::TypeReal)
		ApplyShapeToGridSmooth<Real>((Grid<Real> *)grid, phi, sigma, shift,
		                             _args.get<Real>("value"), respectFlags);
	else if (grid->getType() & GridBase::TypeVec3)
		ApplyShapeToGridSmooth<Vec3>((Grid<Vec3> *)grid, phi, sigma, shift,
		                             _args.get<Vec3>("value"), respectFlags);
	else
...
{
	const int quadidx[24] = { 0, 4, 6, 2, 3, 7, 5, 1, 0, 1, 5, 4,
...
void Box::generateLevelset(Grid<Real> &phi) { BoxSDF(phi, mP0, mP1); }
...
void Sphere::generateMesh(Mesh *mesh)
{
	...
	Vec3 p[6] = { Vec3(0, 1, 0),  Vec3(0, -1, 0), Vec3(-d, 0, -d),
		          Vec3(d, 0, -d), Vec3(d, 0, d),  Vec3(-d, 0, d) };
	...
	for (size_t i = 0; i < tris.size(); i++) {
		for (int t = 0; t < 3; t++) {
			...
			if (idx == (int)nodes.size()) nodes.push_back(p);
			tris[i].i[t] = idx;
		}
	}
	...
	for (size_t t = 0; t < tris.size(); t++)
		mesh->addTri(
		    Triangle(tris[t].i[0] + ni, tris[t].i[1] + ni, tris[t].i[2] + ni));
...
{
	phi(i, j, k) =
	    norm((Vec3(i + 0.5, j + 0.5, k + 0.5) - center) / scale) - radius;
}
...
KERNEL() void CylinderSDF(Grid<Real> &phi, Vec3 center, Real radius, Vec3 zaxis,
                          Real maxz)
{
...
Slope::Slope(FluidSolver *parent, Real anglexy, Real angleyz, Real origin,
             Vec3 gs)
    : Shape(parent), mAnglexy(anglexy), mAngleyz(angleyz), mOrigin(origin),
      mGs(gs)
{
	mType = TypeSlope;
}
...
bool Slope::isInside(const Vec3 &pos) const
{
	...
	return ((n.x * (double)pos.x + n.y * (double)pos.y + n.z * (double)pos.z -
	         mOrigin) /
	        fac) <= 0.;
}

KERNEL() void SlopeSDF(const Vec3 &n, Grid<Real> &phiObs, const Real &fac,
                       const Real &origin)
{

	phiObs(i, j, k) =
	    (n.x * (double)i + n.y * (double)j + n.z * (double)k - origin) * fac;
}
