...
void MainThread::run() { runScript(mArgs); }
...
void MainThread::send(int e) { emit sendToGui(e); }
...
void GuiThread::exitApp() { mApp.exit(1); }
...
void guiMain(int argc, char *argv[])
{
	...
	if (argc <= 1) {
		QString filename = QFileDialog::getOpenFileName(
		    0, "Open scene file", "", "Python scene files (*.py)");
	...
	QObject::connect(&worker, SIGNAL(sendToGui(int)), &gui,
	                 SLOT(sendEvent(int)));
	QObject::connect(gui.getWindow(), SIGNAL(wakeMain()), &worker,
	                 SLOT(wakeUp()));
	QObject::connect(gui.getWindow(), SIGNAL(killMain()), &worker,
	                 SLOT(killMe()));
...
void updateQtGui(bool full, int frame, float time, const string &curPlugin)
{
	...
	gMainThread->sendAndWait(full ? (int)MainWnd::EventFullUpdate
	                              : (int)MainWnd::EventStepUpdate);
