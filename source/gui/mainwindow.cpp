...
MainWnd::MainWnd()
    : QMainWindow(0), mPaused(true), mRequestPause(false), mRequestClose(false),
      mStep(0), mKbwScene(0), mKbwView(0), mKbwPixmap(0), mMenuBar(0)
{
	// Frame info label
	mInfo = new QLabel;
	setStep(0, 0.);
	...
	connect(mGlWidget,
	        SIGNAL(clickLine(QPoint, float, float, float, float, float, float)),
	        SLOT(clickLine(QPoint, float, float, float, float, float, float)));
	...
	mPainter.push_back(
	    new GridPainter<Real>((FlagGrid **)intPainter->getGridPtr(), this));
	...
	connect(this, SIGNAL(setBackgroundMesh(Mesh *)), ptr,
	        SLOT(setBackgroundMesh(Mesh *)));

	for (int i = 0; i < (int)mPainter.size(); i++) {
		connect(mGlWidget, SIGNAL(paintSub()), mPainter[i], SLOT(paint()));
		connect(mGlWidget, SIGNAL(painterEvent(int, int)), mPainter[i],
		        SLOT(doEvent(int, int)));
		connect(this, SIGNAL(painterEvent(int, int)), mPainter[i],
		        SLOT(doEvent(int, int)));
		connect(mPainter[i], SIGNAL(setViewport(const Vec3i &)), mGlWidget,
		        SLOT(setViewport(const Vec3i &)));
	...
	painterDock->setAllowedAreas(Qt::LeftDockWidgetArea |
...
void MainWnd::clickLine(QPoint pos, float p0, float p1, float p2, float q0,
                        float q1, float q2)
{
	string msg;
	...
	if (!msg.empty()) QToolTip::showText(pos, QString(msg.c_str()));
}
...
void MainWnd::setPauseStatus(bool v) { mPaused = v; }
...
void MainWnd::keyPressEvent(QKeyEvent *e)
{
	if (e->key() == Qt::Key_Escape) {
		...
		if (mRequestClose) {
			...
			emit painterEvent(mPaused ? Painter::UpdateFull
			                          : Painter::UpdateRequest);
			mGlWidget->updateGL();
		}
		...
		mGlWidget->keyPressEvent(
		    e); // let gl widget take care of keyboard shortcuts
		// QMainWindow::keyPressEvent(e);
	}
}
void MainWnd::keyReleaseEvent(QKeyEvent *e) { mGlWidget->keyReleaseEvent(e); }

void MainWnd::pause()
{
	mRequestPause = true;
	// dont call: mAcPlay/mAcPause ->setEnabled(true) here; wrong thread if
	// called from python
}
...
void MainWnd::showHelp() { mKbwView->show(); }

void MainWnd::nextRealGrid() { emit painterEvent(Painter::EventNextReal); }
void MainWnd::nextVec3Grid() { emit painterEvent(Painter::EventNextVec); }
void MainWnd::nextMesh() { emit painterEvent(Painter::EventNextMesh); }
void MainWnd::nextParts() { emit painterEvent(Painter::EventNextSystem); }
void MainWnd::nextPdata() { emit painterEvent(Painter::EventToggleParticles); }
...
void MainWnd::nextMeshDisplay() { emit painterEvent(Painter::EventMeshMode); }
...
void MainWnd::screenshot(QString file) { mGlWidget->screenshot(file); }
}
