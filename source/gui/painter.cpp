...
GridPainter<T>::GridPainter(FlagGrid **flags, QWidget *par)
    : LockedObjPainter(par), mMaxVal(0), mDim(0), mPlane(0), mMax(0),
      mLocalGrid(NULL), mFlags(flags), mInfo(NULL), mHide(false),
      mHideLocal(false), mDispMode(VecDispCentered), mValScale()
{
	mDim = 2; // Z plane
	mPlane = 0;
	mInfo = new QLabel();
...
template <class T> GridPainter<T>::~GridPainter()
{
	if (mLocalGrid) delete mLocalGrid;
}

template <class T> void GridPainter<T>::attachWidget(QLayout *layout)
{
	layout->addWidget(mInfo);
}

template <class T> void GridPainter<T>::update()
{
	...
	if ((mLocalGrid->getSize() != src->getSize()) ||
	    (mLocalGrid->getParent() != src->getParent())) {
		delete mLocalGrid;
		mLocalGrid = new Grid<T>(src->getParent());
		// int grid is base for resolution
		if (src->getType() & GridBase::TypeInt)
...
void GridPainter<T>::processKeyEvent(PainterEvent e, int param)
{
	if (e == EventSetDim) {
		...
		mPlane = param;
		if (mObject) {
			if (mMax > 0) mPlane = mPlane * mLocalGrid->getSize()[mDim] / mMax;
			mPlane = clamp(mPlane, 0, mLocalGrid->getSize()[mDim] - 1);
		}
...
template <class T> Real GridPainter<T>::getScale()
{
	if (!mObject) return 0;
	std::pair<void *, int> id;
	id.first = mObject;
	id.second = mDispMode;
	...
	if (mValScale.find(id) == mValScale.end()) {
		...
		if (mLocalGrid->getType() & GridBase::TypeVec3)
			...
			if (mDispMode == RealDispShadeVol)
				s = 4.0; // depends a bit on grid size in practice...
...
template <class T> void GridPainter<T>::setScale(Real v)
{
	std::pair<void *, int> id;
	id.first = mObject;
	id.second = mDispMode;
...
void GridPainter<Real>::processSpecificKeyEvent(PainterEvent e, int param)
{
	if (e == EventNextReal) {
		...
		if (mLocalGrid->getType() & GridBase::TypeLevelset)
			mDispMode = RealDispLevelset;
		else
			mDispMode = RealDispStd;
...
template <> void GridPainter<Real>::updateText()
{
	...
	s << "Display Plane " << mPlane << " [" << (char)('X' + mDim) << "]" << endl
	  << endl;
	if (mObject) {
		s << "Solver '" << mLocalGrid->getParent()->getName() << "'" << endl;
		s << "Grid resolution [" << mLocalGrid->getSizeX() << ", "
		  << mLocalGrid->getSizeY() << ", " << mLocalGrid->getSizeZ() << "]"
		  << endl;
		s << endl;
	}
	...
	if (mObject && !mHide && !mHideLocal) {
		...
		s << "-> Max " << fixed << setprecision(2) << mMaxVal << endl
		  << "-> Scale " << getScale() << endl;
	}
	mInfo->setText(s.str().c_str());
}
...
template <> void GridPainter<Vec3>::updateText()
{
	...
	if (mObject) {
		...
		if (mHide || mHideLocal)
			...
			s << "-> Max norm " << fixed << setprecision(2) << mMaxVal << endl
			  << "-> Scale " << getScale() << endl;
	}
	mInfo->setText(s.str().c_str());
}
...
template <> string GridPainter<int>::clickLine(const Vec3 &p0, const Vec3 &p1)
{
	...
	stringstream m;
	m << "Grid [ " << s.x << ", " << s.y << ", " << s.z << " ]" << endl
	  << mLocalGrid->getName() << ": " << mLocalGrid->get(s) << endl;
	return m.str();
}
...
template <> string GridPainter<Real>::clickLine(const Vec3 &p0, const Vec3 &p1)
{
	...
	if (!mLocalGrid->isInBounds(s)) return "";
	stringstream m;
	m << mLocalGrid->getName() << ": " << setprecision(2) << mLocalGrid->get(s)
	  << endl;
	return m.str();
}
...
template <> string GridPainter<Vec3>::clickLine(const Vec3 &p0, const Vec3 &p1)
{
	...
	if (!mLocalGrid->isInBounds(s)) return "";
	stringstream m;
	m << mLocalGrid->getName() << ": [ " << setprecision(2)
	  << mLocalGrid->get(s).x << ", " << mLocalGrid->get(s).y << ", "
	  << mLocalGrid->get(s).z << " ]" << endl;
	return m.str();
}
...
#define FOR_P_SLICE(__g, __dim, __plane)                                       \
	for (Vec3i __g0(__fRange(Vec3i(0, 0, 0), __dim, __plane)),                 \
	     __g1(__fRange((__g)->getSize(), __dim, __plane + 1)), p(__g0);        \
	     p.z < __g1.z; p.z++)                                                  \
		for (p.y = __g0.y; p.y < __g1.y; p.y++)                                \
			for (p.x = __g0.x; p.x < __g1.x; p.x++)
inline Vec3i __fRange(Vec3i size, int dim, int plane)
{
	Vec3i p(size);
	p[dim] = plane;
	return p;
...
{
	const int box[24] = { 0, 1, 0, 2, 0, 4, 7, 6, 7, 5, 7, 3,
		                  1, 3, 1, 5, 2, 3, 2, 6, 4, 5, 4, 6 };
	for (int i = 0; i < 24; i++) {
		const int b = box[i];
		glVertex(Vec3((b & 1) ? p1.x : p0.x, (b & 2) ? p1.y : p0.y,
		              (b & 4) ? p1.z : p0.z),
		         dx);
	}
}
...
{
	if (!mObject || mHide || mPlane < 0 ||
	    mPlane >= mLocalGrid->getSize()[mDim])
		return;
...
{
	if (!mObject || mHide || mHideLocal || mPlane < 0 ||
	    mPlane >= mLocalGrid->getSize()[mDim] || !mFlags || !(*mFlags))
		return;
...
template <> void GridPainter<Vec3>::paint()
{
	...
	if ((mDispMode == VecDispCentered) || (mDispMode == VecDispStaggered)) {
			...
			if (mDispMode == VecDispCentered) {
				if (mac) {
					if (p.x < mLocalGrid->getSizeX() - 1)
						vel.x = 0.5 *
						        (vel.x +
						         scale * mLocalGrid->get(p.x + 1, p.y, p.z).x);
					if (p.y < mLocalGrid->getSizeY() - 1)
						vel.y = 0.5 *
						        (vel.y +
						         scale * mLocalGrid->get(p.x, p.y + 1, p.z).y);
					if (p.z < mLocalGrid->getSizeZ() - 1)
						vel.z = 0.5 *
						        (vel.z +
						         scale * mLocalGrid->get(p.x, p.y, p.z + 1).z);
				}
				glColor3f(0, 1, 0);
				glVertex(pos, dx);
				glColor3f(1, 1, 0);
		...
		// draw as "uv" coordinates (ie rgb), note - this will completely hide
		// the real grid display!
		Vec3 box[4];
		glBegin(GL_QUADS);
		FOR_P_SLICE(mLocalGrid, mDim, mPlane)
		{
			...
			if (mac) {
				if (p.x < mLocalGrid->getSizeX() - 1)
					v.x = 0.5 *
					      (v.x + scale * mLocalGrid->get(p.x + 1, p.y, p.z).x);
				if (p.y < mLocalGrid->getSizeY() - 1)
					v.y = 0.5 *
					      (v.y + scale * mLocalGrid->get(p.x, p.y + 1, p.z).y);
				if (p.z < mLocalGrid->getSizeZ() - 1)
					v.z = 0.5 *
					      (v.z + scale * mLocalGrid->get(p.x, p.y, p.z + 1).z);
			}
			for (int c = 0; c < 3; ++c) {
				if (v[c] < 0.) v[c] *= -1.;
				v[c] = fmod((Real)v[c], (Real)1.);
			}
			// v *= mLocalGrid->get(0)[0]; // debug, show uv grid weight as
			// brightness of values
