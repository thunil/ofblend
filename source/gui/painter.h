...
class Painter : public QObject
{
  ...
  public:
	enum PainterEvent {
		...
		EventScaleVecUp,
		EventScaleVecDown,
		EventNextRealDisplayMode,
		EventScaleRealUp,
		EventScaleRealDown,
		EventScaleRealUpSm,
		EventScaleRealDownSm,
		EventChangePlane,
		EventSetPlane,
		EventSetDim,
		EventNextInt,
		EventNextReal,
		EventNextVec,
		EventNextVecDisplayMode,
		EventNextMesh,
		EventMeshMode,
		EventToggleGridDisplay,
		EventScaleMeshUp,
		EventScaleMeshDown,
		EventMeshColorMode,
		EventNextSystem,
		EventToggleParticles,
		EventNextParticleDisplayMode,
		EventToggleBackgroundMesh,
		EventSetMax,
		EventScalePdataDown,
		EventScalePdataUp
	};
...
/*! Derived classes have to implement paint, update, getID, processKeyEvent.
 * doEvent is handled in this class */
class LockedObjPainter : public Painter
{
	Q_OBJECT
  public:
	LockedObjPainter(QWidget *par = 0)
	    : Painter(par), mRequestUpdate(false), mObject(NULL), mObjIndex(-1)
	{
	}
	...
	doEvent(int e,
	        int param =
	            0); // don't overload, use processKeyEvent and update instead
...
template <class T> class GridPainter : public LockedObjPainter
{
  public:
