...
MeshPainter::MeshPainter(QWidget *par)
    : LockedObjPainter(par), mColorScale(1.0), mMode(ModeFlatShade),
      mVorticityMode(VModeNone), mBackgroundMode(BModeNormal), mLocalMesh(0),
      mBackground(0), mHide(false)
{
	mInfo = new QLabel();
}
...
{
	if (mLocalMesh) delete mLocalMesh;
}

void MeshPainter::attachWidget(QLayout *layout) { layout->addWidget(mInfo); }
...
void MeshPainter::processKeyEvent(PainterEvent e, int param)
{
	if (e == EventNextMesh)
		...
		mVorticityMode =
		    (VorticityMode)((mVorticityMode + 1) % (int)Num_VorticityModes);
	else if (e == EventToggleBackgroundMesh)
		mBackgroundMode =
		    (BackgroundMode)((mBackgroundMode + 1) % (int)Num_BackgroundModes);
	else
		return;
...
void MeshPainter::updateText()
{
	...
	if (mObject && !mHide && mLocalMesh) {
		s << "Mesh '" << mLocalMesh->getName() << "' [" << mLocalMesh->numTris()
		  << " tris]" << endl;
		...
		if (mLocalMesh->getType() == Mesh::TypeVortexSheet) {
			...
			if (mVorticityMode == VModeSmoothed)
				s << "Vorticity: gauss smooth" << endl;
			if (mVorticityMode == VModeDiff)
				s << "Vorticity: gauss diff" << endl;
			if (mVorticityMode == VModeSmoke) s << "Smoke Density" << endl;
			if (mVorticityMode == VModeTex)
				s << "Texture coordinates 0" << endl;
			if (mVorticityMode != VModeNone)
				s << "  Scale " << 1.0 / mColorScale << "" << endl;
		}
	}
	mInfo->setText(s.str().c_str());
}

void MeshPainter::setBackgroundMesh(Mesh *bgr) { mBackground = bgr; }
...
void MeshPainter::setupLights(bool specular)
{
	...
	if (shininess == 0.)
		specReflection[0] = specReflection[1] = specReflection[2] = 0.;
...
// glColor3f(std::max(_0,std::min(_1,color.x)),
// std::max(_0,std::min(_1,color.y)), std::max(_0,std::min(_1,color.z)));
//}
...
static inline void glNormal(const Vec3 &v) { glNormal3f(v.x, v.y, v.z); }
...
void MeshPainter::paint()
{
	...
	bool triColor = (mMode == ModeFlatShade) &&
	                (mLocalMesh->getType() == Mesh::TypeVortexSheet) &&
	                (mVorticityMode != VModeNone);
	bool nodeColor = (mLocalMesh->getType() == Mesh::TypeVortexSheet) &&
	                 (mVorticityMode == VModeTex);
	...
	if (triColor) glDisable(GL_LIGHTING);
	...
	if (mMode == ModeFlatShade || mMode == ModeTrans) {
		...
		for (int tri = 0; tri < numTris; tri++) {
			if (!nodeColor && triColor) {
				VortexSheetInfo &info =
				    ((VortexSheetMesh *)mLocalMesh)->sheet(tri);
			...
			for (int c = 0; c < 3; c++) {
				if (nodeColor) {
					Vec3 tc = ((VortexSheetMesh *)mLocalMesh)
					              ->tex1(mLocalMesh->tris(tri).c[c]);
					// Vec3 tc2 =
					// ((VortexSheetMesh*)mLocalMesh)->tex2(mLocalMesh->tris(tri).c[c]);
					// Vec3 tc = gAlpha*tc1+(1-gAlpha)*tc2;
					tc = mColorScale *
					     (tc / toVec3(mLocalMesh->getParent()->getGridSize()));
	...
	if (mMode == ModePoints) {
		...
		// static const Vec3 colortable[] = { Vec3(0.5), Vec3(1,0,0),
