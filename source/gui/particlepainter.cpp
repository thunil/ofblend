...
ParticlePainter::ParticlePainter(GridPainter<int> *gridRef, QWidget *par)
    : LockedObjPainter(par), mGridRef(gridRef), mLocal(0), mMode(PaintVel),
      mDisplayMode(0), mLastPdata(-1), mHavePdata(false), mMaxVal(0.)
{
	mInfo = new QLabel();
}
...
void ParticlePainter::updateText()
{
	...
	if (mObject && !(mMode == PaintOff)) {
		...
		s << mPdataInfo;
		if (mHavePdata) {
			s << "-> Max " << fixed << setprecision(2) << mMaxVal << "  Scale "
			  << getScale() << endl;
		}
	}
...
void ParticlePainter::paint()
{
	...
	glDisable(GL_BLEND);
	glDisable(GL_DEPTH_TEST); // disable depth test for particles, clashes with
	                          // display plane for regular ones
...
void ParticlePainter::paintBasicSys()
{
	...
	if (pdataId == 0) {
		...
		case ParticleDataBase::TypeReal: {
			ParticleDataImpl<Real> *pdi =
			    dynamic_cast<ParticleDataImpl<Real> *>(pdb);
			if (!pdi) break;
			mHavePdata = true;
			drewPoints = true;
			glPointSize(1.5);
			...
			glEnd();
			infoStr << "Pdata '" << pdi->getName() << "' #" << pdNum
			        << ", real\n";
		} break;

		case ParticleDataBase::TypeInt: {
			ParticleDataImpl<int> *pdi =
			    dynamic_cast<ParticleDataImpl<int> *>(pdb);
			if (!pdi) break;
			mHavePdata = true;
			drewPoints = true;
			glPointSize(1.5);
		...
		case ParticleDataBase::TypeVec3: {
			ParticleDataImpl<Vec3> *pdi =
			    dynamic_cast<ParticleDataImpl<Vec3> *>(pdb);
			if (!pdi) break;
			mHavePdata = true;
			...
			case 2:
				...
				for (int i = 0; i < (int)bp->size(); i++) {
					...
					val[2] += 0.5; // base blue
					for (int c = 0; c < 3; ++c)
						val[c] = std::min((Real)val[c], (Real)1.);
