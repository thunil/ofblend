...
CustomCheckbox::CustomCheckbox(string name, bool val)
    : mVal(val), mSName(name), mCheckbox(0)
{
}
...
bool TextCheckbox::get() { return isChecked(); }

void TextCheckbox::set(bool v) { setChecked(v); }
...
Gui::Gui() : PbClass(NULL), mGuiPtr(gGuiThread), mMainPtr(gMainThread) {}

void Gui::setBackgroundMesh(Mesh *m) { mGuiPtr->getWindow()->setBackground(m); }
void Gui::show(bool twoD)
{
	if (twoD) mMainPtr->send((int)MainWnd::EventSet2DCam);
	mMainPtr->sendAndWait((int)MainWnd::EventGuiShow);
}
void Gui::update() { updateQtGui(true, -1, -1., ""); }
void Gui::pause()
{
...
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextRealGrid");
}
void Gui::nextVec3Grid()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextVec3Grid");
}
void Gui::nextParts()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextParts");
}
void Gui::nextPdata()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextPdata");
}
void Gui::nextMesh()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextMesh");
}

void Gui::nextVec3Display()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextVec3Display");
}
void Gui::nextPartDisplay()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextPartDisplay");
}
void Gui::nextMeshDisplay()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "nextMeshDisplay");
}

void Gui::toggleHideGrids()
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "toggleHideGrids");
}

void Gui::setCamPos(float x, float y, float z)
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "setCamPos",
	                          Q_ARG(float, x), Q_ARG(float, y),
	                          Q_ARG(float, z));
}
...
{
	QMetaObject::invokeMethod(mGuiPtr->getWindow(), "windowSize", Q_ARG(int, w),
	                          Q_ARG(int, h));
}
...
PbClass *Gui::addControl(PbType t)
{
	...
	if (t.str() == "")
		throw Error(
		    "Need to specify object type. Use e.g. gui.create(Slider, ...)");
