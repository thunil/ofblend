...
GLWidget::GLWidget(QWidget *p)
    : QGLWidget(QGLFormat(QGL::SampleBuffers), p), mRotX(0), mRotY(0),
      mGridsize(0), mScreenshotNumber(0), mWidth(800), mHeight(600)
{
	mPlaneDim = 2; // Y plane
	mPlane = -1;
	mCamPos = Vec3(0, 0, -2);
...
QSize GLWidget::minimumSizeHint() const { return QSize(400, 300); }

QSize GLWidget::sizeHint() const { return QSize(mWidth, mHeight); }
...
void GLWidget::mouseReleaseEvent(QMouseEvent *event)
{
	...
	if ((mDownPos - pos).manhattanLength() == 0) {
		...
		if (!gluUnProject(wx, wy, 0, modelMatrix, projMatrix, viewport,
		                  &line[0], &line[1], &line[2]))
			return;
		if (!gluUnProject(wx, wy, 1.0, modelMatrix, projMatrix, viewport,
		                  &line[3], &line[4], &line[5]))
			return;

		// calculate intersection with plane
		emit clickLine(event->globalPos(), line[0], line[1], line[2], line[3],
		               line[4], line[5]);
	}
}
...
void GLWidget::timerEvent(QTimerEvent *e)
{
	...
	if (mMoveState[MoveUp]) {
		mCamPos.y -= speed;
		doRepaint = true;
	}
	if (mMoveState[MoveDown]) {
		mCamPos.y += speed;
		doRepaint = true;
	}
	if (mMoveState[MoveOut]) {
		mCamPos.z -= speed;
		doRepaint = true;
	}
	if (mMoveState[MoveIn]) {
		mCamPos.z += speed;
		doRepaint = true;
	}
	if (doRepaint) updateGL();
}

void GLWidget::setViewport(const Vec3i &gridsize)
{
	if (mGridsize.x != gridsize.x || mGridsize.y != gridsize.y ||
...
bool GLWidget::keyProcess(int key, int modifier, bool down)
{
	...
	if (key == Qt::Key_A) {
		mMoveState[MoveLeft] = down;
		mMoveFast = shift;
	} else if (key == Qt::Key_D) {
		mMoveState[MoveRight] = down;
		mMoveFast = shift;
	} else if (key == Qt::Key_W) {
		mMoveState[MoveUp] = down;
		mMoveFast = shift;
	} else if (key == Qt::Key_S) {
		mMoveState[MoveDown] = down;
		mMoveFast = shift;
	} else if (key == Qt::Key_Q) {
		mMoveState[MoveIn] = down;
		mMoveFast = shift;
	} else if (key == Qt::Key_E) {
		mMoveState[MoveOut] = down;
		mMoveFast = shift;
	} else if (down) {
		// only press events
		...
		// data grids, first int
		         ...
		         shift) { /* int display mdoes, not yet used */
		} else if (key == Qt::Key_X) {
			emit painterEvent(Painter::EventNextInt);
			updatePlane(mPlane);
		}
		// real
		else if (key == Qt::Key_C && shift) {
			emit painterEvent(
			    Painter::EventNextRealDisplayMode); /* real display modes */
		} else if (key == Qt::Key_C) {
			emit painterEvent(Painter::EventNextReal);
			updatePlane(mPlane);
		} else if (shift && ((key == Qt::Key_Less) || (key == Qt::Key_Comma))) {
			emit painterEvent(Painter::EventScaleRealDownSm);
		} else if (shift &&
		           ((key == Qt::Key_Greater) || (key == Qt::Key_Period))) {
			emit painterEvent(Painter::EventScaleRealUpSm);
		} else if ((key == Qt::Key_Less) || (key == Qt::Key_Comma)) {
			emit painterEvent(Painter::EventScaleRealDown);
		} else if ((key == Qt::Key_Greater) || (key == Qt::Key_Period)) {
			emit painterEvent(Painter::EventScaleRealUp);
		}

		// vec3 grids, scaling can be used with two key combinations (the second
		// one is for international keyboards)
		else if (key == Qt::Key_V && shift) {
		...
		// grid scaling
			...
			emit painterEvent(Painter::EventScaleVecDown);
		} else if (key == Qt::Key_Apostrophe) {
			emit painterEvent(Painter::EventScaleVecUp);
		}

		// particles
		else if (key == Qt::Key_B && shift) {
			emit painterEvent(Painter::EventNextParticleDisplayMode);
		} else if (key == Qt::Key_B && alt) {
			emit painterEvent(Painter::EventNextSystem);
		} else if (key == Qt::Key_B) {
			emit painterEvent(Painter::EventToggleParticles);
		}

		else if ((key == Qt::Key_ParenLeft) || // a bit ugly, but for some
		                                       // reason parentheses dont work
		                                       // in some cases... fall back
		                                       // with dual assignment
		         (key == Qt::Key_9)) {
			emit painterEvent(Painter::EventScalePdataDown);
		} else if ((key == Qt::Key_ParenRight) || (key == Qt::Key_0)) {
			emit painterEvent(Painter::EventScalePdataUp);
		}
		...
		// mesh display
			...
			emit painterEvent(Painter::EventScaleMeshDown);
		} else if (key == Qt::Key_BracketLeft) {
			emit painterEvent(Painter::EventScaleMeshDown);
		} else if (key == Qt::Key_BraceRight) {
			emit painterEvent(Painter::EventScaleMeshUp);
		} else if (key == Qt::Key_BracketRight) {
			emit painterEvent(Painter::EventScaleMeshUp);
		}
		// special mesh display modes
		else if (key == Qt::Key_M && alt)
			emit painterEvent(Painter::EventMeshColorMode);
		else if (key == Qt::Key_M && ctrl)
			emit painterEvent(Painter::EventToggleBackgroundMesh);
		...
		// move plane (+shift is faster)
			...
			QString filename = QString("scr_%1.png").arg(
			    QString::number(mScreenshotNumber), 3, QChar('0'));
			screenshot(filename);
			mScreenshotNumber++;
		}
...
void GLWidget::screenshot(QString file) { grabFrameBuffer().save(file); }
