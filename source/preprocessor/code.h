...
struct Text
{
	...
	void prequel(const Text *a)
	{
		minimal = a->minimal + minimal;
		original = a->original + original;
	}
	void post(const Text *a)
	{
		minimal += a->minimal;
		original += a->original;
	}
...
template <class T> struct List : Text
{
	std::vector<T> _data;
	std::string listText;
	...
	std::string copier(const std::string &prefix, bool useValue) const
	{
		return "";
	};
...
{
	Function()
	    : returnType(), isInline(false), isVirtual(false), isConst(false),
	      noParentheses(false), isOperator(false)
	{
	}
...
{
	Instantiation(const std::string &c, const std::string &n) : cls(c), name(n)
	{
	}
	std::string cls, name;
...
struct Class : Text
{
	...
	{
		return isTemplated() ? (name + "<" + templateTypes.names() + ">")
		                     : name;
	}
	inline bool isTemplated() const { return !templateTypes.empty(); }
...
template <>
std::string List<Argument>::copier(const std::string &prefix,
                                   bool useVal) const;
