...
void TokenPointer::consumeWhitespace()
{
	...
	while (cur().type == TkWhitespace || cur().type == TkComment) {
		string minimal =
		    (!txt->minimal.empty() && *(txt->minimal).rbegin() == ' ') ? ""
		                                                               : " ";
		forward(minimal, cur().text, 1);
		if (ptr >= (int)queue.size())
			errMsg(-1,
			       "Preprocessor ran out of tokens. This shouldn't happen.");
	}
}
...
void TokenPointer::forward(const string &minimal, const string &original,
                           int offset)
{
	txt->minimal += minimal;
	txt->original += original;
	ptr += offset;
	if (parent) parent->forward(minimal, original, offset);
}

string TokenPointer::backtrace()
{
	string bt = "[" + txt->dynamicClass() + "]: " + txt->original + "\n";
	if (parent) bt += parent->backtrace();
	return bt;
}
...
void TokenPointer::errorMsg(const string &msg)
{
	...
	int line = -1;
	if (!queue.empty()) line = done() ? queue.back().line : cur().line;
...
void tokenizeBlock(vector<Token> &tokens, const string &kw, const string &text,
                   size_t &i, int &line, bool &hasBrackets)
{
...
void processText(const string &text, int baseline, Sink &sink,
                 const Class *parent, vector<Instantiation> &inst)
{
		...
		if (comment) {
			...
			if (isNameChar(c))
				...
				if (word == "KERNEL" || word == "PYTHON") {
					...
					if (!brackets) {
						errMsg(line,
						       "KERNEL and PYTHON keywords must have \"()\" ");
					}
