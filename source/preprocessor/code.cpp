...
bool Type::operator==(const Type &a) const
{
	...
	return a.name == name && a.isConst == isConst && a.isRef == isRef &&
...
string Type::build(bool refify) const
{
	...
	if (isConst) s += "const ";
	s += name;
	if (!templateTypes.empty()) s += templateTypes.minimal;
...
{
	string s;
	if (isTemplated()) s = "template " + templateTypes.minimal;
...
template <>
std::string List<Argument>::copier(const std::string &prefix, bool useVal) const
{
	std::stringstream s;
	for (int i = 0; i < (int)_data.size(); i++) {
		s << ',' << _data[i].name << '(';
