...
void usage()
{
	...
	cerr << "  Usage : prep generate <dbg_mode> <mt_type> <inputdir> "
	        "<inputfile> <outputfile>" << endl;
	cerr << "     or : prep docgen <dbg_mode> <mt_type> <inputdir> <inputfile> "
	        "<outputfile>" << endl;
...
void doGenerate(int argc, char *argv[], bool docs)
{
	...
	bool isPython =
	    infile.size() > 3 && !infile.compare(infile.size() - 3, 3, ".py");

	// TP : only enable in cmake's PREP_DEBUG mode (passed via cmd line option
	// dbg_mode)
	...
	if (text.empty()) {
		cerr << "preprocessor error: Can't read file '" << infile << "'"
		     << endl;
		exit(1);
	}
	...
	if (gDocMode) {
		...
		sink.inplace << "\n\n\n\n\n// DO NOT EDIT !\n";
		sink.inplace << "// This file is generated using the MantaFlow "
		                "preprocessor (prep generate).";
		sink.inplace << "\n\n\n\n\n";
	}
	...
	if (isPython) {
		...
		replaceAll(text, "\t", "\\t");
		replaceAll(text, "\"",
		           "<qtm>"); // split into two ops to avoid interference
		replaceAll(text, "<qtm>", "\\\"");
		sink.link << "#include \"registry.h\"\n";
		sink.link << "static const Pb::Register _reg(\"" + infile + "\", \"" +
		                 text + "\");\n";
	} else {
		if (!gDocMode) {
