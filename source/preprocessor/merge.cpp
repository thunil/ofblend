...
void resolveRequests(RegFile &file)
{
	...
	// process requests
	for (map<string, vector<Request *> >::iterator it = sortedReqs.begin();
	     it != sortedReqs.end(); ++it) {
		...
		for (int i = 0; i < (int)it->second.size(); i++) {
			...
			for (int j = 0; j < (int)info.snippets.size(); j++) {
				stringstream idxStr;
				idxStr << file.idx++;
				const string table[] = { "CT",  req.tpl,      "BT", req.base,
					                     "IDX", idxStr.str(), "" };
...
void generateMerge(int num, char *files[])
{
	...
	for (int i = 0; i < num; i++) {
		...
		if (regFiles[i]->idx > 0) {
			text = "\n\n\n\n\n// DO NOT EDIT !\n";
			text += "// This file is generated using the MantaFlow "
			        "preprocessor (prep link).";
			text += "\n\n\n\n\n";
			...
			text = regFiles[i]->header.str() + regFiles[i]->out.str() +
			       regFiles[i]->footer.str();
		}
