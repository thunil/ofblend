...
string parseRunaway(TokenPointer &parentPtr)
{
	...
	for (; !tk.done(); tk.next()) {
		if (stack.empty() &&
		    (tk.curType() == TkComma || tk.curType() == TkBracketR ||
		     tk.curType() == TkTBracketR)) {
			break;
		}
...
Argument parseArgument(TokenPointer &parentPtr, bool requireName,
                       bool requireType)
{
	Argument cur = Argument();
	TokenPointer tk(parentPtr, &cur);

	if (requireType) cur.type = parseType(tk);

	if (tk.curType() != TkDescriptor && !requireName) return cur;
	...
	if (tk.curType() == TkBracketL ||
	    (tk.curType() == TkOperator && tk.cur().text == "=")) {
		if (tk.curType() == TkOperator) tk.next();
...
List<Type> parseTypeList(TokenPointer &parentPtr)
{
	...
	if (tk.curType() != TkTBracketR) {
		for (;;) {
			...
			if (tk.curType() == TkTBracketR) break;
			tkAssert(tk.curType() == TkComma,
...
List<Argument> parseArgumentList(TokenPointer &parentPtr, bool requireName,
                                 bool requireType)
{
	List<Argument> list;
	TokenPointer tk(parentPtr, &list);
	...
	if (tk.curType() != TkBracketR) {
		for (int idx = 0;; idx++) {
			...
			list.back().index = idx;
			if (tk.curType() == TkBracketR) break;
			tkAssert(tk.curType() == TkComma,
			         "expect comma or closing bracket");
			tk.next();
		}
...
Function parseFunction(TokenPointer &parentPtr, bool requireNames,
                       bool requireType, bool requireArgs)
{
	Function cur;
	TokenPointer tk(parentPtr, &cur);
	...
	if (requireType) cur.returnType = parseType(tk);
...
Class parseClass(TokenPointer &parentPtr)
{
	...
	tk.next();
	tkAssert(tk.curType() == TkDescriptor, "malformed preprocessor keyword "
	                                       "block. Expected 'PYTHON class name "
	                                       ": public X {}'");
	cur.name = tk.cur().text;
	tk.next();
	tkAssert(tk.curType() == TkColon,
	         "PYTHON class must publicly derive from PbClass (or a subclass)");
	tk.next();
	tkAssert(tk.curType() == TkPublic,
	         "PYTHON class must publicly derive from PbClass (or a subclass)");
	tk.next();
	tkAssert(tk.curType() == TkDescriptor,
	         "PYTHON class must publicly derive from PbClass (or a subclass)");
...
void parseBlock(const string &kw, const vector<Token> &tokens,
                const Class *parent, Sink &sink, vector<Instantiation> &inst)
{
	Block block = Block();
	block.parent = parent;
	...
	if (kw == "KERNEL") {
		...
		if (!templTypes.empty()) block.func.templateTypes = templTypes;

		tkAssert(tk.curType() == TkCodeBlock && tk.isLast(),
		         "Malformed KERNEL, expected KERNEL(opts...) ret_type "
		         "name(args...) { code }");
		...
		if (tk.curType() == TkDescriptor &&
		    (tk.cur().text == "alias" || tk.cur().text == "instantiate")) {
			...
			if (kw == "alias") {
				tkAssert(tk.curType() == TkDescriptor,
				         "malformed preprocessor block. Expected 'PYTHON alias "
				         "cname pyname;'");
			...
			tkAssert(tk.curType() == TkSemicolon && tk.isLast(),
			         "malformed preprocessor block. Expected 'PYTHON "
			         "alias/instantiate cname [pyname];'");
		...
		} else // function or member
		{
			...
			if (isConstructor && tk.curType() == TkColon) {
				// read till end
				while (!tk.done() && tk.curType() != TkSemicolon &&
				       tk.curType() != TkCodeBlock) {
					block.initList += tk.cur().text;
					tk.next();
				}
				tkAssert(!tk.done(),
				         "Constructor initializer list not limited");
			}

			if (tk.curType() == TkSemicolon && block.func.noParentheses) {
				tkAssert(tk.curType() == TkSemicolon && tk.isLast(),
				         "malformed preprocessor keyword block. Expected "
				         "'PYTHON type varname;'");
				processPythonVariable(block, sink);
			} else {
				tkAssert((tk.curType() == TkCodeBlock ||
				          tk.curType() == TkSemicolon) &&
				             tk.isLast(),
				         "malformed preprocessor keyword block. Expected "
				         "'PYTHON type funcname(args) [{}|;]'");
