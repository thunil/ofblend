...
class GridCgInterface
{
  public:
	...
	// precond
	virtual void setPreconditioner(PreconditionType method, Grid<Real> *A0,
...
template <class APPLYMAT> class GridCg : public GridCgInterface
{
  public:
	//! constructor
	GridCg(Grid<Real> &dst, Grid<Real> &rhs, Grid<Real> &residual,
	       Grid<Real> &search, FlagGrid &flags, Grid<Real> &tmp, Grid<Real> *A0,
	       Grid<Real> *pAi, Grid<Real> *pAj, Grid<Real> *pAk);
	~GridCg() {}
	...
	void setPreconditioner(PreconditionType method, Grid<Real> *A0,
	                       Grid<Real> *Ai, Grid<Real> *Aj, Grid<Real> *Ak);
                 ...
                 Grid<Real> &A0, Grid<Real> &Ai, Grid<Real> &Aj, Grid<Real> &Ak)
{
	...
	dst[idx] = src[idx] * A0[idx] + src[idx - X] * Ai[idx - X] +
	           src[idx + X] * Ai[idx] + src[idx - Y] * Aj[idx - Y] +
	           src[idx + Y] * Aj[idx] + src[idx - Z] * Ak[idx - Z] +
	           src[idx + Z] * Ak[idx];
}
...
void ApplyMatrix2D(FlagGrid &flags, Grid<Real> &dst, Grid<Real> &src,
                   Grid<Real> &A0, Grid<Real> &Ai, Grid<Real> &Aj,
                   Grid<Real> &Ak)
{
	unusedParameter(
	    Ak); // only there for parameter compatibility with ApplyMatrix
...
KERNEL(bnd = 1)
void MakeLaplaceMatrix(FlagGrid &flags, Grid<Real> &A0, Grid<Real> &Ai,
                       Grid<Real> &Aj, Grid<Real> &Ak, MACGrid *fractions = 0)
{
