...
static Real _aCoeffs[32] = {
	0.000334, -0.001528, 0.000410,  0.003545, -0.000938, -0.008233, 0.002172,
	0.019120, -0.005040, -0.044412, 0.011655, 0.103311,  -0.025936, -0.243780,
	0.033979, 0.655340,  0.655340,  0.033979, -0.243780, -0.025936, 0.103311,
	0.011655, -0.044412, -0.005040, 0.019120, 0.002172,  -0.008233, -0.000938,
...
void WaveletNoiseField::upsample(Real *from, Real *to, int n, int stride)
{
	...
	for (int i = 0; i < n; i++) {
		...
		for (int k = i / 2 - 1; k < i / 2 + 3; k++) {
			to[i * stride] +=
			    0.5 * pp[k - i / 2] * from[modSlow(k, n / 2) * stride];
		} // new */
	}
}
...
WaveletNoiseField::WaveletNoiseField(FluidSolver *parent, int fixedSeed,
                                     int loadFromFile)
    : PbClass(parent), mPosOffset(0.), mPosScale(1.), mValOffset(0.),
      mValScale(1.), mClamp(false), mClampNeg(0), mClampPos(1), mTimeAnim(0),
      mGsInvX(0), mGsInvY(0), mGsInvZ(0)
{
...
void WaveletNoiseField::generateTile(int loadFromFile)
{
	...
	for (int i = 0; i < n3d; i++) {
		// noise3[i] = (randStream.getReal() + randStream2.getReal()) -1.; //
		// produces repeated values??
	...
	// Avoid even/odd variance difference by adding odd-offset version of noise
	// to itself.
	int offset = n / 2;
	...
	if (n != 128)
		errMsg(
		    "WaveletNoise::Fast 128 mod used, change for non-128 resolution");
	...
	for (int tile = 0; tile < 3; tile++)
		for (int ix = 0; ix < n; ix++)
			for (int iy = 0; iy < n; iy++)
				for (int iz = 0; iz < n; iz++) {
					temp13[icnt] =
					    noise3[modFast128(ix + offset) +
					           modFast128(iy + offset) * n +
...
void WaveletNoiseField::downsampleNeumann(const Real *from, Real *to, int n,
                                          int stride)
{
...
void WaveletNoiseField::upsampleNeumann(const Real *from, Real *to, int n,
                                        int stride)
{
                                            ...
                                            Grid<Real> &tempIn2)
{
	...
	FOR_IJK_BND(input, 1)
	{
		...
		Real res = temp13[k * sx * sy + j * sx + i - 1] +
		           temp13[k * sx * sy + j * sx + i + 1];
		res += temp13[k * sx * sy + j * sx + i - sx] +
		       temp13[k * sx * sy + j * sx + i + sx];
		if (input.is3D())
