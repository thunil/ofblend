...
struct VortexRing
{
	...
	VortexRing(Real c, bool closed = false)
	    : isClosed(closed), circulation(c), flag(0)
	{
	}
	void renumber(int *_renumber);
	inline int size() const { return indices.size(); }
	inline int idx(int i) const
	{
...
//! Vortex filaments
PYTHON class VortexFilamentSystem
    : public ConnectedParticleSystem<BasicParticleData, VortexRing>
{
  public:
	...
	PYTHON void advectSelf(Real scale = 1.0, Real regularization = 0.1,
	                       int integrationMode = IntRK4);
	//! advect a particle system
	PYTHON void advectParticles(BasicParticleSystem &sys, Real scale = 1.0,
	                            Real regularization = 0.1,
	                            int integrationMode = IntRK2);
	//! advect triangle mesh using filaments
	PYTHON void advectMesh(Mesh &mesh, Real scale = 1.0,
	                       Real regularization = 0.1,
	                       int integrationMode = IntRK4);
	...
	PYTHON void addRing(const Vec3 &position, Real circulation, Real radius,
	                    Vec3 normal, int number);
  ...
  protected:
	...
	void integrate(const std::vector<Vec3> &nodesOld,
	               std::vector<Vec3> &nodesNew, Real scale, Real reg,
	               int integrationMode);
};
