...
#define FOR_IDX(grid)                                                          \
	for (IndexInt idx = 0, total = (grid).getSizeX() * (grid).getSizeY() *     \
	                               (grid).getSizeZ();                          \
	     idx < total; idx++)
...
#define FOR_IJKT_BND(grid, bnd)                                                \
	for (int t = ((grid).is4D() ? bnd : 0);                                    \
	     t < ((grid).is4D() ? ((grid).getSizeT() - bnd) : 1); ++t)             \
		for (int k = ((grid).is3D() ? bnd : 0);                                \
		     k < ((grid).is3D() ? ((grid).getSizeZ() - bnd) : 1); ++k)         \
			for (int j = bnd; j < (grid).getSizeY() - bnd; ++j)                \
				for (int i = bnd; i < (grid).getSizeX() - bnd; ++i)

//! Basic data structure for kernel data, initialized based on kernel type (e.g.
//single, idx, etc).
struct KernelBase
{
